...
class DymolaMatDiffer:
  ...
  def diff(self):
    ...
    goldfiles = []
    filesingold = os.listdir(self.__goldDir)
    
    for file in filesingold:
        if file.endswith(".mat"):
            goldfiles.append(file)
    
    numgoldfiles = len(goldfiles)
    failedvec = [True]*numgoldfiles
    ...
    for file in goldfiles:
        ...
        matgold = scipy.io.loadmat(os.path.join(self.__goldDir,file), chars_as_strings=False)
    ...
    # Check the version of the output file (version 1.1).
      ...
      # #######Test if they have the same size####
          ...
          #End of new code  
                    ...
                    diffVariables2 = relativeDf.columns[mask]
                    if len(diffVariables2):
                        failedvec[location] = False
                        print("Failed file: ", file)
                    else:
                        failedvec[location] = True
                        print("Successful file: ", file)
                       
                    if location>0:
                        diffVariables = np.append(diffVariables,relativeDf.columns[mask])
                        maxValuestolist = np.append(maxValuestolist,maxValues[mask])
                    else:
  
                        diffVariables = relativeDf.columns[mask]
                        maxValuestolist = maxValues[mask]
            else:
    ...
    if True not in failedvec:
        diffVariableslist =  diffVariables.tolist()
        
        maxValueslist = maxValuestolist.flatten().tolist()
        
        
        res =  "\nMax_Rel_Diff  Variable\n"
        res += "\n".join("{:.10f}  {} ".format(x, y) for x, y in zip(maxValueslist, diffVariableslist))
