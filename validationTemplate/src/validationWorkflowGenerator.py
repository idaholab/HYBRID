...
def readInputFile(inputFilePath, logger):
	...
	targetList = list(x.strip()
										for x in config.get('MODEL', 'targets').split(','))
	model['targets'] = {}
	try:
			it = iter(targetList)
			for x in it:
			...
			logger.exception(
					"Targets should be listed as such: aliasSRQ_1, pathSRQ_1, aliasSRQ_2, pathSRQ_2...")
			raise e
	for srq, path in model['targets'].items():
			logger.info(
					'SRQ for the validation/calibration : {} \n \t\tPath in Dymola model : {} '.format(str(srq), str(path)))
	try:
	...
	if exp['pp']:
			...
			try:
					assert os.path.exists(
							postprocesssingScript), "The post processing script passed does not exist !"
					code = exp['pp_script']
					data = exp['dataset']
					ppResults = subprocess.run(
							["python", code, data], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=True)
	...
	for var in config['VARIABLES'].keys():
			distrib = list(x.strip()
										 for x in config.get('VARIABLES', var).split(','))
			path = distrib[0]
			distribName = distrib[1].strip()    # name of the variable distribution
			try:
					assert distribName in authorized['distributions'].keys(), \
					"Use one of the following distribution for your variable: {}".format(
							authorized['distributions'].keys())
					# list of numerical parameters of the chosen distribution
					distribParams = [float(k) for k in distrib[2:]]
			except AssertionError as e:
					logger.exception('A test failed : ')
					raise e
	...
	if calibration:
			...
			for var in variables.keys():
					try:
							...
							logger.exception('A test failed : ')
							raise e
					initials = [float(elem) for elem in config.get(
							'OPTIMIZATION', var).split(',')]
	...
	if calibration:
			...
			settings['calibration_file'] = config.get(
					'SETTINGS', 'calibration_file', fallback=calFileFallback)+'.xml'
	...
	if config.has_section('ROM'):
			...
			if type == 'DMDc':
					...
					rom['PolyExponential']['numberExpTerms'] = config.getint(
							'ROM', 'numberExpTerms', fallback=5)
					rom['PolyExponential']['coeffRegressor'] = config.get(
							'ROM', 'coeffRegressor', fallback='nearest')
					rom['PolyExponential']['tol'] = config.getfloat(
							'ROM', 'tol', fallback=0.001)
					rom['PolyExponential']['max_iter'] = config.getint(
							'ROM', 'max_iter', fallback=1000)
					if rom['PolyExponential']['coeffRegressor'] == 'poly' and config.has_option('ROM', 'polyOrder'):
							rom['PolyExponential']['polyOrder'] = config.getint(
									'ROM', 'polyOrder', fallback=5)
					...
					rom['DMD']['exactModes'] = config.get(
							'ROM', 'exactModes', fallback=True)
			else:
					# Type not recognized
					try:
							...
							logger.exception('A test failed : ')
							raise e
			rom['pivotParameter'] = config.get(
					'ROM', 'pivotParameter', fallback='Time')
			...
			rom['workDir'] = config.get(
					'SETTINGS', 'work_dir', fallback=workDirFallback)
...
def writeXMLValidation(model, exp, variables, metric, validationFile, calibration, romTag):
	...
	valTemplate = ValidationTemplate(
			authorized['metrics'], authorized['distributions'], calibration)
	if calibration and romTag:
			valTemplate.loadTemplate('validation_ROM_incomplete_XML.xml', os.path.dirname(
					inspect.getfile(ValidationTemplate)))
	elif calibration:
			valTemplate.loadTemplate('validation_incomplete_XML.xml', os.path.dirname(
					inspect.getfile(ValidationTemplate)))
	else:
			valTemplate.loadTemplate('validation_incomplete_XML_valonly.xml', os.path.dirname(
					inspect.getfile(ValidationTemplate)))
	...
	valXMLToWrite = valTemplate.createWorkflow(
			model=model, exp=exp, variables=variables, metric=metric, romTag=romTag)
