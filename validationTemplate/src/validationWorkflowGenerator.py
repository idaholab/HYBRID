...
def readInputFile(inputFilePath, logger):
    ...
    if exp['pp']: 
        ...
        try: 
            ...
            code = exp['pp_script']
            data = exp['dataset']
            pp_results = subprocess.run(["python", code, data], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=True)
            logger.info(str(pp_results.stdout.decode('utf-8')))
            new_path = str(pp_results.stdout.decode('utf-8'))
            exp['clean_dataset'] = new_path
    ...
    for var in config['VARIABLES'].keys():
        ...
        path = distrib[0]
        distrib_name = distrib[1].strip()    # name of the variable distribution
        try:
            assert distrib_name in authorized['distributions'].keys(), \
            "Use one of the following distribution for your variable: {}".format(authorized['distributions'].keys())
            distrib_params = [float(k) for k in distrib[2:]] # list of numerical parameters of the chosen distribution
        except AssertionError as e:
            logger.exception('A test failed : ')
            raise e        
        variables[var] = {'path':path,
                        'distribution': distrib_name,
                        'dist_params': distrib_params, 
                        'alias':var}
...
def writeXMLValidation(model, exp, variables, metric, validationFile, calibration, romTag): 
    ...
    validation_template = ValidationTemplate(authorized['metrics'], authorized['distributions'], calibration)
    if calibration and romTag: 
        validation_template.loadTemplate('validation_ROM_incomplete_XML.xml', os.path.dirname(inspect.getfile(ValidationTemplate)))
    elif calibration:
        validation_template.loadTemplate('validation_incomplete_XML.xml', os.path.dirname(inspect.getfile(ValidationTemplate)))
    else: 
        validation_template.loadTemplate('validation_incomplete_XML_valonly.xml', os.path.dirname(inspect.getfile(ValidationTemplate)))
    ...
    valXMLToWrite = validation_template.createWorkflow(model=model, exp=exp, variables=variables, metric=metric, romTag=romTag)
    errors = validation_template.writeWorkflow(valXMLToWrite, validationFile, run=False)
    if errors == 0:
...
def writeXMLCalibration(model, exp, variables, metric, optimization, work_dir, calibrationFile, validationFile): 
    ...
    calibration_template = CalibrationTemplate(authorized['distributions'], authorized['metrics_sync'])
    #calibration_template.loadTemplate('calibration_incomplete_XML.xml', os.path.dirname(__file__))
    calibration_template.loadTemplate('calibration_incomplete_XML.xml', os.path.dirname(inspect.getfile(CalibrationTemplate)))
    ...
    calXMLToWrite = calibration_template.createWorkflow(model, metric, exp, variables, optimization, work_dir, validationFile)
    errors = calibration_template.writeWorkflow(calXMLToWrite, calibrationFile, run=False)
    if errors == 0:
...
def writeXMLROM(model, rom, variables, romFile): 
    ...
    rom_template = ROMTemplate(authorized['roms'], authorized['samplers'], authorized['distributions'])
    rom_template.loadTemplate('trainROM_incomplete_XML.xml', os.path.dirname(inspect.getfile(ROMTemplate)))
    ...
    romXMLToWrite = rom_template.createWorkflow(model=model, variables=variables, rom=rom)
    errors = rom_template.writeWorkflow(romXMLToWrite, romFile, run=False)
