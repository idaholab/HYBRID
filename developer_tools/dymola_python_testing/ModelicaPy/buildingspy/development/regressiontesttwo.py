...
def runSimulation(worDir,comd):
    ...
    .. note:: This method is outside the class definition to
              ...
              For our purposes we redefined runSimulation to only take 
              one argument and moved cmd inside this function. We could not 
              adequately pass all the seperate temporary directories from 
              outside the function. This is also where the Dymola window is
              set to be open.
    '''

    import subprocess
    if len(comd)==1:
        cmd = [comd[0],os.path.join(worDir,"runAll.mos")] #,"/nowindow"]
    else:
...
class Tester:
    ...
    def setMosLocation(self,moslocation):
        ...
        be easilly added to the temp files.
        Needs to be absolute path to .mos file.
        """
        self._MosLocation = moslocation
    ...
    def setLibraryRoot(self, HomeLibrary, rootDir):
        ''' Set the root directory of the library.

        :param rootDir: The directory containing the Resources folder.
        :param HomeLibrary: The directory that contains the top most package.mo file
        ...
        Usage: Type
           ...
           >>> rt = r.Tester()
           >>> myMoLib = os.path.join("buildingspy", "tests", "MyModelicaLibrary")
           >>> rt.setLibraryRoot(HomeLibrary, rootDir)
        '''
        self._libHome = os.path.abspath(HomeLibrary)
    ...
    def isValidLibrary(library_home, root_lib):
        ...
        expectedRoot = os.path.join(root_lib, "Resources", "Scripts")
        print "Directory with .mo" , topPackage
        if root_lib == "":
            if not os.path.isfile(topPackage):
                raise ValueError("Directory %s is not a Modelica library.\n    Expected file '%s'."
                                 % (library_home, topPackage))
            return os.path.exists(os.path.join(library_home, "package.mo"))
        else:
            if not os.path.isfile(topPackage):
                raise ValueError("Directory %s is not a Modelica library.\n    Expected file '%s'."
                             % (library_home, topPackage))
            print "Directory with Resources is" , root_lib
            if not os.path.exists(os.path.join(root_lib, "Resources", "Scripts")):
                raise ValueError("Directory %s is not a Modelica library.\n    Expected directories '%s'."
                                 % (root_lib, expectedRoot))
            return os.path.exists(os.path.join(root_lib, "Resources", "Scripts"))
    ...
    def setSinglePackage(self, packageName):
        ...
        rooPat = os.path.join(self._rootPackage, 'Resources', 'Scripts', 'Dymola', pacPat)
...
'%s' does not exist.""" % (packageName, rooPat)
    ...
    def writeOpenModelicaResultDictionary(self):
        ...
        desDir=os.path.join(self._rootPackage, "Resources", "Scripts", "OpenModelica", "compareVars")
        ...
        for pai in data['ResultVariables']: # pairs of variables that are plotted together
            ...
            for var in pai:
                ...
                try:
                    ...
                    # RAK - If len(val) is less than self._nPoi, future comparisons will fail because it will end
                    #       up saving an array of the shorter length and always compare it against that of the 
                    #       self._nPoi (and the mismatched lengths will crash the test).  For now, the easiest
                    #       way to fix this is to always use self._nPoi.
                    #
                    # Original line that attempts to take the lesser of the length of the results array  
                    #   or self._nPoi, which causes problems later if _nPoi is not used:
  ...
  Stop processing.""" % (filNam, varNam, len(yOld), len(yInt)))
        ...
        for i in range(len(yInt)):
            ...
            if (abs(yOld[i]) > 10*tol):
                ...
                errRel[i] = 0
            # RAK - If errAbs[i] is included in the numeric error, then very small changes in a large value 
            #       will trigger a failure when it should not.  For example, a reference value of 15527033 vs.
            #       a calculated value of 15527034 has an absolute difference of 1 but a relative difference
            #       of 6.44E-08.  The sum of 1 and 6.44E-08 is greater than the tolerance of 0.001 and so it
            #       fails.
            #
            # Original line that computes the error as the sum of absolute and relative
...
#                f.write('statistics-%s=\n%s\n' % (stage, _pretty_print(y_tra[stage])))
                ...
                if varNam != 'time':
                    if y_ref.has_key(varNam):
                        ...
                        (res, timMaxErr, warning) = self.areResultsEqual(t_ref, y_ref[varNam], \
                                                                         ...
                                                                         varNam, matFilNam)
                        if warning:
                            # RAK - Elevate failed comparisons to Error
                            self._reporter.writeError(warning)
    ...
    def _compare_and_rewrite_fmu_dependencies(self, new_dependencies, reference_file_path, reference_file_name, ans):
        ...
        if not os.path.exists(abs_ref_fil_nam):
            # RAK - We want to know if the reference doesn't exist yet
            self._reporter.writeError( "Reference file %s does not yet exist." % reference_file_name);
    ...
    def _check_fmu_statistics(self, ans):
        ...
        retVal = 0
        #Check if the directory "self._rootPackage\\Resources\\ReferenceResults\\Dymola" exists, if not create it.
        refDir=os.path.join(self._rootPackage, 'Resources', 'ReferenceResults', 'Dymola')
            ...
            if self._includeFile(data['ScriptFile']) and data['mustSimulate']:
                ...
                try:
                    ...
                    if os.path.exists(oldRefFulFilNam):
                        ...
                        # RAK - We want to know if the reference doesn't exist yet
                        self._reporter.writeError("Reference file %s does not yet exist." % refFilNam);
...
"""
        ...
        for iPro in range(self._nPro):
            ...
            runFil.write("cd(\"%s\");\n" % os.path.join(self._temDir[iPro],self.getLibraryName()))
            if self._modelicaCmd == 'dymola':
...
""")
        ...
        for dat in self._data:
            ...
            # RAK - Cases were observed where the .mos file scan was finding files that didn't have
            #       a simulateModel command in it.  When that happens dat['ResultFile'] doesn't exist.
            if not dat['mustSimulate'] and not dat['mustExportFMU'] and 'ResultFile' in dat:
    ...
    def _setTemporaryDirectories(self):
        ...
        for iPro in range(self._nPro):
            ...
            tempDir = os.path.join(dirNam, self.getLibraryName())
            shutil.copytree(libDir,
                            tempDir,
                            symlinks=True,
                            ignore=shutil.ignore_patterns('.svn', '.mat', 'request.', 'status.'))
            #Copy resources folder into temp directory
            src = self._Resources
            dest = os.path.join(tempDir,"Resources")
            copy_tree(src,dest)
            self.AbsTempDir = tempDir.replace("\\","\\\\")
        return
    ...
    def run(self):
        ...
        if not self._useExistingResults:
            if not self.isExecutable(self._modelicaCmd):
                ...
                return 3
        print "Executable is on path"
        # Check current working directory
        if not self.isValidLibrary(self._libHome, self._rootPackage):
            print "*** %s is not a valid Modelica library." % self._libHome
            print "*** The current directory is ", os.getcwd()
            print "*** Expected directory ", os.path.abspath(os.path.join(self._rootPackage, "Resources", "Scripts"))
            print "*** Exit with error. Did not do anything."
            return 2
        print "Current working directory is good"
        ...
        if not self._useExistingResults:
            ...
            print "Temp Directories set... preparing to write runscripts..."
        ...
        if not self._useExistingResults:
            ...
            if self._nPro > 1:
                print "number of processors is greater than 1"
                os.chdir(self._libHome)                
                po = multiprocessing.Pool(self._nPro)
                print "multiprocessing pool set up.... executing..."
                
                """ This is the old function for running the simulations
                ...
                # RAK - We don't want to actually try to run anything when _temDir[0] hasn't been set
                #       or it crashes. 
                if self._temDir:
                    print "Running simulation"
                    runSimulation(os.path.join(self._temDir[0], libNam), comd=cmd)
                else:
                    print "temDir not set...skipping"
            ...
            for d in self._temDir:
                for temLogFilNam in glob.glob( os.path.join(d, self.getLibraryName(), '*.translation.log') ):
                    if os.path.exists(temLogFilNam):
                        ...
                        retVal = 1
            logFil.close()
            print "Preparing to concatenate simulator statistics files"
        ...
        print "Printing list of files that may be excluded..."
