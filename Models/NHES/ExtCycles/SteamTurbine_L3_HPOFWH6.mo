...
model SteamTurbine_L3_HPOFWH6
  ...
  Modelica.Blocks.Sources.RealExpression Actual_dp(y=LPT1_bypass_valve.dp)
    ...
    annotation (Placement(transformation(extent={{-196,-2},{-184,10}})));
  Modelica.Blocks.Sources.RealExpression setpoint_dp(y=0.5*100000)
    "Heat loss/gain not accounted for in connections (e.g., energy vented to atmosphere) [W]"
    annotation (Placement(transformation(extent={{-190,12},{-178,24}})));
  TRANSFORM.Controls.LimPID PA_LPT(
    controllerType=Modelica.Blocks.Types.SimpleController.PI,
    k=-3e-5,
    Ti=100,
    yMax=1,
    yMin=0) annotation (Placement(transformation(extent={{-162,28},{-142,48}})));
...
equation
  ...
  connect(LPT1_bypass.port_3, LPT1_bypass_valve.port_a)
    annotation (Line(points={{-14,64},{-14,0},{-46,0}}, color={0,127,255}));
  ...
  connect(HPT.portLP, LPT1_bypass.port_1)
    annotation (Line(points={{-26,60},{-24,60},{-24,74}},
                                                 color={0,127,255}));
  ...
  connect(Actual_dp.y, PA_LPT.u_m)
    annotation (Line(points={{-183.4,4},{-152,4},{-152,26}}, color={0,0,127}));
  connect(setpoint_dp.y, PA_LPT.u_s) annotation (Line(points={{-177.4,18},{-170,
          18},{-170,38},{-164,38}}, color={0,0,127}));
  connect(LPT1_bypass.port_2, LPT1_bypass_valve1.port_a)
    annotation (Line(points={{-4,74},{4,74}}, color={0,127,255}));
  connect(LPT1_bypass_valve1.port_b, LPT1.portHP)
    annotation (Line(points={{24,74},{34,74},{34,60}}, color={0,127,255}));
  connect(PA_LPT.y, LPT1_bypass_valve1.opening) annotation (Line(points={{-141,
          38},{-122,38},{-122,14},{-114,14},{-114,-14},{-68,-14},{-68,18},{-10,
          18},{-10,56},{14,56},{14,66}}, color={0,0,127}));
