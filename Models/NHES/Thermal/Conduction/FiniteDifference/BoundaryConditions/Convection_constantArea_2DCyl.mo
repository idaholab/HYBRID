within NHES.Thermal.Conduction.FiniteDifference.BoundaryConditions;
...
parameter Integer nNodes(min=3) "# of nodal points";
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer alphas[nNodes]
...
parameter Boolean isInner = false
  ...
  parameter Modelica.Units.SI.Length r_inner=1 "Inner radius"
    annotation (Dialog(enable=(if not isAxial then true else isInner)));
  parameter Modelica.Units.SI.Length r_outer=1 "Outer radius"
    annotation (Dialog(enable=(if not isAxial then true else not isInner)));
  parameter Modelica.Units.SI.Length length=1 "Axial length"
    annotation (Dialog(enable=isAxial));
