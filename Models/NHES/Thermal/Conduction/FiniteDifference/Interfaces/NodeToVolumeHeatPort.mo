...
model NodeToVolumeHeatPort "Converts nodal heat port to volume heat port"
  ...
  final parameter Integer nVolumes = nNodes-1;
  ...
  Modelica.Fluid.Interfaces.HeatPorts_a heatPorts_FD[nNodes]
    "Finite difference connector" annotation (Placement(transformation(
        ...
        rotation=-90,
        origin={-100,0})));
  Modelica.Fluid.Interfaces.HeatPorts_b heatPorts_DynP[nVolumes]
...
equation
  ...
  for i in 2:nVolumes-1 loop
    heatPorts_DynP[i].Q_flow + 0.5*(heatPorts_FD[i].Q_flow + heatPorts_FD[i+1].Q_flow) = 0;
  end for;
  heatPorts_DynP[nVolumes].Q_flow + 0.5*heatPorts_FD[nVolumes].Q_flow + heatPorts_FD[nVolumes+1].Q_flow = 0;

  for i in 1:nVolumes loop
