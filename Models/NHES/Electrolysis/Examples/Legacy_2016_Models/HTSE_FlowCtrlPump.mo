...
model HTSE_FlowCtrlPump
  import NHES.Electrolysis;
  extends Modelica.Icons.Example;

  Electrolysis.HTSE.HTSEplant_FlowCtrlPump hTSEplant
...
equation
  ...
  connect(We_SOEC.W, signalBus.Signals_IP.c_We_SOEC)
    annotation (Line(points={{56,7.52},{56,60},{0,60}}, color={255,204,51},
      thickness=0.5));
  connect(heatingMedium_out.ports[1], hTSEplant.port_b) annotation (Line(points={{-62,-15},
          {-62,-15},{-40,-15}},           color={0,127,255}));
  connect(We_SOEC.port_a, prescribedPowerFlow.port_b) annotation (Line(
      points={{64,0},{70,0},{70,10}},
      color={255,0,0},
      thickness=0.5));
  connect(hTSEplant.portElec_a, We_SOEC.port_b) annotation (Line(
      points={{40,0},{48,9.99201e-16}},
      color={255,0,0},
      thickness=0.5));
