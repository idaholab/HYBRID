...
model HTSEplant_notModulized
  ...
  */
  ...
  NHES.Electrolysis.Compressor.CompressionSystem_elecPorts
    ...
    comp_3rd(Wc(start=3602150))) annotation (Placement(
        transformation(extent={{-126,-104},{-62,-64}})));
  ...
  Modelica.Fluid.Sources.Boundary_pT sink_anodeStream(
    redeclare package Medium =
        Electrolysis.Media.Electrolysis.AnodeGas_air,
    p=system.p_ambient,
    nPorts=1) annotation (Placement(transformation(extent={{-70,-26},{-50,
            -6}}, rotation=0)));
  NHES.Electrolysis.Electrical.ElectricGenerator_constSpeed
    generator(w_fixed=turbine.N0) annotation (Placement(
        transformation(extent={{-8,-40},{-28,-20}})));
  NHES.Electrolysis.Electrical.SwitchYard_HTSE SY_HTSE annotation (
      Placement(transformation(extent={{120,-2},{140,18}})));
  ...
  NHES.Electrolysis.Sensors.PowerSensor W_vessel(W(unit="W",
        ...
        rotation=0,
        origin={112,0})));
  NHES.Electrolysis.Sensors.PowerSensor W_aux(W(unit="W",
        displayUnit="MW")) annotation (Placement(transformation(
        extent={{8,8},{-8,-8}},
        rotation=90,
        origin={-162,50})));
  NHES.Electrolysis.Sensors.PowerSensor W_GT(W(unit="W",
        displayUnit="MW")) annotation (Placement(transformation(
        extent={{-8,-8},{8,8}},
        rotation=90,
        origin={138,-30})));
...
equation
  ...
  */
  ...
  connect(steamSink_anodeStream.ports[1],
    hEX_nuclearHeatAnodeGasRecup_ROM.shell_out) annotation (Line(points={{-50,-44},
