...
model SteamSplit_test_sourcePressure
  ...
  Modelica.Blocks.Sources.Ramp valveOpening_cathodeGas(
    ...
    height=-0.1,
    startTime=100,
    offset=0.85)   annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={30,100})));
...
equation
  ...
  connect(steamNukeSink_cathodeGas.ports[1],
    ...
    annotation (Line(points={{0,20},{0,40}},        color={0,127,255}));
  ...
  connect(flowSplit.port_2, steamPressureIn.ports[1]) annotation (Line(
        points={{-80,10},{-80,10},{-104,10}}, color={0,127,255}));
  ...
  connect(steamNukeSink_anodeGas.ports[1],
    hEX_nuclearHeatAnodeGasRecup_ROM.shell_out)
    annotation (Line(points={{0,-30},{0,-52}}, color={0,127,255}));
  connect(TCV_cathodeGas.port_a, flowSplit.port_3) annotation (Line(
        points={{-48,80},{-90,80},{-100,80},{-100,28},{-70,28},{-70,20}},
        color={0,127,255}));
  connect(TCV_cathodeGas.opening, valveOpening_cathodeGas.y) annotation (
      Line(points={{-38,88},{-38,98},{-38,100},{19,100}}, color={0,0,127}));
  connect(TCV_cathodeGas.port_b, hEX_nuclearHeatCathodeGasRecup_ROM.shell_in)
    annotation (Line(points={{-28,80},{0,80},{0,56}}, color={0,127,255}));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,
            -120},{120,120}})), Icon(coordinateSystem(extent={{-120,-120},
            {120,120}})));
end SteamSplit_test_sourcePressure;
