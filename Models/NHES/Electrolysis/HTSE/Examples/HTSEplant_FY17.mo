...
model HTSEplant_FY17
  ...
  */
  ...
  Modelica.Blocks.Sources.Ramp power_set(
    ...
    height=-1.929*1e6*5*3 + 1613748*0 + 1658789.33*0 - 339049.79*1)
  ...
  Modelica.Blocks.Continuous.FirstOrder actuator_TNOut_cathodeGas(
    ...
    y_start=0.9)                                             annotation (
      Placement(transformation(
        ...
        rotation=0,
        origin={-60,144})));
  Modelica.Fluid.Sources.Boundary_pT steamSource_(
    use_p_in=false,
    redeclare package Medium = Medium,
    nPorts=1,
    p=5800000,
  ...
  NHES.Electrolysis.HeatExchangers.HEX_nuclearHeatAnodeGasRecupVessel_ROM_NHES
    ...
    redeclare package Medium_tube =
        Electrolysis.Media.Electrolysis.AnodeGas_air,
    initOpt=Electrolysis.Utilities.OptionsInit.steadyState) annotation (
     Placement(transformation(extent={{-30,-62},{-10,-82}})));
  ...
  Modelica.Fluid.Sensors.Pressure pH2O_out(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{-152,-22},{-168,-6}})));
  ...
  Modelica.Fluid.Fittings.TeeJunctionVolume flowSplit(
    ...
    use_T_start=true,
    redeclare package Medium = Medium,
    port_2(m_flow(start=7.311637), p(start=5800000)),
    energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState,
    massDynamics=Modelica.Fluid.Types.Dynamics.SteadyState,
    p_start=5800000,
  ...
  Modelica.Fluid.Sensors.Temperature TH2O_in(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{-180,92},{-196,108}})));
  Modelica.Fluid.Sensors.Pressure pH2O_in(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{-172,112},{-188,128}})));
equation
  ...
  */
  ...
  connect(SY_HTSE.generation_GT, W_GT.port_b) annotation (Line(
      points={{158,0},{158,-11},{158,-22}},
      color={255,0,0},
      thickness=0.5));
  connect(generator.powerGeneration, W_GT.port_a) annotation (Line(
      points={{-6,-30},{-10,-30},{-10,-54},{158,-54},{158,-38}},
      color={255,0,0},
      thickness=0.5));
  connect(prescribedPowerFlow.P_flow, power_set.y)
    annotation (Line(points={{201.6,0},{209,0}}, color={0,0,127}));
  connect(prescribedPowerFlow.port_b, W_HTSE.port_a) annotation (Line(
      points={{180,0},{180,0},{180,16}},
      color={255,0,0},
      thickness=0.5));
  ...
  connect(hEX_nuclearHeatAnodeGasRecup_ROM.shell_out, flowJoin.port_1)
    annotation (Line(points={{-20,-62},{-20,-40},{-40,-40}}, color={0,127,
          255}));
  connect(mH2O_out.port_b, steamSink.ports[1]) annotation (Line(points={{-160,
          -88},{-160,-98},{-200,-98}},        color={0,127,255}));
  connect(pH2O_out.port, mH2O_out.port_a) annotation (Line(points={{-160,
          -22},{-160,-38},{-160,-72}}, color={0,0,127}));
  ...
  connect(flowSplit.port_2, mH2O_in.port_b) annotation (Line(points={{
          -170,20},{-180,20},{-180,52}}, color={0,127,255}));
  connect(steamSource_.ports[1], mH2O_in.port_a) annotation (Line(points={{-200,80},
          {-180,80},{-180,68}},            color={0,127,255}));
  connect(pH2O_in.port, mH2O_in.port_a) annotation (Line(points={{-180,
          112},{-180,90},{-180,68}}, color={0,0,127}));
  connect(mH2O_in.port_a, TH2O_in.port) annotation (Line(points={{-180,68},
          {-180,68},{-180,92},{-188,92}}, color={0,0,127}));
