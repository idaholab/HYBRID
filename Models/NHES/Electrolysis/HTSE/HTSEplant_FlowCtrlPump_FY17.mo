...
model HTSEplant_FlowCtrlPump_FY17
  import NHES;
  ...
  NHES.Electrolysis.HTSE.Intermediate_HTSE.IdealPump
                                                returnPump(
    redeclare package Medium = Medium,
    PR0=62.7/51.3042,
    pstart_out=6270000,
    Tstart=497.15,
    wstart=7.3112)
    annotation (Placement(transformation(extent={{-92,-48},{-108,-32}})));
equation
  ...
  connect(compressionSystem.loadElec_comp, SY_aux.load_MSCS)
    annotation (Line(
      ...
      color={255,0,0},
      thickness=0.5));
  connect(flowJoin.port_2, returnPump.inlet)
...
end HTSEplant_FlowCtrlPump_FY17;
