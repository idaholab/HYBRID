...
model SimpleVolume_XChange
    ...
    extends NHES.Electrolysis.HTSE.BaseClasses.PartialVolume_Xichange(
      final V=geometry.V,
      ...
      mCb=port_a.m_flow*actualStream(port_a.C_outflow) + port_b.m_flow*
        ...
        mXib=port_a.m_flow*actualStream(port_a.Xi_outflow) + port_b.m_flow*actualStream(port_b.Xi_outflow)+dmX,
        mb = port_a.m_flow + port_b.m_flow + sum(dmX));
      //mb=port_a.m_flow + port_b.m_flow,                              //balance terms
    ...
    input TRANSFORM.Units.ExtraPropertyFlowRate mC_gen[Medium.nC]=fill(0, Medium.nC) "Internal trace mass generation"  annotation (Dialog(tab="Advanced", group="Trace Mass Transfer"));

    SI.MassFlowRate dmX[Medium.nXi] annotation (Dialog(tab="General"));
    ...
    annotation (Placement(transformation(extent={{30,-50},{50,-30}}), iconTransformation(extent={{30,-50},{50,-30}})));
