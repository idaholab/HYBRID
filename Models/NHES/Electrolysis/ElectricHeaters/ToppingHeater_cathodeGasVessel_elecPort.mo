...
model ToppingHeater_cathodeGasVessel_elecPort "Electrical topping heater"

  // ---------- Fluid packages -------------------------------------------------
  replaceable package Medium = Modelica.Media.Interfaces.PartialMedium
    "Medium model"  annotation (choicesAllMatching = true, Dialog(group="Working fluid (Medium)"));

  parameter Modelica.Units.SI.Current i_start=381.23 "Start value of current"
  ...
  parameter Modelica.Units.SI.Temperature T_in_start=626.584 + 273.15
  ...
  parameter Modelica.Units.SI.Temperature T_out_start=850 + 273.15
    ...
    annotation (Dialog(tab="Initialisation", group="Heated pipe"));
  parameter Modelica.Units.SI.MoleFraction yH2_start=0.1
    "Start value of H2 mole fraction in a pipe"
    annotation (Dialog(tab="Initialisation", group="Heated pipe"));
  parameter Modelica.Units.SI.MassFlowRate w_start=0.908083*5
    "Start value of mass flow rate [kg/s]"
    annotation (Dialog(tab="Initialisation", group="Heated pipe"));
  parameter Real Gc( unit="W/K")= 20469.9
    "Convective thermal conductance [W/K]";
  parameter Real TC_set(unit="K",displayUnit="degC") = 850 + 273.15
    "Set point temperature for cathode gas entering the SOEC";

  parameter Modelica.Units.SI.Temperature T_start=850 + 10 + 273.15
    "Start value of temperature for a resistor"
    annotation (Dialog(tab="Initialisation", group="Resistive heating coil"));
  parameter Modelica.Blocks.Types.Init initType=Modelica.Blocks.Types.Init.NoInit
    "Type of initialization for TC_in control (1: no init, 2: steady state, 3: initial state, 4: initial output)"
  ...
  HeatedPipe_cathodeGasVessel heatedPipe(
  ...
  Modelica.Thermal.HeatTransfer.Components.Convection convection
    annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-40,-10})));
  Modelica.Blocks.Sources.Constant TC_in_set(k=TC_set, y(displayUnit="degC",
        unit="K"))                                  annotation (Placement(
        transformation(
        extent={{10,10},{-10,-10}},
        rotation=270,
        origin={60,-10})));
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_b(  redeclare package Medium =
        ...
        transformation(extent={{90,-10},{110,10}}), iconTransformation(extent={{
            90,-10},{110,10}})));
  Electrolysis.Interfaces.ElectricalPowerPort_a loadElecHeater annotation (
      Placement(transformation(extent={{-10,-110},{10,-90}}),
        iconTransformation(extent={{-10,-110},{10,-90}})));
  Electrolysis.Sources.PrescribedLoad load_elecHeater annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=-90,
        origin={0,-70})));
  Modelica.Blocks.Sources.RealExpression load_elecHeaterSingal(y=
...
equation
  ...
  connect(load_elecHeater.load, loadElecHeater)
    annotation (Line(points={{0,-80},{0,-100}}, color={255,0,0}));
  connect(load_elecHeater.powerConsumption, load_elecHeaterSingal.y)
  ...
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
        ...
        Line(
          ...
          color={0,0,255},
          thickness=0.5)}));
end ToppingHeater_cathodeGasVessel_elecPort;
