...
model OxEon_v2 "Solid Oxide Electrolysis Cell (SOEC) stack model"
...
//   constant Integer numElecTransferred_per_H2prod = 2   "Number of electrons transferred per molecule of H2 produced";
  ...
  final parameter SI.Temperature T_stack_K= Modelica.Units.Conversions.from_degC(T_stack_C);
...
//   final parameter SI.MassFraction XAnodeIn[:]=Electrolysis.Utilities.moleToMassFractions(ystartAnodeIn, {mwN2*1000,mwO2*1000}) "Start value of inlet mass Fraction at anode {N2, O2}";
  ...
  // ---------- Connectors -----------------------------------------------------
  Modelica.Fluid.Interfaces.FluidPort_a cathodeFlangeIn(p(start = P_operating),
  ...
  Modelica.Fluid.Interfaces.FluidPort_b cathodeFlangeOut(p(start = P_operating),
  ...
  Modelica.Fluid.Interfaces.FluidPort_a anodeFlangeIn(p(start=P_operating),
  ...
  Modelica.Fluid.Interfaces.FluidPort_b anodeFlangeOut(p(start=P_operating),
...
equation
  ...
  molflow_H2_in = wstartCathodeIn*XstartCathodeIn[1]/mwH2; //These need to be converted to molar flow rates in derivative forms
  molflow_H2O_in = wstartCathodeIn*XstartCathodeIn[2]/mwH2O;
  molflow_O2_in = wstartAnodeIn*XstartAnodeIn[2]/mwO2;
  molflow_N2_in = wstartAnodeIn*XstartAnodeIn[1]/mwN2;

  SteamUtilization = molTransfer_H2/(molflow_H2O_in);

  molflow_H2O_out = molflow_H2O_in*(1-SteamUtilization); //These need to be converted to molar flow rates in derivative forms
  ...
  ycathodeIn[:] = Electrolysis.Utilities.massToMoleFractions(XcathodeIn, {mwH2*1000,mwH2O*1000}) "inlet mole fraction at cathode {H2, H2O}";
  yanodeIn[:] = Electrolysis.Utilities.massToMoleFractions(XanodeIn, {mwN2*1000,mwO2*1000}) "inlet mole fraction at anode {N2, O2}";
  XcathodeOut[:] = Electrolysis.Utilities.moleToMassFractions(ycathodeOut, {mwH2*1000,mwH2O*1000}) "outlet mass Fraction at cathode {H2, H2O}";
  XanodeOut[:] = Electrolysis.Utilities.moleToMassFractions(yanodeOut, {mwN2*1000,mwO2*1000}) "outlet mass Fraction at anode {N2, O2}";
