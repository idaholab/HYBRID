...
//----- Cell Operating Conditions -----
 ...
 //************Need to add to equation later**************
  ...
  SI.Power Q_transfer "Energy transfer from Cathode to Anode due to mass transfer";
  SI.Power Q_gen1,Q_gen2  "Energy produced";
...
equation
  ...
  Q_transfer = molTransfer_O2*mwO2*MediumAnode.specificEnthalpy_pTX(P_operating_anode,CathodeTemp.T,{0,1}); //energy gain from pure oxygen stream coming into the anode (should not be on the cathode)
  Q_gen1 = molTransfer_H2*mwH2*MediumCathode.specificEnthalpy_pTX(P_operating_cathode,CathodeTemp.T,{1,0}); //energy gain from production of hydrogen
  Q_gen2 =(molflow_H2O_out-molflow_H2O_in)*mwH2O*MediumCathode.specificEnthalpy_pTX(P_operating_cathode, CathodeTemp.T, {0,1}); //energy loss from splitting of water
