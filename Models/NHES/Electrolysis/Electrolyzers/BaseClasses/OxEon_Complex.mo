...
//----- Cell Operating Conditions -----
  parameter SI.Voltage TNV = 1.283  "Operating Thermo-Neutral Voltage [V]";
 ...
 //************Need to add to equation later**************
  ...
  SI.MassFraction XCathodeOut[nX], XAnodeOut[nX];
  ...
  parameter SI.MassFlowRate wstartCathodeOut = 0.0017861    "Start value of H2/H2O outlet mass flow rate at cathode [kg/s]";
  ...
  SI.MassFlowRate wCathodeOut, wAnodeOut "Outlet flow rates for Cathode (H2/H2O) and Anode (N2/O2) product streams";
  NHES.Electrolysis.Types.MassFlowRate_kg_hr wH2_Out "Outlet flow rate for H2 product stream";
  ...
  HTSE.SimpleVolume_XChange AnodeVolume(redeclare package Medium = MediumAnode, dmX={(molflow_N2_out - molflow_N2_in)*mwN2,(molflow_O2_out -
        molflow_O2_in)*mwO2})                                                                                                                                       annotation (Placement(transformation(extent={{-10,-58},
            {10,-38}})));
...
equation
  ...
  wCathodeOut = molflow_H2_out*mwH2 + molflow_H2O_out*mwH2O;
  wAnodeOut = molflow_N2_out*mwN2 + molflow_O2_out*mwO2;
  wH2_Out = molflow_H2_out*mwH2*3600;

  XCathodeOut[1] = molflow_H2_out*mwH2/wCathodeOut "Mass Fraction of H2 Out";
  XCathodeOut[2] = molflow_H2O_out*mwH2O/wCathodeOut "Mass Fraction of H2O Out";
  XAnodeOut[1] = molflow_N2_out*mwN2/wAnodeOut "Mass Fraction of N2 Out";
  XAnodeOut[2] = molflow_O2_out*mwO2/wAnodeOut "Mass Fraction of O2 Out";

  electricalLoad.f = f;

  connect(CathodeIn, CathodeVolume.port_a) annotation (Line(points={{-100,50},{-6,50}},                   color={0,127,255}));
  connect(CathodeVolume.port_b, CathodeOut) annotation (Line(points={{6,50},{100,50}},                  color={0,127,255}));
  connect(AnodeIn, AnodeVolume.port_a) annotation (Line(points={{-100,-48},{-6,-48}},                     color={0,127,255}));
