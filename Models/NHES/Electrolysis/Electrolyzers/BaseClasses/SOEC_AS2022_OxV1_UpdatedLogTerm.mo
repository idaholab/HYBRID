...
model SOEC_AS2022_OxV1_UpdatedLogTerm
  ...
  parameter SI.Length Wcell = 0.105 "Stack cell width [m]" annotation (Dialog(group="SOEC stack"));
  parameter SI.Length Lcell = 0.105 "Stack cell length [m]" annotation (Dialog(group="SOEC stack"));
  ...
  parameter SI.Pressure pstartCathodeIn=101325
  ...
  final parameter SI.Pressure pstartCathodeOut = pstartCathodeIn
    ...
    annotation (Dialog(tab="Initialisation"));
  parameter SI.Pressure pstartAnodeIn = 101325
  ...
  parameter Modelica.Units.NonSI.Temperature_degC TCstartIn_C=790
    "Start value of temperature in Celsius at cathode - in"
    annotation (Dialog(tab="Initialisation"));
  parameter Modelica.Units.NonSI.Temperature_degC TCstartOut_C=790
    "Start value of temperature in Celsius at cathode - out"
    annotation (Dialog(tab="Initialisation"));
  parameter Modelica.Units.NonSI.Temperature_degC TAstartIn_C=790
    "Start value of temperature in Celsius at anode - in"
    annotation (Dialog(tab="Initialisation"));
  parameter Modelica.Units.NonSI.Temperature_degC TAstartOut_C=790
  ...
  final parameter Modelica.Units.NonSI.Temperature_degC TSstartIn_C=(
    ...
    annotation (Dialog(tab="Initialisation"));
  parameter Modelica.Units.NonSI.Temperature_degC TSstartOut_C=790
  ...
  final parameter Modelica.Units.NonSI.Temperature_degC TIstartIn_C=(
    ...
    annotation (Dialog(tab="Initialisation"));
  parameter Modelica.Units.NonSI.Temperature_degC TIstartOut_C=790
  ...
  parameter SI.MoleFraction yH2startCathodeOut=0.82  "Start value of H2 inlet mole fraction of the cathode stream"
                                                                                                                  annotation (Dialog(tab="Initialisation"));
  parameter SI.MoleFraction yO2startAnodeOut=0.29678  "Start value of O2 outlet mole fraction of the anode gas"
                                                                                                               annotation (Dialog(tab="Initialisation"));
  ...
  final parameter SI.MassFraction XstartCathodeIn[:] = Electrolysis.Utilities.moleToMassFractions(ystartCathodeIn, {mwH2*1000,  mwH2O*1000}) "Start value of inlet mass Fraction at cathode {H2, H2O}";
  final parameter SI.MassFraction XstartCathodeOut[:] =  Electrolysis.Utilities.moleToMassFractions(ystartCathodeOut, {mwH2*1000,  mwH2O*1000}) "Start value of outlet mass Fraction at cathode {H2, H2O}";
  final parameter SI.MassFraction XstartCathodeAvg[:] = (XstartCathodeIn + XstartCathodeOut)/2  "Start value of average mass Fraction at cathode {H2, H2O}";
  final parameter SI.MassFraction XstartAnodeIn[:] = Electrolysis.Utilities.moleToMassFractions(ystartAnodeIn, {mwN2*1000,mwO2*  1000}) "Start value of inlet mass Fraction at anode {N2, O2}";
  final parameter SI.MassFraction XstartAnodeOut[:] =  Electrolysis.Utilities.moleToMassFractions(ystartAnodeOut, {mwN2*1000,  mwO2*1000}) "Start value of outlet mass Fraction at anode {N2, O2}";
  final parameter SI.MassFraction XstartAnodeAvg[:]= (XstartAnodeIn + XstartAnodeOut)/2 "Start value of average mass Fraction at anode {N2, O2}";

  parameter SI.MassFlowRate wstartCathodeIn = 0.002605  "Start value of H2/H2O inlet mass flow rate at cathode [kg/s]"
    annotation (Dialog(tab="Initialisation"));
  parameter SI.MassFlowRate wstartCathodeOut = 0.002605  "Start value of H2/H2O outlet mass flow rate at cathode [kg/s]"
    annotation (Dialog(tab="Initialisation"));
  parameter SI.MassFlowRate wstartAnodeIn = 0.005519  "Start value of N2/O2 inlet mass flow rate at anode [kg/s]"
    annotation (Dialog(tab="Initialisation"));
  ...
  */
  ...
  // The following variables are defined to calculate the cell potential based on reversible and irreversible losses. Equation 1 of J. Udagawa.
  // Note: The losses dur to concentration overpotential have been ignored.
  ...
  SI.Power W_elec(min=0, start=3e4)
    "Electrical power provided to the SOEC stacks per vessel [W]";
  SI.Power W_elec_total(min=0, start=3e4)
  ...
  SI.MoleFraction ycathodeIn[MediumCathode.nX](min={0,0},max={1,1}, start=ystartCathodeIn, nominal={0.1,0.9})  "Inlet mole fraction of cathode stream";
  SI.MoleFraction ycathodeOut[MediumCathode.nX](min={0,0},max={1,1}, start=ystartCathodeOut, nominal={0.82,0.18})  "Outlet mole fraction of cathode stream";
  SI.MoleFraction yanodeIn[MediumAnode.nX](min={0,0},max={1,1}, start=ystartAnodeIn, nominal={0.79,0.21})  "Inlet mole fraction of anode gas";
  SI.MoleFraction yanodeOut[MediumAnode.nX](min={0,0},max={1,1}, start=ystartAnodeOut, nominal={0.703,0.297})  "Outlet mole fraction of anode gas";

  SI.MoleFraction yH2_cathodeIn(min = 0, start=0.1)  "Inlet mole fraction of hydrogen - cathode []";
  SI.MoleFraction yH2_cathodeOut(min = 0, start=0.82)  "Outlet mole fraction of hydrogen - cathode []";
  SI.MoleFraction yH2O_cathodeIn(min = 0, start=0.9)  "Inlet mole fraction of steam - cathode []";
  SI.MoleFraction yH2O_cathodeOut(min = 0, start=0.18)  "Outlet mole fraction of steam - cathode []";
  SI.MoleFraction yO2_anodeIn(min = 0, start=0.21)  "Inlet mole fraction of oxygen - anode []";
  SI.MoleFraction yO2_anodeOut(min = 0, start=0.297)  "Outlet mole fraction of oxygen - anode []";
  SI.MoleFraction yN2_anodeIn(min = 0, start=0.79)  "Inlet mole fraction of nitrogen - anode []";
  SI.MoleFraction yN2_anodeOut(min = 0, start=0.703)  "Outlet mole fraction of nitrogen - anode []";

  Real yaIn(start=4.07, unit="1") "Lumped inlet mole fraction of oxygen [] ** yaIn = yO2*(p_in/p_std)**";
  ...
  SI.MolarFlowRate N_cathodeIn(min=0,start=0.1589)
    "H2/H2O inlet molar flow rate of cathode stream [mol/s]";
  SI.MolarFlowRate N_cathodeOut(min=0,start=0.1589)
    "H2/H2O outlet molar flow rate of cathode stream [mol/s]";
  SI.MolarFlowRate N_anodeIn(min=0,start=0.191)
    "N2/O2 inlet molar flow rate of anode gas [mol/s]";
  SI.MolarFlowRate N_anodeOut(min=0,start=0.191)
    "N2/O2 inlet molar flow rate of anode gas [mol/s]";
  ...
  SI.MolarFlowRate deltaN_O2
    "Number of moles of O2  produced(positive)/consumed(negative) during electrolysis [mol/s]";
  SI.MolarFlowRate deltaN_H2
    "Number of moles of H2  produced(positive)/consumed(negative) during electrolysis [mol/s]";
  SI.MolarFlowRate deltaN_H2O
    "Number of moles of H2O produced(positive)/consumed(negative) during electrolysis [mol/s]";
  ...
  Electrolysis.Types.SteamUtilization SUfactor(start=85)
    "Steam utilization factor, i.e. the fraction of total inlet steam consumed during electrolysis [%]";
  Electrolysis.Types.SteamUtilization SUfactor_est(start=85)
...
equation
  ...
  deltaGibbsE = Electrolysis.Utilities.GibbsEnergy_NASA_7Term(TSavg); //Gibbs Energy Calculated on NASA 7 Term Equation
  IntegraldeltaGibbsE = Electrolysis.Utilities.integralGibbsEnergy_NASA_7Term(TSout) - Electrolysis.Utilities.integralGibbsEnergy_NASA_7Term(TSin);
  coef_isothermal = 1/(2*Fa*delta_yH2*delta_ya); //term A in equation 38 Dominion report

  Integral_y_H2_1 = (-(1 - yH2_cathodeOut)*log(max(0.001,1 - yH2_cathodeOut)) - yH2_cathodeOut) - (-(1 - yH2_cathodeIn)*log(max(0.001,1 - yH2_cathodeIn)) - yH2_cathodeIn); //this is integral of ln(1-yH2)dyH2 with limits of cathodeOut and cathodeIn. The additional negative sign comes from equation 32 of Dominion report

  Integral_y_H2_2 = (yH2_cathodeOut*log(max(0.001, yH2_cathodeOut)) - yH2_cathodeOut) - (yH2_cathodeIn*log(max(0.001, yH2_cathodeIn)) - yH2_cathodeIn); //this is integral of ln(yH2)dyH2 with limits of cathodeOut and cathodeIn. Refer to equaitons 32 and 33 from Dominion report.

  Integral_ya = (yaOut*log(max(0.001,yaOut)) - yaOut) - (yaIn*log(max(0.001, yaIn)) - yaIn); //This is the O2 term from Dominion report equation 38 without the dyH2/2 term. It is included in the next equation

  Vn_isothermal = coef_isothermal*(deltaGibbsE*delta_yH2*delta_ya - Rg*TSavg*(delta_ya*(Integral_y_H2_1 - Integral_y_H2_2) - delta_yH2/2*Integral_ya)); // Equation 37 in Dominion Report

  Vn = if noEvent(TSout > TSin-TStol_Vn and TSout < TSin+TStol_Vn) then Vn_isothermal
  else coef_isothermal/(TSout - TSin)*(IntegraldeltaGibbsE*delta_yH2*delta_ya - Rg/2*(TSout^2 - TSin^2)*(delta_ya*(Integral_y_H2_1 - Integral_y_H2_2) - delta_yH2/2*Integral_ya));

  j0_cathode_cm2 = Electrolysis.Utilities.cathodeExCurrentDensity(TSavg); //equation 12 paper J. Udagawa
  j0_anode_cm2 = Electrolysis.Utilities.anodeExCurrentDensity(TSavg);  //equation 12 paper J. Udagawa

  j_stack_cm2 = j0_cathode_cm2*(exp(2*(1-0.5)*Fa*lossActOPcathode/(Rg*TSavg)) - exp(-2*0.5*Fa*lossActOPcathode/(Rg*TSavg)))
    "Solve for cathode activation overpotential.....//equation 11 paper J. Udagawa";

  j_stack_cm2 = j0_anode_cm2*(exp(2*(1-0.5)*Fa*lossActOPanode/(Rg*TSavg)) - exp(-2*0.5*Fa*lossActOPanode/(Rg*TSavg)))
    " Solve for anode activation overpotential.....//equation 11 paper J. Udagawa";

  conductivityElectrolyte = 33.4*1000*exp(-10.3*1000/TSavg);

  Rohm_cm2 = (thicknessCathode/conductivityC + thicknessAnode/conductivityA + thicknessElectrolyte/conductivityElectrolyte)*10000; //Ohmic losses - equation 4 paper J. Udagawa
  ASR = Rohm_cm2 + (lossActOPcathode + lossActOPanode)/j_stack_cm2;
  Vop_single = Vn + (ASR)*j_stack_cm2; //cell potential - equation 1 paper J. Udagawa
...
initial equation
  ...
  annotation (experiment(StopTime=10, Interval=1),
    ...
    <p>This is a cathode supported SOEC</p>        
 </html>"),
    ...
    Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},
            {100,100}})));
end SOEC_AS2022_OxV1_UpdatedLogTerm;
