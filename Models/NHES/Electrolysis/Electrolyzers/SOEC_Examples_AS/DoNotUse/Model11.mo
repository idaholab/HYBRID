...
model Model11
  ...
  Modelica.Fluid.Sources.Boundary_pT Sink_H2(
    ...
    T=313.15,
    nPorts=1) annotation (Placement(transformation(extent={{-390,138},{-370,158}})));
  Modelica.Fluid.Sources.Boundary_pT Sink_H2O(
    redeclare package Medium = Media.Electrolysis.CathodeGas,
    p(displayUnit="Pa") = 103299.8,
    T=313.15,
    nPorts=1) annotation (Placement(transformation(extent={{-396,60},{-376,80}})));
  DoNotUse.ComponentSplitter2 componentSplitter2_1 annotation (Placement(transformation(extent={{-202,58},{-286,154}})));
equation
  ...
  connect(componentSplitter2_1.vaporOutlet, Sink_H2.ports[1])
    annotation (Line(points={{-244,149.2},{-244,160},{-360,160},{-360,148},{-370,148}}, color={0,127,255}));
  connect(componentSplitter2_1.liquidOutlet, Sink_H2O.ports[1])
    annotation (Line(points={{-244,62.8},{-244,52},{-366,52},{-366,70},{-376,70}}, color={0,127,255}));
  connect(FuelHX.port_b1, componentSplitter2_1.feedInlet) annotation (Line(points={{-16,86},{-196,86},{-196,106},{-210.4,106}}, color={0,127,255}));
