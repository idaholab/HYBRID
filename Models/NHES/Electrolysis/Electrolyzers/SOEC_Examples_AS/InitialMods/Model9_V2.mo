...
model Model9_V2
  ...
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort FuelTempSensor(redeclare package Medium = Media.Electrolysis.CathodeGas)
    annotation (Placement(transformation(
        ...
        rotation=90,
        origin={118,32})));
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort CathodeOutTemp(redeclare package Medium = Media.Electrolysis.CathodeGas)                    annotation (Placement(transformation(extent={{174,20},
            {194,40}})));
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort AnodeOutTemp(redeclare package Medium = Media.Electrolysis.AnodeGas_air)                    annotation (Placement(transformation(extent={{196,-40},
            {216,-20}})));
  Modelica.Fluid.Sources.Boundary_pT cathodeSink1(
    ...
    T=313.15,
    nPorts=1)
    annotation (Placement(transformation(extent={{-318,76},{-298,96}})));
  ...
  TRANSFORM.Fluid.Sensors.TemperatureTwoPort AnodeOutTemp1(redeclare package Medium = Media.Electrolysis.AnodeGas_air)                   annotation (Placement(transformation(extent={{50,-64},
            {70,-44}})));
  ...
  Modelica.Fluid.Sensors.MassFractions X_H2O(redeclare package Medium = Media.Electrolysis.CathodeGas, substanceName="H2O")
    annotation (Placement(transformation(extent={{-170,104},{-190,124}})));
  ...
  Modelica.Fluid.Sensors.Temperature temperature(redeclare package Medium = Media.Electrolysis.CathodeGas)
    annotation (Placement(transformation(extent={{-282,86},{-262,106}})));
equation
  ...
  connect(X_H2O.Xi, mH2O_sep_out.u1) annotation (Line(points={{-191,114},{-212,114},{-212,124},{-209.6,124},{-209.6,128.2}},
                                                                                                              color={0,0,127}));
  connect(massFlowRate.m_flow, mH2O_sep_out.u2)
    annotation (Line(points={{-234,97},{-234,104},{-220.4,104},{-220.4,128.2}},                  color={0,0,127}));
  connect(mH2_sep_out.y, H2_feed.m_flow_in) annotation (Line(points={{-213,17.1},{-213,-2},{-230,-2}},                 color={0,0,127}));
  connect(H2O_feed.m_flow_in, mH2O_sep_out.y) annotation (Line(points={{-230,162},{-215,162},{-215,148.9}},        color={0,0,127}));
  connect(H2_feed.ports[1], H2Product.ports[1]) annotation (Line(points={{-238,-22},{-238,-36}}, color={0,127,255}));
  connect(massFlowRate.port_a, SinkTemp.port_b) annotation (Line(points={{-224,86},{-144,86}}, color={0,127,255}));
  connect(X_H2.port, SinkTemp.port_b) annotation (Line(points={{-160,58},{-160,86},{-144,86}}, color={0,127,255}));
  connect(X_H2O.port, SinkTemp.port_b) annotation (Line(points={{-180,104},{-180,86},{-144,86}}, color={0,127,255}));
  connect(massFlowRate.m_flow, mH2_sep_out.u2)
    annotation (Line(points={{-234,97},{-234,104},{-214,104},{-214,44},{-218.4,44},{-218.4,37.8}}, color={0,0,127}));
  connect(X_H2.Xi, mH2_sep_out.u1) annotation (Line(points={{-171,48},{-206,48},{-206,42},{-207.6,42},{-207.6,37.8}}, color={0,0,127}));
  connect(massFlowRate.port_b, temperature.port) annotation (Line(points={{-244,86},{-272,86}}, color={0,127,255}));
  connect(temperature.port, cathodeSink1.ports[1]) annotation (Line(points={{-272,86},{-298,86}}, color={0,127,255}));
  connect(H2O_feed.ports[1], CondensateSink.ports[1]) annotation (Line(points={{-238,182},{-238,196}}, color={0,127,255}));
  connect(H2O_feed.T_in, temperature.T) annotation (Line(points={{-234,160},{-234,106},{-256,106},{-256,96},{-265,96}}, color={0,0,127}));
  connect(H2_feed.T_in, temperature.T) annotation (Line(points={{-234,-8.88178e-16},{-234,70},{-252,70},{-252,96},{-265,96}}, color={0,0,127}));
