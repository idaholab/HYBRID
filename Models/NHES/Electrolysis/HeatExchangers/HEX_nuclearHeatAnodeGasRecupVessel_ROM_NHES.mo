...
model HEX_nuclearHeatAnodeGasRecupVessel_ROM_NHES
  "Reduced order model (ROM) for nuclear heat recuperation with anode gas"
  ...
  final parameter SI.Volume Vf = 0.8 "Fluid internal volume [m3]";
  ...
  parameter SI.ThermalConductance AhTube = 77843.3
    "Thermal conductance in tube side [W/K]";
  parameter SI.ThermalConductance AhShell = 77843.3
    "Thermal conductance in shell side [W/K]";

  parameter Electrolysis.Types.HydraulicConductanceSquared coeff_dpTube=180.82
    "Coefficient for the pressure drop in tube side [Pa.s2/(kg2)]";
  parameter Electrolysis.Types.HydraulicConductanceSquared coeff_dpShell=20740.5
    "Coefficient for the pressure drop in shell side [Pa.s2/(kg2)]";
  parameter SI.Pressure pTube_in_start = 23.7020628982604*1e5
    "Start value of inlet pressure in tube side [Pa]" annotation (Dialog(tab="Initialisation"));
  parameter SI.Pressure pShell_in_start = 51.4542*1e5
...
/*      
  ...
  parameter SI.MassFlowRate wTube_start = 23.2785
    ...
    annotation (Dialog(tab="Initialisation"));
  parameter SI.MassFlowRate wShell_start = 0.850426
  ...
  SI.Temperature TTube_in(min=273.15,nominal=188.778+273.15,start=TTube_in_start)
    "Inlet temperature in tube side [K]";
  SI.Temperature TTube_out(min=273.15,nominal=532.15,start=TTube_out_start)
    "Outlet temperature in tube side [K]";
  SI.Temperature TShell_in(min=273.15+259,nominal=310.79+273.15,start=TShell_in_start)
    "Inlet temperature in shell side [K]";
  SI.Temperature TShell_out(min=273.15,nominal=224+273.15,start=TShell_out_start)
    "Outlet temperature in shell side [K]";
  SI.Temperature TTube_mean(min=273.15,nominal=223.889+273.15,start = TTube_mean_start)
    "Average temperature in tube side [K]";
  SI.Temperature TShell_mean(min=273.15,nominal=267.395,start = TShell_mean_start)
...
end HEX_nuclearHeatAnodeGasRecupVessel_ROM_NHES;
