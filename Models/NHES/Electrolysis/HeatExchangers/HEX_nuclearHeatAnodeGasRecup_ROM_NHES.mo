...
model HEX_nuclearHeatAnodeGasRecup_ROM_NHES
  ...
  parameter SI.ThermalConductance AhTube = 92355
    "Thermal conductance in tube side [W/K]";
  parameter SI.ThermalConductance AhShell = 92355
  ...
  parameter SI.Pressure pTube_in_start = 2.14299*1e6
    "Start value of inlet pressure in tube side [Pa]" annotation (Dialog(tab="Initialisation"));
  parameter SI.Pressure pShell_in_start = 4.35*1e6
  ...
  parameter SI.Temperature TTube_in_start = 25 + 273.15
    "Start value of inlet temperature in tube side [K]" annotation (Dialog(tab="Initialisation"));
  parameter SI.Temperature TShell_in_start = 301.548 + 273.15
  ...
  parameter SI.Temperature TShell_out_start = 192.688 + 273.15
...
/*      
  ...
  parameter SI.MassFlowRate wTube_start = 23.2794
    ...
    annotation (Dialog(tab="Initialisation"));
  parameter SI.MassFlowRate wShell_start = 2.61448
  ...
  SI.Temperature TTube_in(min=273.15,nominal=298.4,start=TTube_in_start)
  ...
  SI.Temperature TShell_in(min=273.15+259,nominal=574.698,start=TShell_in_start)
    "Inlet temperature in shell side [K]";
  SI.Temperature TShell_out(min=273.15,nominal=465.838,start=TShell_out_start)
    "Outlet temperature in shell side [K]";
  SI.Temperature TTube_mean(min=273.15,nominal=415.15,start = TTube_mean_start)
    "Average temperature in tube side [K]";
  SI.Temperature TShell_mean(min=273.15,nominal=520.268,start = TShell_mean_start)
...
end HEX_nuclearHeatAnodeGasRecup_ROM_NHES;
