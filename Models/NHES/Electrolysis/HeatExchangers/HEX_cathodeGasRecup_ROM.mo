...
model HEX_cathodeGasRecup_ROM
  "Reduced order model (ROM) for heat recuperation via cathode gas"
  ...
  parameter SI.ThermalConductance AhTube = 75919
    "Thermal conductance in tube side [W/K]";
  parameter SI.ThermalConductance AhShell = 75919
    "Thermal conductance in shell side [W/K]";

  parameter Electrolysis.Types.HydraulicConductanceSquared coeff_dpTube=1988.8
    "Coefficient for the pressure drop in tube side [Pa.s2/(kg2)]";
  parameter Electrolysis.Types.HydraulicConductanceSquared coeff_dpShell=1988.8
...
/*
  final parameter SI.SpecificEnthalpy hTube_in_start = Medium_tube.specificEnthalpy(tubeState_in_start) 
    "Start value of inlet specific enthalpy in tube side [J/kg]";
  final parameter SI.SpecificEnthalpy hShell_in_start = Medium_shell.specificEnthalpy(shellState_in_start) 
    "Start value of inlet specific enthalpy in shell side [J/kg]";
  final parameter SI.SpecificEnthalpy hTube_out_start = Medium_tube.specificEnthalpy(tubeState_out_start) 
    "Start value of outlet specific enthalpy in tube side [J/kg]";
  final parameter SI.SpecificEnthalpy hShell_out_start = Medium_shell.specificEnthalpy(shellState_out_start) 
  ...
  parameter SI.MassFlowRate wTube_start = 4.54042
    "Start value of mass flow rate of the medium in tube side [kg/s]"
    annotation (Dialog(tab="Initialisation"));
  parameter SI.MassFlowRate wShell_start = 1.35415
  ...
  SI.Temperature TTube_in(min=273.15,nominal=556.55,start=TTube_in_start)
    "Inlet temperature in tube side [K]";
  SI.Temperature TTube_out(min=273.15,nominal=900,start=TTube_out_start)
    "Outlet temperature in tube side [K]";
  SI.Temperature TShell_in(min=273.15,nominal=1023.15,start=TShell_in_start)
    "Inlet temperature in shell side [K]";
  SI.Temperature TShell_out(min=273.15,nominal=618,start=TShell_out_start)
    "Outlet temperature in shell side [K]";
  SI.Temperature TTube_mean(min=273.15,nominal=728,start = TTube_mean_start)
    "Average temperature in tube side [K]";
  SI.Temperature TShell_mean(min=273.15,nominal=821,start = TShell_mean_start)
