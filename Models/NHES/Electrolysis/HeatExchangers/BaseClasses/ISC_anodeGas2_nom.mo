...
model ISC_anodeGas2_nom "Nominal condition for the inter-stage cooler2"
  ...
  // ---------- Define parameters ----------------------------------------------
  Electrolysis.Types.HydraulicConductanceSquared coeff_dpTube
    "Coefficient for the pressure drop in tube side [Pa.s2/(kg2)]";
  Electrolysis.Types.HydraulicConductanceSquared coeff_dpShell
  ...
  SI.Power QTube_lost "Energy rejected to the colder fluid [W]";
  SI.Power QShell_gained "Energy gained from the hotter fluid [W]";
  ...
  SI.Temperature TShell_in(min=273.15,nominal=574.698);
...
equation
  ...
  pTube_in = 8.18176525939608*1e5 "[Pa]";
  //pShell_in = 4.35*1e6 "[Pa]";
  pShell_out = 1.21325*1e5 "[Pa]";
  ...
  TTube_in = 273.15 + 181.587;
  TShell_in = 273.15 + 7;

  TTube_out = 273.15 + 40;
  TShell_out = 273.15 + 25;

  AhTube*(Tm - TTube_mean) = -QTube_lost;
  AhShell*(Tm - TShell_mean) = QShell_gained;
