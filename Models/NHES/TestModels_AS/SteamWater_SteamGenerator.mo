...
model SteamWater_SteamGenerator
  ...
  Modelica.Fluid.Sources.Boundary_ph tube_outlet(
    ...
    h=2.95363e6,
    nPorts=1)
    annotation (Placement(transformation(extent={{91,33},{81,43}})));
  Modelica.Fluid.Sources.MassFlowSource_h tube_inlet(
    m_flow=502.8,
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    h=962463,
    nPorts=1) annotation (Placement(transformation(extent={{83,-44},{71,-32}})));
  Modelica.Fluid.Sources.Boundary_pT shell_outlet(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    nPorts=1,
    p(displayUnit="MPa") = 15712238,
    T=556.15)
    annotation (Placement(transformation(extent={{-81,-43},{-71,-33}})));
  Modelica.Fluid.Sources.MassFlowSource_T shell_inlet(
    m_flow=4712,
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    nPorts=1,
    T=594.15) annotation (Placement(transformation(extent={{-73,34},{-61,46}})));
  Fluid.HeatExchangers.Generic_HXs.Generic_STHX STHX(
    nParallel=8,
    ...
    redeclare model HeatTransfer_tube = Fluid.Pipes.BaseClasses.HeatTransfer.Overall_Evaporation,
    hs_start_tube={1092619.183,1169910.614,1229588.908,1295560.906,1368587.743,1449483.535,1539113.913,1638416.293,1748414.32,1870228.947,
        2005086.711,2154323.65,2319377.564,2501734.143,2650870.103,2794340.368,2874110.975,2916877.195,2939456.778,2951404.247},
    redeclare package Tube_Material = Media.Solids.Inconel690,
    redeclare model HeatTransfer_shell = Fluid.Pipes.BaseClasses.HeatTransfer.Grimson_FlowAcrossTubeBundels (
        D=STHX.diameter_tube + 2*STHX.th_tube,
        S_T=1.25*(STHX.diameter_tube + 2*STHX.th_tube),
        S_L=1.25*(STHX.diameter_tube + 2*STHX.th_tube)),
    nTubes=750,
    dheight_shell=STHX.length_shell) annotation (Placement(transformation(extent={{21,-20},{-21,20}})));
equation
  connect(shell_inlet.ports[1], STHX.port_a_shell) annotation (Line(points={{-61,40},{-10,40},{-10,24},{-9.66,24},{-9.66,20}},
                                          color={0,127,255}));
  connect(shell_outlet.ports[1], STHX.port_b_shell) annotation (Line(points={{-71,-38},{-10,-38},{-10,-24},{-9.66,-24},{-9.66,-20}},
                                                       color={0,127,255}));
  connect(tube_inlet.ports[1], STHX.port_a_tube)
    annotation (Line(points={{71,-38},{0,-38},{0,-20}}, color={0,127,255}));
  connect(tube_outlet.ports[1], STHX.port_b_tube)
    annotation (Line(points={{81,38},{0,38},{0,20}}, color={0,127,255}));
  annotation (
