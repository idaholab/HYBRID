within NHES.GasTurbine.PowerPlant.Examples;
model GT_withoutAccelCtrl
  ...
  GasTurbine.PowerPlant.Blocks.LowValueSelector_twoInputs LVS(switch(u2(
          start=true))) "Low value selector"
  ...
  Modelica.Blocks.Math.Gain N_pu(k=1/GTunit.N0, y(unit="1")) annotation (
      Placement(transformation(
        ...
        rotation=-90,
        origin={48,34})));
  Sensors.PowerSensor elecLoadSensor
  ...
  Modelica.Fluid.Sensors.MassFlowRate mNatGas(redeclare package Medium =
        NHES.Media.NaturalGas)        annotation (Placement(
        transformation(
        extent={{8,8},{-8,-8}},
        rotation=90,
        origin={16,32})));
...
equation
  ...
  connect(frequencySensor.f, f_pu.u)
    annotation (Line(points={{96,30.16},{96,51.2}}, color={0,0,127}));
  connect(FB_f.u2, f_pu.y)
    annotation (Line(points={{96,97.6},{96,60.4}},           color={0,0,127}));
  ...
  connect(f_pu_sp.y, FB_f.u1)
    annotation (Line(points={{107.4,104},{106,104},{106,104},{104,104},{
          104,104},{102.4,104}},                       color={0,0,127}));
  ...
  connect(electricGenerator.powerGeneration, elecLoadSensor.port_a)
    annotation (Line(points={{88.4,0},{88.4,0},{98,0}}, color={0,127,255}));
  connect(elecLoadSensor.port_b, elecLoad.load) annotation (Line(points={{114,
