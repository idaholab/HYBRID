within NHES.Utilities.ErrorAnalysis;
model Errors_AbsRelRMS
  "Calculation of absolute, relative, and rms errors"

parameter Integer n "Length of variable vector";
parameter Real tolerance = 100 "eps = tolerance*MachineError";

input Real[n] x_1 "Values of first variable" annotation(Dialog(group="Input Variables:"),HideResult=true);
input Real[n] x_2 "Values of second variable" annotation(Dialog(group="Input Variables:"),HideResult=true);
...
protected
  ...
  SIadd.nonDim[n] Error_relRaw "Relative error not filtered by machine tolerance";

equation

  for i in 1:n loop
    Error_absRaw[i] = x_1[i] - x_2[i];
    Error_abs[i] = noEvent(if abs(Error_absRaw[i]) <= eps then 0 else Error_absRaw[i]);
    ...
    if abs(x_2[i]) <= eps then
      ...
      Error_rel[i] = noEvent(if abs(Error_relRaw[i]) <= eps then 0 else Error_relRaw[i]);
  end for;
