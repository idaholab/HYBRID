...
model Regions_3_CHF_EPRI_with_hotchannel
  ...
  TRANSFORM.Fluid.Pipes.GenericPipe_MultiTransferSurface coolantSubchannel(
    ...
    hs_start=hs_start,
    ps_start=ps_start,
    Ts_wall(start={{fuelModel.Ts_start_3[end, i] for j in 1:coolantSubchannel.heatTransfer.nSurfaces}
          for i in 1:coolantSubchannel.nV}),
    Xs_start=Xs_start,
    Cs_start=Cs_start,
    X_a_start=X_a_start,
    X_b_start=X_b_start,
  ...
  TRANSFORM.Nuclear.FuelModels.Regions_3_FD2DCyl fuelModel(
    ...
    r_3_outer=geometry.rs_outer[3]) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=-90,
        origin={0,10})));
...
//   Real DNBR[geometry.nV] = {(CHF[i]*3.169983306e-7/qflux[i]) for i in 1:geometry.nV};
  PowerProfiles.Hot_Channel Hot_Channel_Calculations(
    redeclare package Medium = Medium,
    ...
    nV=geometry.nV,
    Q_power=Q_total.y,
    SurfaceArea=geometry.surfaceArea[1],
    H_flow=coolantSubchannel.H_flows,
    n_hot=n_hot,
    nAssembly=nParallel,
    Q_shape=Q_shape,
    Fh=Fh,
    T_in=531.15)
    annotation (Placement(transformation(extent={{-98,50},{-76,72}})));
...
//
  ...
  Modelica.Blocks.Sources.RealExpression Hot_Channel_Effective(y=kinetics.Q_total*
        Fh)  "Total power (fission+decay heat)"
    annotation (Placement(transformation(extent={{78,-44},{62,-34}})));
  TRANSFORM.Nuclear.FuelModels.Regions_3_FD2DCyl Fuel_Centerline_Calcs_Hot_Channel(
    energyDynamics=energyDynamics_fuel,
    length=geometry.length,
    nParallel=geometry.nPins*nParallel,
    redeclare package Material_1 = Material_1,
    ...
    nZ=n_hot,
    T_start_1=T_start_1_Hotchannel,
    T_start_2=T_start_2_Hotchannel,
    T_start_3=T_start_3_Hotchannel,
    Ts_start_1=Ts_start_1_Hotchannel,
    Ts_start_2=Ts_start_2_Hotchannel,
    Ts_start_3=Ts_start_3_Hotchannel)
              annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=-90,
        origin={28,-60})));
  TRANSFORM.Blocks.ShapeFactor
                     shapeFactor1(n=n_hot, SF_start=Q_shape)
    annotation (Placement(transformation(extent={{52,-44},{42,-34}})));
  Thermal.Conduction.FiniteDifference.Interfaces.Temperature_multi
    Hot_Channel_Coolant_Temps(
    use_port=false,
    nPorts=n_hot,
    T=Hot_Channel_Calculations.T_co_sat)                    annotation (
      Placement(transformation(
        extent={{7,-7},{-7,7}},
        rotation=-90,
        origin={27,-91})));
...
equation
 //  sat.psat = port_a.p;
  ...
  connect(shapeFactor1.u,Hot_Channel_Effective. y)
    annotation (Line(points={{53,-39},{61.2,-39}}, color={0,0,127}));
  connect(shapeFactor1.y,Fuel_Centerline_Calcs_Hot_Channel. Power_in)
    annotation (Line(points={{41.5,-39},{28,-39},{28,-49}}, color={0,0,127}));
  connect(Hot_Channel_Coolant_Temps.port,Fuel_Centerline_Calcs_Hot_Channel. heatPorts_b)
    annotation (Line(points={{28,-84},{28,-70.2},{27.9,-70.2}},          color={
          191,0,0}));
  connect(coolantSubchannel.port_b, Mass_Sensor.port_a) annotation (Line(points=
         {{15,-14},{40,-14},{40,0},{52,0}}, color={0,127,255}));
...
Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{
        ...
        Line(
          ...
          smooth=Smooth.None,
          visible=DynamicSelect(true,showDesignFlowDirection))}));
end Regions_3_CHF_EPRI_with_hotchannel;
