...
function lithiumBromideWaterBaseProp_pTX
...
algorithm
  ...
  aux.T := T;
  aux.X := X;
  aux.region := if region == 0 then BaseLiBrW.Regions.region_pTX(
          aux.p,
          aux.T,
          aux.X) else region;
  ...
  elseif (aux.region == 6) then
    ...
    aux.x := aux_w.x;
    aux.dpT := aux_w.dpT;
    //     aux.h := IGW.Functions.h_Tlow(IGW.SingleGasesData.H2O, aux.T, h_off=h_os);
    //     aux.s := IGW.Functions.s0_Tlow(T=aux.T, data=IGW.SingleGasesData.H2O) + s_os;
    //     g.g := aux.h - aux.T*aux.s;
    //     aux.R := IGW.SingleGasesData.H2O.R;
    //     aux.rho := aux.p/(aux.R*aux.T);
    //     aux.vt := aux.R/max(aux.p, 1e-4);
    //     aux.vx := 0;
...
end lithiumBromideWaterBaseProp_pTX;
