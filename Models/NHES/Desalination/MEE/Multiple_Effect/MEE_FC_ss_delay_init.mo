...
model MEE_FC_ss_delay_init "Multi-Effect Evaporator"
...
parameter Modelica.Units.SI.MassFlowRate [data.nE] mHXnom=ones(data.nE);
equation
  ...
  h_in[2:data.nE]=steament.y;

  //hout and Qs
  h_outnom=Medium.bubbleEnthalpy(sat);
  for i in 2:data.nE loop
    if h_in[i]>h_innom[i] then
    h_out[i]/(2.75*(steamflow[i-1].y/mHXnom[i-1]) -1.75)=h_in[i]-(h_innom[i]-h_outnom[i])-sqrt(abs(h_in[i]-h_innom[i]));
    else
    h_out[i]= h_outnom[i]*(2.75*(steamflow[i-1].y/mHXnom[i-1]) -1.75);
    end if;
  end for;
  if h_in[1]>h_innom[1] then
  h_out[1]/(2.75*(port_a.m_flow/mHXnom[1]) -1.75)=h_in[1]-(h_innom[1]-h_outnom[1])-sqrt(abs(h_in[1]-h_innom[1]));
  else
  h_out[1]= h_outnom[1]*(2.75*(port_a.m_flow/mHXnom[1]) -1.75);
  end if;
  Q[1]=port_a.m_flow*(h_in[1]-h_out[1]);
  //Q[2:data.nE]=-evaporator_Brine_SHP_ss[1:data.nE-1].m_steam.*(h_in[2:data.nE]-h_out[2:data.nE]);
  Q[2:data.nE]=steamflow.y.*(h_in[2:data.nE]-h_out[2:data.nE]);
