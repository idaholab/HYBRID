...
model MEE_ss_test_delay_fixsmooth "Test of a single effect with constant UA"
  parameter Modelica.Units.SI.Area AX =1e4;

  Multiple_Effect.MEE_FC_ss_delay_fixsmooth mEE_FC_ss_delay_fixsmooth(
    redeclare replaceable Data.MEE_Data data(nE=12, T_b_in=333.15),
    Cs_in=0.03,
...
equation
  connect(boundary1.ports[1], mEE_FC_ss_delay_fixsmooth.port_b)
    annotation (Line(points={{-100,-28},{-38,-28}}, color={0,127,255}));
  connect(mEE_FC_ss_delay_fixsmooth.port_a, boundary.ports[1]) annotation (Line(
...
end MEE_ss_test_delay_fixsmooth;
