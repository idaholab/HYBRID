...
model boilingTestsmooth "Test of a single effect with constant UA"
  ...
  Components.SEE_Tube_Side_PoolBoiling_smooth
                                       sEE_Tube_Side_PoolBoiling_smooth(Ax=2e4)
...
equation
  connect(Tube_Inlet.ports[1], sEE_Tube_Side_PoolBoiling_smooth.Steam_Inlet_Port)
    annotation (Line(points={{-80,0},{-40,0}}, color={0,127,255}));
  connect(boundary.port, sEE_Tube_Side_PoolBoiling_smooth.Heat_Port)
    annotation (Line(points={{-40,70},{0,70},{0,20}}, color={191,0,0}));
  connect(sEE_Tube_Side_PoolBoiling_smooth.Liquid_Outlet_Port, cond_out.ports[1])
