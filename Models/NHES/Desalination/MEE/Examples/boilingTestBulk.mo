...
model boilingTestBulk "Test of a single effect with constant UA"
  ...
  Components.SEE_Tube_Side_PoolBoilingBulk
                                       sEE_Tube_Side_PoolBoilingBulk(Ax=PID.y)
  ...
  Modelica.Blocks.Sources.RealExpression
                                   realExpression2(y=
        sEE_Tube_Side_PoolBoilingBulk.hfT)
    annotation (Placement(transformation(extent={{-78,-60},{-58,-40}})));
  Modelica.Blocks.Sources.RealExpression
                                   realExpression3(y=
        sEE_Tube_Side_PoolBoilingBulk.h_out)
    annotation (Placement(transformation(extent={{-66,-98},{-46,-78}})));
equation
  connect(Tube_Inlet.ports[1], sEE_Tube_Side_PoolBoilingBulk.Steam_Inlet_Port)
    annotation (Line(points={{-80,0},{-40,0}}, color={0,127,255}));
  connect(boundary.port, sEE_Tube_Side_PoolBoilingBulk.Heat_Port)
    annotation (Line(points={{-40,70},{0,70},{0,20}}, color={191,0,0}));
  connect(sEE_Tube_Side_PoolBoilingBulk.Liquid_Outlet_Port, cond_out.ports[
  ...
  connect(realExpression1.y, Tube_Inlet.m_flow_in) annotation (Line(points=
          ...
          127}));
  connect(realExpression2.y, PID.u_s) annotation (Line(points={{-57,-50},{
          -57,-46},{-42,-46}}, color={0,0,127}));
  connect(PID.u_m, realExpression3.y) annotation (Line(points={{-30,-58},{
          -30,-88},{-45,-88}}, color={0,0,127}));
  annotation (
