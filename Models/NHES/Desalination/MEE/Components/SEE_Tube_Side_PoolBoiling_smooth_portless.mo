...
model SEE_Tube_Side_PoolBoiling_smooth_portless
...
parameter Modelica.Units.SI.Temperature TwSstart[:]=linspace(TwSst,TwSst,nV);
  ...
  Modelica.Units.SI.CoefficientOfHeatTransfer ho[nV];
    Modelica.Units.SI.CoefficientOfHeatTransfer hi_smooth[nV](start=fill(9700,nV));
  Modelica.Units.SI.CoefficientOfHeatTransfer ho_smooth[nV](start=fill(735,nV));
...
equation
  ...
  for i in 1:nV loop
    der(ho_smooth[i])=(ho[i]-ho_smooth[i])/tau;
    der(hi_smooth[i])=(hi[i]-hi_smooth[i])/tau;
...
end SEE_Tube_Side_PoolBoiling_smooth_portless;
