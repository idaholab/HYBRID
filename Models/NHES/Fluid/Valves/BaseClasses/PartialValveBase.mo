within NHES.Fluid.Valves.BaseClasses;
model PartialValveBase
  ...
  parameter SI.VolumeFlowRate Av=0 "Av (metric) flow coefficient"
          ...
          ThermalPower.SubComponents.Internal.Choices.FlowCoefficients.Av,
        group="Flow coefficient"));

  parameter Real Kv(unit="m3/h") = 0 "Kv (metric) flow coefficient" annotation (
     Dialog(enable=CvData == ThermalPower.SubComponents.Internal.Choices.FlowCoefficients.Kv,
        group="Flow coefficient"));
  parameter Real Cv=0 "Cv (US) flow coefficient in USG/min" annotation (Dialog(
        enable=CvData == ThermalPower.SubComponents.Internal.Choices.FlowCoefficients.Cv,
        group="Flow coefficient"));
