...
model IdealCondenser_ThreePort "Ideal condenser with fixed pressure"
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_c(redeclare package Medium =
        Medium) "Steam port" annotation (Placement(transformation(extent={{96,-20},
            {136,20}}, rotation=0), iconTransformation(extent={{90,-38},{110,
            -18}})));
initial equation
...
equation
  ...
  der(m_total) = port_a.m_flow + port_b.m_flow + port_c.m_flow;
  der(E) = port_a.m_flow*actualStream(port_a.h_outflow) + port_b.m_flow*actualStream(port_b.h_outflow) + port_c.m_flow*actualStream(port_c.h_outflow) + Q_total;
