within NHES.Fluid.Vessels.BaseClasses.BaseDrum.DrumTypes;
model Traditional "Traditional: Cylinder with spherical caps"

  extends NHES.Fluid.Vessels.BaseClasses.BaseDrum.DrumTypes.PartialDrumType;
...
algorithm
  ...
  level := smooth(1, noEvent(if V_liquid < V_1 then
    NHES.Math.cubicRoots_SingleReal(
                a=-pi/3,
                b=pi*r_1,
                c=0,
                d=-V_liquid,
                u_min=-0.1,
                u_max=r_1 + 0.1) elseif (V_liquid >= V_1 and
    V_liquid <= V_1 + V_2) then (V_liquid - V_1)/A_2c + r_1 else
    h_2 + r_1 + NHES.Math.cubicRoots_SingleReal(
                a=-pi/3,
                b=0,
                c=pi*r_3^2,
                d=V_1 + V_2 - V_liquid,
