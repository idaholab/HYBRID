...
model DetailedPipeFlow
  ...
  extends NHES.Fluid.Pipes.BaseClasses.FlowModels.PartialGenericPipeFlow;

  NHES.Fluid.ClosureModels.PressureLoss.Pipes.Detailed.dp_IN_con[nFM]
    IN_con(
    length=lengths,
    ...
    Re_turbulent=Res_turbulent) annotation (Placement(transformation(
          extent={{-80,20},{-60,40}})));

  NHES.Fluid.ClosureModels.PressureLoss.Pipes.Detailed.dp_IN_var[nFM]
    IN_var(
    rho_a=rhos_a,
    rho_b=rhos_b,
    mu_a=mus_a,
    mu_b=mus_b) annotation (Placement(transformation(extent={{-60,20},
            {-40,40}})));
...
protected
  ...
  NHES.Fluid.ClosureModels.PressureLoss.Pipes.Detailed.dp_IN_var IN_var_nominal(
    rho_a=rho_nominal,
    rho_b=rho_nominal,
    mu_a=mu_nominal,
    mu_b=mu_nominal);
  ...
  SI.AbsolutePressure dp_fric_nominal=sum(
      NHES.Fluid.ClosureModels.PressureLoss.Pipes.Detailed.dp_DP(
      IN_con,
      IN_var_nominal,
  ...
  if continuousFlowReversal then
    ...
    if from_dp and not dp_is_zero then
      m_flows = homotopy(actual=
        NHES.Fluid.ClosureModels.PressureLoss.Pipes.Detailed.dp_MFLOW(
                  IN_con,
                  IN_var,
                  dps_fg - {g*dheights[i]*rhos_act[i] for i in 1:nFM},
                  dp_small/(nFM))*nParallel, simplified=
      ...
      dps_fg = homotopy(actual=
        NHES.Fluid.ClosureModels.PressureLoss.Pipes.Detailed.dp_DP(
                  ...
                  IN_var,
                  m_flows/nParallel,
                  m_flow_small/nParallel) + {g*dheights[i]*rhos_act[i]
        for i in 1:nFM}, simplified=dp_nominal/m_flow_nominal*m_flows
    ...
    if from_dp and not dp_is_zero then
      m_flows = homotopy(actual=
        NHES.Fluid.ClosureModels.PressureLoss.Pipes.Detailed.dp_MFLOW_staticHead(
                  IN_con,
                  IN_var,
                  dps_fg,
                  dp_small/(nFM),
                  g*dheights)*nParallel, simplified=m_flow_nominal/
        dp_nominal*(dps_fg - g*dheights*rho_nominal));
    else
      dps_fg = homotopy(actual=
        NHES.Fluid.ClosureModels.PressureLoss.Pipes.Detailed.dp_DP_staticHead(
                  IN_con,
                  IN_var,
                  m_flows/nParallel,
                  m_flow_small/nParallel,
                  g*dheights), simplified=dp_nominal/m_flow_nominal*
        m_flows + g*dheights*rho_nominal);
    end if;
  end if;
