...
model NTU_HX_SinglePhase_V2
  ...
  Modelica.Units.SI.Power Q(start = Q_init)    "Power source/sink in each of the heat transfer volumes";
  Modelica.Units.SI.Power Q_max "Minimum of Q_max_s and Q_max_t";
  Modelica.Units.SI.Power Q_max_s    "Max Q of shell side assuming reaches inlet tube temperature";
  Modelica.Units.SI.Power Q_max_t    "Max Q of tube side assuming reaches inlet shell temperature";
  ...
  parameter Modelica.Units.SI.Volume V_Tube=0.1 "Total tube-side volume"    annotation (Dialog(tab="General", group="Sizing"));
  parameter Modelica.Units.SI.Volume V_Shell=0.1 "Total shell-side volume"    annotation (Dialog(tab="General", group="Sizing"));

  parameter Modelica.Units.SI.Length dh_Tube=0.0    "Total tube-side change in height"    annotation (Dialog(tab="General", group="Sizing"));
  parameter Modelica.Units.SI.Length dh_Shell=0.0    "Total shell-side change in height"    annotation (Dialog(tab="General", group="Sizing"));
  parameter Modelica.Units.SI.Pressure p_start_tube=101325 "Initial tube pressure"    annotation (Dialog(tab="Initialization", group="Tube"));
  parameter Modelica.Units.SI.SpecificEnthalpy h_start_tube_inlet=200e3 "Initial tube inlet enthalpy"    annotation (Dialog(tab="Initialization", group="Tube"));
  parameter Modelica.Units.SI.SpecificEnthalpy h_start_tube_outlet=500e3 "initial tube outlet enthalpy"    annotation (Dialog(tab="Initialization", group="Tube"));
  parameter Modelica.Units.SI.Pressure p_start_shell=1013250 "Initial shell pressure"    annotation (Dialog(tab="Initialization", group="Shell"));
  parameter Modelica.Units.SI.SpecificEnthalpy h_start_shell_inlet=1600e3  "Initial shell inlet enthalpy"    annotation (Dialog(tab="Initialization", group="Shell"));
  parameter Modelica.Units.SI.SpecificEnthalpy h_start_shell_outlet=600e3 "Initial shell outlet enthalpy"    annotation (Dialog(tab="Initialization", group="Shell"));
  parameter Modelica.Units.SI.Pressure dp_init_tube=500000 "Initial pressure drop tube side"    annotation (Dialog(tab="Initialization", group="Tube"));
  parameter Modelica.Units.SI.Pressure dp_init_shell=10000 "Initial pressure drop shell side"    annotation (Dialog(tab="Initialization", group="Shell"));
  parameter Modelica.Units.SI.Pressure dp_general=100 "Initialization pressure drop calculation"    annotation (Dialog(tab="Initialization", group="Both"));
  ...
  parameter Modelica.Units.SI.MassFlowRate m_start_tube=66.3 "Initial tube mass flow rate"    annotation (Dialog(tab="Initialization", group="Tube"));
  parameter Modelica.Units.SI.MassFlowRate m_start_shell=10 "Initial shell mass flow rate"    annotation (Dialog(tab="Initialization", group="Shell"));

  Modelica.Units.SI.Temperature Tin_t  "Inlet temperature based on mass flow direction at midpoint of NTUHX";
  Modelica.Units.SI.Temperature Tin_s  "Inlet shell temp based on mass flow direction at midpoint of NTUHX";
...
//Find the inlet temperatures and enthalpies given the flow direction
   if
     ...
     Tin_t = Tube_medium.temperature_phX(Tube_in.p, hin_t,Tube_in.Xi_outflow);
     hin_t = actualStream(Tube_in.h_outflow);
   else
     Tin_t =Tube_medium.temperature_phX(Tube_out.p,hin_t,Tube_in.Xi_outflow);
     hin_t =actualStream(Tube_out.h_outflow);
   end if;
   if
     (Shell_dp.m_flow > 0) then
     Tin_s = Shell_medium.temperature_phX(Shell_in.p,hin_s,Shell_in.Xi_outflow);
     hin_s = actualStream(Shell_in.h_outflow);
   else
     Tin_s =Shell_medium.temperature_phX(Shell_out.p,hin_s,Shell_in.Xi_outflow);
 ...
 // "For sign consistency, Q>0 means that the shell side is heating the tube side, and Q<0 reverses the flow of heat. The clearest indicator of heat flow direction is temperature."
   Q_max_s =abs(Shell_dp.m_flow)*(-Shell_medium.specificEnthalpy_pTX(Shell.medium.p,Tin_t,Shell_in.Xi_outflow) - hin_s);
   Q_max_t = abs(Tube_dp.m_flow)*(Tube_medium.specificEnthalpy_pTX(Tube_dp.port_a.p,Tin_s,Tube_in.Xi_outflow) - hin_t);
