...
model UTubeHX_v2
  ...
  TRANSFORM.Fluid.Interfaces.FluidPort_Flow port_b_tube(redeclare package Medium =
               NHES.Electrolysis.Media.Electrolysis.CathodeGas) annotation (Placement(transformation(extent={{90,-10},
  ...
  TRANSFORM.HeatAndMassTransfer.BoundaryConditions.Heat.CounterFlow counterFlow1(
                                                             n=geometry.nV, counterCurrent=counterCurrent)
  ...
  Geometry geometry(nV=1)
    annotation (Placement(transformation(extent={{-98,80},{-78,100}})));
  Electrolysis.Separator.Temp_flashDrumVessel_v2
                                              SpeciesSplitter(redeclare package Medium = Medium_tube) annotation (Placement(transformation(extent={{-82,-78},
            {-62,-58}})));
  Electrolysis.Separator.Combiner_Final combiner_Final
...
equation
  ...
  connect(tube_two.heatPorts, counterFlow1.port_a) annotation (Line(points={{32,-87},{32,-24}},
                                            color={191,0,0}));
  connect(tube_one.heatPorts, counterFlow.port_a)
    annotation (Line(points={{-40,-41},{-40,-26}}, color={191,0,0}));
  connect(counterFlow.port_b, shell.heatPorts[:, 1]) annotation (Line(points={{-40,-6},
          {-40,14},{0,14},{0,28},{-1.11022e-16,28},{-1.11022e-16,41}},
                                                             color={191,0,0}));
  connect(counterFlow1.port_b, shell.heatPorts[:, 2]) annotation (Line(points={{32,-4},{32,14},{0,14},{0,41}},
                                                       color={191,0,0}));
  connect(port_a_tube, SpeciesSplitter.feedInlet) annotation (Line(points={{-100,0},{-86,0},{-86,-68},{-80,-68}}, color={0,127,255}));
  connect(SpeciesSplitter.vaporOutlet, tube_one.port_a) annotation (Line(points={{-72,-59},{-72,-46},{-50,-46}}, color={0,127,255}));
  connect(SpeciesSplitter.liquidOutlet, tube_two.port_a)
    annotation (Line(points={{-72,-77},{-72,-92},{22,-92}},                   color={0,127,255}));
  connect(tube_one.port_b, combiner_Final.vaporInlet) annotation (Line(points={{-30,-46},{50,-46},{50,-56},{58,-56}}, color={0,127,255}));
  connect(tube_two.port_b, combiner_Final.liquidInlet) annotation (Line(points={{42,-92},{52,-92},{52,-68},{58,-68}}, color={0,127,255}));
  connect(combiner_Final.Outlet, port_b_tube) annotation (Line(points={{68,-71},{68,-76},{86,-76},{86,0},{100,0}}, color={0,127,255}));
  annotation (
