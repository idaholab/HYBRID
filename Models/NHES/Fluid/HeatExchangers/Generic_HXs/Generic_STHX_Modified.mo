...
model Generic_STHX_Modified
  ...
  extends NHES.Fluid.HeatExchangers.Generic_HXs.Generic_HX(final crossArea_shell = crossAreaNew_shell,
                                                           final perimeter_shell = perimeterNew_shell);

  parameter Boolean isCircular_shell=true "= true if cross sectional area is circular or a circular annulus" annotation(Dialog(tab="Shell Parameters",group="Geometry"));

  parameter SI.Diameter D_o_shell=if isCircular_tube then 0 else 4*crossAreaEmpty_shell/perimeterEmpty_shell  "Outer diameter of shell" annotation(Dialog(tab="Shell Parameters",group="Geometry",enable=isCircular_shell));
  parameter SI.Diameter D_i_shell=0  "Inner diameter of shell (if itself is an annulus)" annotation(Dialog(tab="Shell Parameters",group="Geometry",enable=isCircular_shell));
  parameter SI.Area crossAreaEmpty_shell = 0.25*pi*D_i_shell^2 "Cross-sectional area of an empty shell (i.e., no tubes)" annotation(Dialog(tab="Shell Parameters",group="Geometry",enable=not isCircular_shell));
  parameter SI.Length perimeterEmpty_shell = pi*D_i_shell "Wetted perimeter of an empty shell (i.e., no tubes)"  annotation(Dialog(tab="Shell Parameters",group="Geometry",enable=not isCircular_shell));
