...
model UTubeHX
  ...
  input SI.Height roughness_shell=2.5e-5
    ...
    annotation (Dialog(tab="Shell Side", group="Inputs"));
  input SI.Area surfaceArea_shell[nSurfaces_shell]={pi*D_o_tube*length_tube*nTubes, pi*D_o_tube*length_tube_two*nTubes} "Outer surface area"
    annotation (Dialog(tab="Shell Side", group="Inputs"));
  // Static head
  ...
  input SI.Length length_tube_two = length_tube annotation (Dialog(tab="Tube Side", group="Inputs Tube Two"));
  input SI.Angle angle_tube_two = -angle_tube annotation (Dialog(tab="Tube Side", group="Inputs Tube Two"));
  input SI.Length dheight_tube_two = length_tube_two*sin(angle_tube_two) annotation (Dialog(tab="Tube Side", group="Inputs Tube Two"));

  input SI.Length dlengths_tube_two[nV] = fill(length_tube_two/nV, nV);
  input SI.Angle angles_tube_two[nV] = fill(angle_tube_two, nV);
  input SI.Length dheights_tube_two[nV] = fill(dheight_tube_two/nV, nV);
