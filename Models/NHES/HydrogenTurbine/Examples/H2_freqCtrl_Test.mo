...
model H2_freqCtrl_Test
  extends Modelica.Icons.Example;

  PowerPlant.H2_freqCtrl GTPP "Natural-gas fired gas turbine power plant"
  ...
  Modelica.Blocks.Sources.Ramp loadSignal2(
    ...
    height=3.5*1e6*3.1)        annotation (Placement(transformation(
        ...
        rotation=0,
        origin={90,40})));
  GasTurbine.Interfaces.SignalBus
                       signalBus
    annotation (Placement(transformation(extent={{-20,50},{20,90}}),
        iconTransformation(extent={{-22,60},{18,100}})));
equation
