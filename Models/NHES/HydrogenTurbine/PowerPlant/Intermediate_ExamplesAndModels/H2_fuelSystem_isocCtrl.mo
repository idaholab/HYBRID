within NHES.HydrogenTurbine.PowerPlant.Intermediate_ExamplesAndModels;
model H2_fuelSystem_isocCtrl
  import GasTurbine =
         NHES.HydrogenTurbine;
  extends Modelica.Icons.Example;

  // Speed/load controller setting
  parameter Real w(unit="1") = 30;
  parameter Real x(unit="1") = 2.5;
  parameter Real y(unit="1") = 0.1;
  parameter Real KG(unit="1") = 30 "Gain for speed governor";
  parameter SI.Time TG = 1 "Speed governor time constant";
  ...
  Modelica.Blocks.Continuous.PI speedLoadCtrl(
    ...
    k=KG)
    annotation (Placement(transformation(extent={{0,80},{-16,96}})));
...
equation
  ...
  connect(feedback.u2, f_pu.y)
    annotation (Line(points={{76,81.6},{76,81.6},{76,48.4}},
                                                           color={0,0,127}));
  ...
  connect(prod.u1, f_pu.y) annotation (Line(points={{-35,54},{-40,54},{-40,70},{
          76,70},{76,48.4}}, color={0,0,127}));
  connect(feedback.y, speedLoadCtrl.u)
    annotation (Line(points={{68.8,88},{1.6,88}},          color={0,0,127}));
  connect(speedLoadCtrl.y, LVS.Fd) annotation (Line(points={{-16.8,88},{-92,88},
          {-92,53},{-87,53}}, color={0,0,127}));
  connect(feedback.u1, fsp_pu.y)
    annotation (Line(points={{82.4,88},{87.4,88},{87.4,88}}, color={0,0,127}));
