within NHES.HydrogenTurbine.PowerPlant.Intermediate_ExamplesAndModels.Examples;
model H2_inertiaConstN
  import GasTurbine =
  ...
  Modelica.Blocks.Sources.Constant constOmega(k=GTunit.N0)
...
equation
  ...
  connect(speed.flange, Inertia.flange_b)
    annotation (Line(points={{48,0},{48,0},{40,0}}, color={0,0,0}));
  connect(constOmega.y, speed.w_ref)
    annotation (Line(points={{79,0},{74,0},{70,0}}, color={0,0,127}));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
            -100},{100,100}})),
    experiment(StopTime=50, Interval=0.1),
    __Dymola_experimentSetupOutput);
end H2_inertiaConstN;
