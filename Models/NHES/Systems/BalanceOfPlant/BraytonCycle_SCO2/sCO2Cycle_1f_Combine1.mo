...
model sCO2Cycle_1f_Combine1
  package Medium = TRANSFORM.Media.ExternalMedia.CoolProp.CarbonDioxide(p_default=8e6);
  inner Modelica.Fluid.System system annotation (Placement(transformation(extent={{-128,118},{-108,138}})));
  ...
  TRANSFORM.Fluid.BoundaryConditions.MassFlowSource_h Tube_Feed1(
    ...
    m_flow=1783,
    h=361690,
    nPorts=1) annotation (Placement(transformation(extent={{-86,-34},{-72,-20}})));
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort
                                              sensor_TubeIn1(
    redeclare package Medium = ExternalMedia.Examples.CO2CoolProp,
    precision=2,
  ...
  GasTurbine.Compressor.Compressor      Comp2(
    ...
    eta0=0.9,
    PR0=2.39,
    w0=1783) annotation (Placement(transformation(extent={{-160,-48},{-134,-26}})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph Tube_Downstream2(
    redeclare package Medium = ExternalMedia.Examples.CO2CoolProp,
    p=20130000,
    h=361690,
    nPorts=1) annotation (Placement(transformation(extent={{-104,-34},{-118,-20}})));
  TRANSFORM.Fluid.BoundaryConditions.MassFlowSource_h Tube_Feed4(
    redeclare package Medium = ExternalMedia.Examples.CO2CoolProp,
    m_flow=1084,
    h=515350,
    nPorts=1) annotation (Placement(transformation(extent={{70,-40},{84,-26}})));
  GasTurbine.Compressor.Compressor      Comp1(
    ...
    eta0=0.9,
    PR0=2.39,
    w0=1783) annotation (Placement(transformation(extent={{96,-58},{122,-36}})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph Tube_Downstream3(
    redeclare package Medium = ExternalMedia.Examples.CO2CoolProp,
    p=20130000,
    h=576280,
    nPorts=1) annotation (Placement(transformation(extent={{156,-40},{142,-26}})));
  ...
  TRANSFORM.Fluid.BoundaryConditions.MassFlowSource_h Tube_Feed7(
    ...
    m_flow=1084,
    h=576280,
    nPorts=1) annotation (Placement(transformation(extent={{110,8},{96,22}})));
  TRANSFORM.Fluid.Volumes.SimpleVolume Cooler(
    ...
    annotation (Placement(transformation(
        ...
        origin={-186,-42})));
equation
  connect(sensor_TubeIn1.port_b,Tube_Feed1. ports[1]) annotation (Line(points={{-58,-27},{-72,-27}},
                                                                                                 color={0,127,255}));
  ...
  connect(Tube_Feed4.ports[1], Comp1.inlet)
    annotation (Line(points={{84,-33},{101.2,-33},{101.2,-38.2}},  color={0,127,255}));
  connect(Comp1.outlet, Tube_Downstream3.ports[1])
    annotation (Line(points={{116.8,-38.2},{116.8,-33},{142,-33}}, color={0,127,255}));
  connect(Split.port_1, resistance1.port_a) annotation (Line(points={{-108,-84},{-125,-84}},
                                                                                           color={0,127,255}));
  connect(Split.port_2, resistance.port_a) annotation (Line(points={{-94,-84},{-77,-84}}, color={0,127,255}));
  connect(resistance.port_b, Tube_Downstream4.ports[1])
    annotation (Line(points={{-63,-84},{-63,-83},{-44,-83}}, color={0,127,255}));
  connect(Tube_Feed7.ports[1], Merge.port_2) annotation (Line(points={{96,15},{68,15}},   color={0,127,255}));
  ...
  connect(Cooler.port_b, Comp2.inlet)
    annotation (Line(points={{-180,-42},{-168,-42},{-168,-28.2},{-154.8,-28.2}}, color={0,127,255}));
  connect(Comp2.outlet, Tube_Downstream2.ports[1])
    annotation (Line(points={{-139.2,-28.2},{-139.2,-34},{-118,-34},{-118,-27}}, color={0,127,255}));
end sCO2Cycle_1f_Combine1;
