...
model New_v7
  ...
  GasTurbine.Compressor.Compressor_v2   Comp2(
    ...
    w0=operatingConditions.Comp2_NominalMassFlowRate)
             annotation (Placement(transformation(extent={{-138,-17},{-112,5}})));
  GasTurbine.Compressor.Compressor_v2   Comp1(
    ...
    w0=operatingConditions.Comp1_NominalMassFlowRate)
             annotation (Placement(transformation(extent={{148,-71},{174,-49}})));
  TRANSFORM.Fluid.BoundaryConditions.MassFlowSource_T Comp2In(
    redeclare package Medium = Medium,
    m_flow=1084,
    T=372.25,
    nPorts=1) annotation (Placement(transformation(extent={{122,-58},{136,-44}})));
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance resistance3(redeclare package Medium = ExternalMedia.Examples.CO2CoolProp, R=1e6)
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        rotation=90,
        origin={125,-106})));
  Modelica.Fluid.Fittings.TeeJunctionIdeal  Split2(
    redeclare package Medium = ExternalMedia.Examples.CO2CoolProp)
    annotation (Placement(transformation(extent={{-7,-7},{7,7}},
        rotation=180,
        origin={125,-86})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_pT Pressurizer3(
    redeclare package Medium = ExternalMedia.Examples.CO2CoolProp,
    p=8411000,
    T=372.25,
    nPorts=1) annotation (Placement(transformation(extent={{-7,-7},{7,7}},
...
equation
  ...
  connect(Comp1.inlet, Comp2In.ports[1])
    annotation (Line(points={{153.2,-51.2},{144,-51.2},{144,-51},{136,-51}}, color={0,127,255}));
  ...
  connect(Comp1.outlet, Merge.port_2) annotation (Line(points={{168.8,-51.2},{202,-51.2},{202,-18},{162,-18}},   color={0,127,255}));
  ...
  connect(resistance3.port_b,Pressurizer1. ports[1]) annotation (Line(points={{125,-113},{124,-113},{124,-123}}, color={0,127,255}));
  connect(resistance3.port_a,Split2. port_3)
    annotation (Line(points={{125,-99},{125,-93}},                           color={0,127,255}));
  connect(Split2.port_2,FlowResistance2. port_b) annotation (Line(points={{118,-86},{103,-86}},
                                                                                              color={0,127,255}));
  connect(Pressurizer3.ports[1], Split2.port_1) annotation (Line(points={{205,-125},{186,-125},{186,-86},{132,-86}},   color={0,127,255}));
