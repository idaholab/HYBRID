...
model Brayton_Cycle_sCO2_TurboGenerator
  ...
  TRANSFORM.Fluid.Interfaces.FluidPort_State     BoundaryOut(
    redeclare package Medium = Medium)
              annotation (Placement(transformation(extent={{-130,-28},{-110,-8}}),
                                                                                 iconTransformation(extent={{-130,-28},{-110,-8}})));
  Fluid.HeatExchangers.Generic_HXs.NTU_HX_SinglePhase_New
                     HX2(
  ...
  GasTurbine.Turbine.Turbine_v3 Turbine(
    redeclare package Medium = Medium,
  ...
  TRANSFORM.Fluid.Interfaces.FluidPort_Flow      BoundaryIn(
    redeclare package Medium = Medium)
              annotation (Placement(transformation(extent={{-130,22},{-110,43}}), iconTransformation(extent={{-130,21},{-110,43}})));
  ...
  Modelica.Fluid.Fittings.TeeJunctionIdeal Merge2(redeclare package Medium = Medium)
    annotation (Placement(transformation(extent={{-105,45},{-91,59}})));
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_pT Sink1(
    ...
    nPorts=1) annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-96,106})));
  Modelica.Blocks.Sources.Constant const1(k=1)    annotation (Placement(transformation(extent={{-142,66},{-122,86}})));
...
Q_gen = Turbine.Wt - Comp1.Wc - Comp2.Wc; //Net power(electricity) of the system = TurbinePower - Comp1Power - Comp2Power
  ...
  connect(HX1_sensor_ShellOut.port_b, BoundaryOut) annotation (Line(points={{-32,-18},{-120,-18}},
                                                                                                 color={0,127,255}));
  connect(FlowResistance2.port_b, Comp1.inlet) annotation (Line(points={{5,-124},{37.2,-124},{37.2,-124.2}}, color={0,127,255}));
  connect(valveLinear1.port_a, Merge2.port_3) annotation (Line(points={{-98,66},{-98,59}}, color={0,127,255}));
  connect(valveLinear1.port_b, Sink1.ports[1]) annotation (Line(points={{-98,86},{-98,99},{-96,99}}, color={0,127,255}));
  connect(Merge2.port_1, BoundaryIn) annotation (Line(points={{-105,52},{-120,52},{-120,32.5}}, color={0,127,255}));
  connect(Merge2.port_2, Turbine.inlet) annotation (Line(points={{-91,52},{-84.8,52},{-84.8,25.2}}, color={0,127,255}));
  connect(const1.y, valveLinear1.opening) annotation (Line(points={{-121,76},{-106,76}}, color={0,0,127}));
  connect(Turbine.shaft_b, generator.shaft)
    annotation (Line(points={{-62.84,14},{-34,14},{-34,-16},{-62,-16},{-62,-4.1},{-58.1,-4.1}}, color={0,0,0}));
...
end Brayton_Cycle_sCO2_TurboGenerator;
