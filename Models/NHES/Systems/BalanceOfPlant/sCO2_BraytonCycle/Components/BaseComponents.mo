...
package BaseComponents
  partial model TurbineBase_v2 "Gas Turbine"
    extends GasTurbine.Icons.Turbine;
    ...
    parameter Modelica.Units.SI.Pressure pstart_in "Inlet start pressure"
      annotation (Dialog(tab="Initialisation"));
    parameter Modelica.Units.SI.Pressure pstart_out=1.01325*1e5
      "Outlet start pressure" annotation (Dialog(tab="Initialisation"));
    parameter Modelica.Units.SI.Temperature Tstart_in "Inlet start temperature"
      annotation (Dialog(tab="Initialisation"));
    parameter Modelica.Units.SI.Temperature Tstart_out "Outlet start temperature"
      annotation (Dialog(tab="Initialisation"));
    parameter Modelica.Units.SI.MassFraction Xstart[Medium.nX]=Medium.reference_X
    ...
    Medium.AbsolutePressure pout(start=pstart_out) "Turbine outlet pressure";
    Modelica.Units.SI.Temperature Tf "Turbine firing temperature";
    Modelica.Units.SI.Temperature Te "Turbine exhaust gas temperature";

    Modelica.Units.SI.MassFlowRate w "Gas flow rate";
    Modelica.Units.SI.Power Wt(min=0, displayUnit="MW")
...
<p>This is a slightly modified version of the turbine base model. The dummy pressure assignment has been changed to 10e5 from 1e5</p>    
...
<ul><li><tt>explicitIsentropicEnthalpy = true</tt>: the isentropic enthalpy <tt>hout_iso</tt> is calculated by the <tt>Medium.isentropicEnthalpy</tt> function. <li><tt>explicitIsentropicEnthalpy = false</tt>: the isentropic enthalpy is determined by equating the specific entropy of the inlet gas <tt>gas_in</tt> and of a fictious gas state <tt>gas_iso</tt>, with the same pressure of the outgoing gas; both are computed with the function <tt>Medium.specificEntropy</tt>.</pp></ul>
  ...
  partial model CompressorBase_v2 "Gas compressor"
    extends GasTurbine.Icons.Compressor;
    ...
    parameter Modelica.Units.SI.Pressure pstart_in "Inlet start pressure"
      annotation (Dialog(tab="Initialisation"));
    parameter Modelica.Units.SI.Pressure pstart_out "Outlet start pressure"
      annotation (Dialog(tab="Initialisation"));
    parameter Modelica.Units.SI.Temperature Tstart_in=15 + 273.15
    ...
    Medium.SpecificEntropy s_in "Inlet specific entropy";
    Medium.AbsolutePressure pin( start=pstart_in) "Compressor inlet pressure";
    Medium.AbsolutePressure pout(start=pstart_out) "Compressor outlet pressure";
    Modelica.Units.SI.Temperature Ti "Compressor inlet temperature";
    Modelica.Units.SI.Temperature Td "Compressor outlet temperature";

    Modelica.Units.SI.MassFlowRate w "Gas flow rate";
    Modelica.Units.SI.Power Wc(min=0, displayUnit="MW")
  ...
  equation
    ...
    annotation (
      Documentation(info="<html>
<p>This is a slightly modified version of the compressor base model. The dummy pressure assignment has been changed to 10e5 from 1e5</p>        
