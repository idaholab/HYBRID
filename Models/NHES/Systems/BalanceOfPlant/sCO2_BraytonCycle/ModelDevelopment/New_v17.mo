...
model New_v17
  ...
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort HX1_sensor_TubeOut(
    ...
    precision=2,
    precision2=1) annotation (Placement(transformation(extent={{44,36},{66,56}})));
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort HX1_sensor_TubeIn(
    redeclare package Medium = Medium,
    precision=2,
    precision2=1) annotation (Placement(transformation(extent={{-30,37},{-8,58}})));
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort HX1_sensor_ShellIn(
    redeclare package Medium = Medium,
    precision=2,
    precision2=1) annotation (Placement(transformation(extent={{64,6},{42,26}})));
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort HX1_sensor_ShellOut(
    redeclare package Medium = Medium,
    precision=2,
    precision2=1) annotation (Placement(transformation(extent={{-10,8},{-32,28}})));
  ...
  TRANSFORM.Fluid.Interfaces.FluidPort_State     BoundaryOut(
    redeclare package Medium = Medium)
              annotation (Placement(transformation(extent={{-160,8},{-140,28}}), iconTransformation(extent={{-160,8},{-140,28}})));
  Fluid.HeatExchangers.Generic_HXs.NTU_HX_SinglePhase_New
                     HX2(
  ...
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort HX2_sensor_TubeOut(
    redeclare package Medium = Medium,
    precision=2,
    precision2=1) annotation (Placement(transformation(extent={{44,-22},{64,-1}})));
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort HX2_sensor_TubeIn(
    redeclare package Medium = Medium,
    precision=2,
    precision2=1) annotation (Placement(transformation(extent={{-32,-24},{-10,-1}})));
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort HX2_sensor_ShellIn(
    redeclare package Medium = Medium,
    precision=2,
    precision2=1) annotation (Placement(transformation(extent={{64,-47},{40,-26}})));
  TRANSFORM.Fluid.Sensors.PressureTemperatureTwoPort HX2_sensor_ShellOut(
    redeclare package Medium = Medium,
    precision=2,
    precision2=1) annotation (Placement(transformation(extent={{-4,-46},{-28,-26}})));
  ...
  TRANSFORM.Fluid.Interfaces.FluidPort_Flow      BoundaryIn(
    redeclare package Medium = Medium)
              annotation (Placement(transformation(extent={{-160,61},{-140,82}}), iconTransformation(extent={{-160,60},{-140,82}})));
  ...
  Modelica.Fluid.Fittings.TeeJunctionIdeal Merge1(redeclare package Medium = Medium)
    annotation (Placement(transformation(extent={{-49,57},{-35,71}})));
...
equation
  ...
  connect(HX1_sensor_ShellIn.port_b, HX1.Shell_in)
    annotation (Line(points={{42,16},{38,16},{38,25.6},{26,25.6}},     color={0,127,255}));
  ...
  connect(HX2.Tube_out, HX2_sensor_TubeOut.port_a)
    annotation (Line(points={{28,-17.2},{36,-17.2},{36,-11.5},{44,-11.5}},
                                                                       color={0,127,255}));
  connect(HX2_sensor_ShellIn.port_b, HX2.Shell_in)
    annotation (Line(points={{40,-36.5},{32,-36.5},{32,-24.4},{28,-24.4}},
                                                                     color={0,127,255}));
  connect(HX2.Shell_out, HX2_sensor_ShellOut.port_a)
    annotation (Line(points={{2,-24.4},{-2,-24.4},{-2,-36},{-4,-36}},
                                                                 color={0,127,255}));
  ...
  connect(Split.port_2,FlowResistance2. port_a) annotation (Line(points={{-38,-88},{-9,-88}},
                                                                                            color={0,127,255}));
  connect(PID.y, valveLinear.opening) annotation (Line(points={{59,88},{-34,88}},  color={0,0,127}));
  ...
  connect(Sink.ports[1], valveLinear.port_b) annotation (Line(points={{-42,113},{-42,98}},            color={0,127,255}));
  ...
  connect(HX1_sensor_ShellOut.port_b, BoundaryOut.ports[1]) annotation (Line(points={{-32,18},{-150,18}},color={0,127,255}));
  connect(HX1_sensor_TubeIn.port_b, HX1.Tube_in) annotation (Line(points={{-8,47.5},{0,47.5},{0,32.8}},
                                                                                                     color={0,127,255}));
  connect(HX1_sensor_TubeOut.port_b, HX2_sensor_ShellIn.port_a) annotation (Line(points={{66,46},{68,46},{68,-36.5},{64,-36.5}},
                                                                                                                             color={0,127,255}));
  connect(HX2_sensor_TubeIn.port_b, HX2.Tube_in) annotation (Line(points={{-10,-12.5},{-4,-12.5},{-4,-17.2},{2,-17.2}},
                                                                                                                    color={0,127,255}));
  connect(Comp2.outlet, HX2_sensor_TubeIn.port_a) annotation (Line(points={{-79.2,-13.2},{-78,-13.2},{-78,-12.5},{-32,-12.5}},
                                                                                                                           color={0,127,255}));
  connect(HX2_sensor_TubeOut.port_b, Merge.port_1) annotation (Line(points={{64,-11.5},{80,-11.5},{80,-64},{86,-64}},
                                                                                                                  color={0,127,255}));
  ...
  connect(valveLinear.port_a, Merge1.port_3) annotation (Line(points={{-42,78},{-42,71}}, color={0,127,255}));
  connect(Turbine.outlet, Merge1.port_1) annotation (Line(points={{-63.2,61.2},{-62,61.2},{-62,64},{-49,64}}, color={0,127,255}));
  connect(Merge1.port_2, HX1_sensor_TubeIn.port_a) annotation (Line(points={{-35,64},{-30,64},{-30,47.5}}, color={0,127,255}));
  connect(BoundaryIn, Turbine.inlet) annotation (Line(points={{-150,71.5},{-84.8,71.5},{-84.8,61.2}}, color={0,127,255}));
  annotation (
