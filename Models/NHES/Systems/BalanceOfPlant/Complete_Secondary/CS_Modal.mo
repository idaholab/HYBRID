...
model CS_Modal
  ...
  Modelica.Blocks.Math.Min min1 annotation (Placement(transformation(
        ...
        rotation=180,
        origin={5,-59})));
  Control_and_Distribution.Delay                delay1(Ti=10) annotation (
  ...
  BaseClasses.Peaking_Operational_Logic
    ...
    Ti_Discharge=0.01) annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-8,110})));
  Control_and_Distribution.PI_Control_Reset_Input                PI_DFV2(
    k=1,
    Ti=25,
    k_s=5e-8,
    k_m=5e-8) annotation (Placement(transformation(extent={{64,108},{84,128}})));
  Control_and_Distribution.Timer timer1(Start_Time=30) annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={104,102})));
  Control_and_Distribution.MinMaxFilter                minMaxFilter1
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        rotation=90,
        origin={104,68})));
  Control_and_Distribution.PI_Control_Reset_Input                PI_TBV_Elec(
    k=1,
    Ti=100,
    k_s=1e-8,
    k_m=1e-8,
  ...
  Control_and_Distribution.Timer timer(Start_Time=30) annotation (Placement(
        ...
        rotation=90,
        origin={-142,50})));
  Control_and_Distribution.MinMaxFilter                minMaxFilter
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-172,-12})));
  Control_and_Distribution.Delay                delay2(Ti=0.001)
    annotation (Placement(transformation(extent={{-16,52},{4,72}})));
  Control_and_Distribution.PI_Control_Reset_Input                PI_TBV_Rx(
    k=-1e-11,
    Ti=10,
    initType=Modelica.Blocks.Types.Init.InitialOutput,
  ...
  Modelica.Blocks.Sources.Constant const1(k=162e6)
    annotation (Placement(transformation(extent={{-2,-2},{2,2}},
        rotation=90,
        origin={-88,70})));
  Control_and_Distribution.PI_Control_Reset_Input                PI_FCV(
    k=1,
    Ti=30,
    k_s=1e-6,
    k_m=1e-6,
