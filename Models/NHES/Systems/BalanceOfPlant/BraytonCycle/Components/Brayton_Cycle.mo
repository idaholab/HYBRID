within NHES.Systems.BalanceOfPlant.BraytonCycle.Components;
model Brayton_Cycle
  extends BalanceOfPlant.BraytonCycle.BaseClasses.Partial_SubSystem_A(
      redeclare replaceable
      NHES.Systems.BalanceOfPlant.BraytonCycle.CS.CS_Dummy CS, redeclare replaceable
  ...
  Fluid.HeatExchangers.Generic_HXs.NTU_HX_SinglePhase Reheater(
    ...
    use_T_start_tube=data.use_T_Tube,
    T_start_tube_inlet=if data.use_T_Tube then data.Recuperator_T_Tube_Inlet
         else Medium.temperature_ph(data.Recuperator_P_Tube, data.Recuperator_h_Tube_Outlet),

    T_start_tube_outlet=if data.use_T_Tube then data.Recuperator_T_Tube_Outlet
         else Medium.temperature_ph(data.Recuperator_P_Tube - data.Recuperator_dp_Tube,
        data.Recuperator_h_Tube_Inlet),
    h_start_tube_inlet=if data.use_T_Tube then Medium.specificEnthalpy_pT(data.Recuperator_P_Tube,
        data.Recuperator_T_Tube_Inlet) else data.Recuperator_h_Tube_Inlet,
    h_start_tube_outlet=if data.use_T_Tube then Medium.specificEnthalpy_pT(data.Recuperator_P_Tube - data.Recuperator_dp_Tube,
        data.Recuperator_T_Tube_Outlet) else data.Recuperator_h_Tube_Outlet,
    p_start_shell=data.Recuperator_P_Tube,
    use_T_start_shell=data.use_T_shell,
    T_start_shell_inlet=if data.use_T_shell then data.Recuperator_T_Shell_Inlet
         else Medium.temperature_ph(data.Recuperator_P_Shell, data.Recuperator_h_Shell_Inlet),

    T_start_shell_outlet=if data.use_T_shell then data.Recuperator_T_Shell_Outlet
         else Medium.temperature_ph(data.Recuperator_P_Shell - data.Recuperator_dp_Shell,
        data.Recuperator_h_Shell_Outlet),
    h_start_shell_inlet=if data.use_T_shell then Medium.specificEnthalpy_pT(
        data.Recuperator_P_Shell, data.Recuperator_T_Shell_Inlet) else data.Recuperator_h_Shell_Inlet,

    h_start_shell_outlet=if data.use_T_shell then Medium.specificEnthalpy_pT(
        data.Recuperator_P_Shell - data.Recuperator_dp_Shell, data.Recuperator_T_Shell_Outlet)
         else data.Recuperator_h_Shell_Outlet,
    dp_init_tube=data.Recuperator_dp_Tube,
    dp_init_shell=data.Recuperator_dp_Shell,
    Q_init=data.HX_Aux_Q_Init,
    Cr_init=0.8,
