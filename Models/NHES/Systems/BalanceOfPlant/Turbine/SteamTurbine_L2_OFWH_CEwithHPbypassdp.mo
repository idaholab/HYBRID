...
model SteamTurbine_L2_OFWH_CEwithHPbypassdp
  ...
  Fluid.Valves.PressureReliefValve pressureReliefValve(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    m_flow_nominal=1,
    dp_nominal=2000000)
    annotation (Placement(transformation(extent={{-166,68},{-186,88}})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph boundary(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    p=2000000,
    nPorts=1)
    annotation (Placement(transformation(extent={{-230,68},{-210,88}})));
equation
  ...
  connect(threeWayValveGrad.port_b, prt_b_steamdump) annotation (Line(points={{
          ...
          255}));
  connect(threeWayValveGrad.port_a2, resistance.port_b) annotation (Line(points
        ={{52,-4},{28,-4},{28,20},{21,20}}, color={0,127,255}));
  connect(resistance.port_a, TCV.port_a) annotation (Line(points={{7,20},{-34,
  ...
  connect(SteamHeader.port_b, pressureReliefValve.port_a) annotation (Line(
        points={{-80,60},{-74,60},{-74,74},{-160,74},{-160,78},{-166,78}},
        color={0,127,255}));
  connect(pressureReliefValve.port_b, boundary.ports[1])
    annotation (Line(points={{-186,78},{-210,78}}, color={0,127,255}));
...
end SteamTurbine_L2_OFWH_CEwithHPbypassdp;
