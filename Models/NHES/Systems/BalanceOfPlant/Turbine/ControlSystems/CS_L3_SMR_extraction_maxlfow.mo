...
model CS_L3_SMR_extraction_maxlfow
  ...
  replaceable Modelica.Blocks.Sources.Ramp           Steam_Extraction(duration=
        1000, startTime=1000) constrainedby
  ...
  Modelica.Blocks.Sources.BooleanStep booleanStep3(startTime=2e7)
    annotation (Placement(transformation(extent={{-176,28},{-156,48}})));
  Modelica.Blocks.Sources.RealExpression T_feed_set1(y=0)
...
equation
  ...
  connect(actuatorBus.LPT2_BV, switch5.y) annotation (Line(
      ...
      extent={{-3,-6},{-3,-6}},
      horizontalAlignment=TextAlignment.Right));
  connect(booleanStep3.y, LPT2_BV_PID.trigger) annotation (Line(points={{-155,
          38},{-106,38},{-106,34},{-16,34},{-16,30},{-8,30},{-8,38}}, color={
          255,0,255}));
  connect(T_feed_set1.y, LPT2_BV_PID.y_reset_in) annotation (Line(points={{-123,
          24},{-110,24},{-110,36},{-22,36},{-22,42},{-12,42}}, color={0,0,127}));
  connect(actuatorBus.LPT1_BV, Steam_Extraction.y) annotation (Line(
      points={{30,-100},{30,-74},{-74,-74},{-74,-30},{-79,-30}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
...
end CS_L3_SMR_extraction_maxlfow;
