...
model CS_Rankine_Xe100_Based_Secondary_Bypass

  extends BaseClasses.Partial_ControlSystem;
  parameter Modelica.Units.SI.MassFlowRate m_flow_bypass_demand = 5;
  ...
  TRANSFORM.Controls.LimPID PI_ProcBV(
    controllerType=Modelica.Blocks.Types.SimpleController.PI,
    k=1e-4,
    Ti=0.5,
    yMax=1.0,
  ...
  Modelica.Blocks.Sources.Constant const1(k=m_flow_bypass_demand)
...
equation
  ...
  connect(actuatorBus.opening_TCV, add1.y) annotation (Line(
      ...
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(PI_ProcBV.u_s, const1.y)
    annotation (Line(points={{-12,108},{-29,108}}, color={0,0,127}));
  connect(sensorBus.Process_Steam_Flow_Rate, PI_ProcBV.u_m) annotation (Line(
      points={{-30,-100},{-104,-100},{-104,86},{0,86},{0,96}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(actuatorBus.PBV_Position, PI_ProcBV.y) annotation (Line(
      points={{30,-100},{30,108},{11,108}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
annotation(defaultComponentName="changeMe_CS", Icon(graphics));
end CS_Rankine_Xe100_Based_Secondary_Bypass;
