...
model SteamTurbine_L1_boundariesMEE
  ...
  TRANSFORM.Fluid.Volumes.IdealCondenser
    ...
    set_m_flow=true,
    p=200000)
    annotation (Placement(transformation(extent={{77,-104},{97,-84}})));
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph Condensate_Out(
    ...
    p=10000,
    nPorts=1)
    annotation (Placement(transformation(extent={{200,-136},{222,-116}})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph Brine_Oulet10(
    ...
    p=10000,
    X={0.92,0.08},
    nPorts=1) annotation (Placement(transformation(extent={{468,-68},{446,-48}})));
  Desalination.MEE.Multiple_Effect.MEE_PF8_FC
                          mEE_PF8_FC(redeclare Desalination.MEE.ControlSystems.CS_PressureControlSystem CS)
    annotation (Placement(transformation(extent={{302,-88},{410,20}})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph Brine_Source(
    ...
    h=1.5e5,
    X={0.92,0.08},
    nPorts=1) annotation (Placement(transformation(extent={{466,-18},{444,2}})));
  TRANSFORM.Fluid.Sensors.MassFlowRate sensor_m_flow(redeclare package Medium = Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{254,-10},{274,10}})));
  TRANSFORM.Fluid.Sensors.MassFlowRate sensor_m_flow1(redeclare package Medium = Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{256,-132},{276,-112}})));
  Desalination.MEE.Components.GOR
                 gOR(msteamStart=10)
    annotation (Placement(transformation(extent={{274,-86},{294,-66}})));
equation
...
connect(boundary_m_flow_a3[i].ports[1], multiPort1.ports_b[i+2]);
  ...
  connect(condenser.port_b, multiPort.ports_b[1]) annotation (Line(points={{87,-102},{87,-100},{20,-100},{20,-68},{-2,-68},{-2,-76}},
                                                           color={0,127,255}));
  ...
  connect(volume_bypass.port_b, resistance3.port_a)
    annotation (Line(points={{-14,0},{4.44089e-16,0},{4.44089e-16,-3}},
                                                       color={0,127,255}));
  connect(resistance3.port_b, multiPort1.ports_b[1]) annotation (Line(points={{-3.88578e-16,-17},{-3.88578e-16,-44},{80,-44},{80,-48}},
                                                      color={0,127,255}));
  connect(resistance.port_b, multiPort1.ports_b[2]) annotation (Line(points={{80,-37},{80,-48}},
                                            color={0,127,255}));
  ...
  connect(Brine_Oulet10.ports[1],mEE_PF8_FC. Saltwater_Reject_Oulet)
    annotation (Line(points={{446,-58},{420,-58},{420,-55.6},{410,-55.6}},
        color={0,127,255}));
  connect(Brine_Source.ports[1],mEE_PF8_FC. Saltwater_Input) annotation (
      Line(points={{444,-8},{427,-8},{427,-12.4},{410,-12.4}},
                                                         color={0,127,255}));
  connect(sensor_m_flow1.port_b,mEE_PF8_FC. Condensate_Oulet) annotation (
      Line(points={{276,-122},{356,-122},{356,-86.92}},
                                                    color={0,127,255}));
  connect(sensor_m_flow1.port_a,Condensate_Out. ports[1]) annotation (Line(
        points={{256,-122},{230,-122},{230,-126},{222,-126}}, color={0,127,255}));
  connect(sensor_m_flow.port_b,mEE_PF8_FC. Steam_Input) annotation (Line(
        points={{274,0},{292,0},{292,-12.4},{302,-12.4}}, color={0,127,255}));
  connect(sensor_m_flow.m_flow,gOR. SteamFlow) annotation (Line(points={{264,3.6},{264,12},{252,12},{252,-58},{268,-58},{268,-71},{274,-71}},
                                                                        color={0,
          0,127}));
  connect(sensor_m_flow1.m_flow,gOR. CondFlow) annotation (Line(points={{266,-118.4},{266,-81},{274,-81}},
                                color={0,0,127}));
  connect(mEE_PF8_FC.Water_Outlet, condenser.port_a)
    annotation (Line(points={{302,-55.6},{248,-55.6},{248,-78},{80,-78},{80,-87}}, color={0,127,255}));
  connect(multiPort1.port_a, sensor_m_flow.port_a) annotation (Line(points={{80,-56},{80,-62},{246,-62},{246,0},{254,0}}, color={0,127,255}));
  annotation (defaultComponentName="BOP", Icon(coordinateSystem(extent={{-160,-160},{560,140}}),
                                               graphics={
        Rectangle(
        ...
        Rectangle(
          ...
          fillColor={0,0,255},
          fillPattern=FillPattern.HorizontalCylinder)}),
    Diagram(coordinateSystem(extent={{-160,-160},{560,140}})),
    experiment(StopTime=1000));
end SteamTurbine_L1_boundariesMEE;
