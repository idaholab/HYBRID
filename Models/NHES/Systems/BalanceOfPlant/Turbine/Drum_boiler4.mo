...
model Drum_boiler4
  ...
  TRANSFORM.Fluid.Volumes.IdealCondenser
    ...
    set_m_flow=false,
    p=5000)
    annotation (Placement(transformation(extent={{307,86},{327,106}})));
  ...
  TRANSFORM.Fluid.Volumes.Deaerator deaerator(
    ...
    cp_wall=420,
    Twall_start=423.15)
    annotation (Placement(transformation(extent={{264,58},{244,78}})));
  TRANSFORM.Fluid.BoundaryConditions.MassFlowSource_T boundary1(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    use_m_flow_in=true,
    m_flow=65,
    T=438.15,
    nPorts=1) annotation (Placement(transformation(extent={{240,14},{222,32}})));
  Modelica.Blocks.Math.Product product1
    annotation (Placement(transformation(extent={{192,14},{200,22}})));
...
equation
  ...
  connect(steamTurbine1.portLP, condenser.port_a) annotation (Line(points={{92,
          248},{302,248},{302,103},{310,103}}, color={0,127,255}));
  connect(condenser.port_b, pump_SimpleMassFlow1.port_a)
    annotation (Line(points={{317,88},{317,39},{312,39}}, color={0,127,255}));
  connect(PID.y, pump_SimpleMassFlow1.in_m_flow)
    annotation (Line(points={{169,-8},{301,-8},{301,30.97}}, color={0,0,127}));
  ...
  connect(tee.port_3,FeedwaterMixVolume. port_a[1]) annotation (Line(points={{158,132},
          {220,132},{220,142},{235.75,142},{235.75,132}},
        color={0,127,255}));
  connect(R_entry1.port_a,FeedwaterMixVolume. port_b[1])
    annotation (Line(points={{238,107},{238,112},{236,112},{236,120}},
                                                          color={0,127,255}));
  connect(deaerator.steam,R_entry1. port_b) annotation (Line(points={{247,75},{
          247,84},{238,84},{238,93}},           color={0,127,255}));
  connect(steamTurbine.portLP, tee.port_1)
    annotation (Line(points={{142,68},{148,68},{148,122}}, color={0,127,255}));
  connect(tee.port_2, DHX3.Tube_in) annotation (Line(points={{148,142},{148,176},
          {94,176},{94,170},{16,170},{16,178}}, color={0,127,255}));
  connect(pump_SimpleMassFlow1.port_b, deaerator.feed) annotation (Line(points=
          {{290,39},{270,39},{270,75},{261,75}}, color={0,127,255}));
  connect(boundary1.ports[1], deaerator.drain) annotation (Line(points={{222,23},
          {216,23},{216,50},{254,50},{254,60}}, color={0,127,255}));
  connect(const8.y, product1.u1) annotation (Line(points={{177,32},{186,32},{
          186,20.4},{191.2,20.4}}, color={0,0,127}));
  connect(product1.u2, PID.y) annotation (Line(points={{191.2,15.6},{180,15.6},
          {180,-8},{169,-8}}, color={0,0,127}));
  connect(product1.y, boundary1.m_flow_in) annotation (Line(points={{200.4,18},
          {216,18},{216,8},{246,8},{246,30.2},{240,30.2}}, color={0,0,127}));
