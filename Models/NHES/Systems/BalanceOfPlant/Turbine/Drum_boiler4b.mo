...
model Drum_boiler4b
  ...
  TRANSFORM.Fluid.Sensors.PressureTemperature sensor_pT16(redeclare package
      Medium = Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{246,46},{266,66}})));
  ...
  TRANSFORM.Fluid.Volumes.Deaerator deaerator(
    ...
    cp_wall=420,
    Twall_start=423.15)
    annotation (Placement(transformation(extent={{308,48},{288,68}})));
...
equation
  ...
  connect(Turb_Divert_Valve.u_m, sensor_pT16.T) annotation (Line(points={{200,
          198},{200,78},{250,78},{250,72},{272,72},{272,53.8},{262,53.8}},
        color={0,0,127}));
  ...
  connect(pump_SimpleMassFlow1.port_b, deaerator.feed) annotation (Line(points=
          {{368,31},{316,31},{316,65},{305,65}}, color={0,127,255}));
  connect(FeedwaterMixVolume.port_b[1], deaerator.steam) annotation (Line(
        points={{252,90},{252,76},{291,76},{291,65}}, color={0,127,255}));
  connect(deaerator.drain, pump_SimpleMassFlow2.port_a)
    annotation (Line(points={{298,50},{298,33},{280,33}}, color={0,127,255}));
  connect(pump_SimpleMassFlow2.port_b, sensor_pT16.port) annotation (Line(
        points={{258,33},{258,38},{256,38},{256,46}}, color={0,127,255}));
  connect(pump_SimpleMassFlow2.port_b, R_InternalBypass1.port_b) annotation (
      Line(points={{258,33},{252,33},{252,34},{243,34}}, color={0,127,255}));
  connect(R_InternalBypass1.port_a, boundary1.ports[1]) annotation (Line(points
        ={{229,34},{220,34},{220,36},{210,36}}, color={0,127,255}));
