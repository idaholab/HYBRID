...
<p>A two stage turbine rankine cycle with feedwater heating internal to the system - can be externally bypassed or LPT can be bypassed both will feedwater heat post bypass</p>
  ...
  model SteamTurbine_OpenFeedHeat_DivertPowerControl_HTGR "Two stage BOP model"
    ...
    replaceable Data.IntermediateTurbineInitialisation init(
      ...
      InternalBypassValve_mflow_start=0.1,
      HPT_p_a_start=3000000,
      HPT_p_b_start=500000,
      HPT_T_a_start=533.15,
      HPT_T_b_start=463.15,
      LPT_p_a_start=500000)
    annotation (Placement(transformation(extent={{68,120},{88,140}})));
...
<p>No part of this HTGR model should be considered to be optimized. Additionally, thermal mass of the system needs references and then will need to be adjusted (likely through pipes replacing current zero-volume volume nodes) to more appropriately reflect system time constants. </p>
  ...
  model SteamTurbine_UseCase_HTGR_VNfix "Two stage BOP model"
    extends BaseClasses.Partial_SubSystem_C(
      redeclare replaceable
        ControlSystems.CS_SteamTurbine_L2_PressurePowerFeedtemp CS,
      redeclare replaceable ControlSystems.ED_Dummy ED,
  ...
  equation
    ...
    connect(R_entry1.port_b, SHS_charge_control.port_a) annotation (Line(points={
            {-92,-121},{-92,-102},{-70,-102}}, color={0,127,255}));
    connect(HPT.portLP, LPT.portHP) annotation (Line(points={{52,40},{64,40},{
            64,-30},{52,-30}}, color={0,127,255}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false), graphics={
          Rectangle(
            extent={{-24,2},{24,-2}},
            lineColor={0,0,0},
...
<p>A two stage turbine rankine cycle with feedwater heating internal to the system - can be externally bypassed or LPT can be bypassed both will feedwater heat post bypass</p>
  ...
  end SteamTurbine_UseCase_HTGR_VNfix;

  model SteamTurbine_OpenFeedHeat_DivertPowerControl_HTGR_AR1
    "Two stage BOP model"
    extends BaseClasses.Partial_SubSystem_C(
      redeclare replaceable
        ControlSystems.CS_SteamTurbine_L2_PressurePowerFeedtemp CS,
    ...
    TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance R_entry1(R=10,
      ...
      annotation (Placement(transformation(
          ...
          rotation=270,
          origin={-92,-128})));
    Modelica.Blocks.Sources.RealExpression W_1stpump(y=firstfeedpump.port_b.m_flow
          /firstfeedpump.Medium.density_ph(firstfeedpump.port_b.p,
          firstfeedpump.port_b.h_outflow)*(firstfeedpump.port_b.p -
          firstfeedpump.port_a.p))
      "Electricity loss/gain not accounted for in connections (e.g., heating/cooling, pumps, etc.) [W]"
      annotation (Placement(transformation(extent={{272,78},{284,90}})));
    Modelica.Blocks.Math.Add         add1(k1=1)
      annotation (Placement(transformation(extent={{224,68},{204,88}})));
    Modelica.Blocks.Math.Add         add2(k1=-1)
      annotation (Placement(transformation(extent={{166,82},{146,102}})));
    Modelica.Blocks.Sources.RealExpression W_1stpump1(y=pump_SimpleMassFlow1.W)
      "Electricity loss/gain not accounted for in connections (e.g., heating/cooling, pumps, etc.) [W]"
      annotation (Placement(transformation(extent={{242,104},{254,116}})));
  initial equation
  ...
  equation
    ...
    connect(TCV.port_b, sensor_T1.port_a) annotation (Line(
        ...
        thickness=0.5));
    connect(sensorBus.Feedwater_Temp, sensor_T2.T) annotation (Line(
        points={{-30,100},{-30,14},{-60,14},{-60,-26},{-78,-26},{-78,-58},{-62,
            -58},{-62,-45.6}},
        color={239,82,82},
        pattern=LinePattern.Dash,
        thickness=0.5));
    ...
    connect(firstfeedpump.port_b, sensor_T4.port_b) annotation (Line(points={{30,-128},
            {10,-128}},                              color={0,127,255}));
    connect(pump_SimpleMassFlow1.port_a, sensor_T2.port_b) annotation (Line(
          points={{-98,-41},{-98,-42},{-72,-42}},                      color={0,
            127,255}));
    connect(Condenser.port_b, firstfeedpump.port_a) annotation (Line(points={{146,
            -112},{146,-128},{50,-128}},          color={0,127,255}));
    connect(powerSensor.flange_b, generator1.shaft_a) annotation (Line(points={{72,-76},
            {84,-76},{84,-82}},                      color={0,0,0}));
    connect(sensor_T2.port_a, FeedwaterMixVolume.port_a[1]) annotation (Line(
          points={{-52,-42},{-46,-42},{-46,-40},{-36,-40}}, color={0,127,255}));
    connect(FeedwaterMixVolume.port_b[1], R_InternalBypass.port_b)
      annotation (Line(points={{-24,-40.3333},{-24,-9}},
                                                      color={0,127,255}));
    connect(FeedwaterMixVolume.port_b[2], sensor_T4.port_a) annotation (Line(
    ...
    connect(R_entry1.port_b, SHS_charge_control.port_a) annotation (Line(points={
            {-92,-121},{-92,-102},{-70,-102}}, color={0,127,255}));
    connect(sensorW.W, add1.u2) annotation (Line(points={{120,-37},{120,-30},{
            246,-30},{246,72},{226,72}}, color={0,0,127}));
    connect(add2.u1, W_1stpump1.y) annotation (Line(points={{168,98},{280,98},{
            280,110},{254.6,110}}, color={0,0,127}));
    connect(W_1stpump.y, add1.u1) annotation (Line(points={{284.6,84},{288,84},
            {288,94},{234,94},{234,84},{226,84}},
                                      color={0,0,127}));
    connect(add1.y, add2.u2) annotation (Line(points={{203,78},{176,78},{176,86},
            {168,86}}, color={0,0,127}));
    connect(sensorBus.Power, sensorW.W) annotation (Line(
        points={{-30,100},{128,100},{128,6},{136,6},{136,-30},{120,-30},{120,
            -37}},
        color={239,82,82},
        pattern=LinePattern.Dash,
        thickness=0.5), Text(
...
<p>A two stage turbine rankine cycle with feedwater heating internal to the system - can be externally bypassed or LPT can be bypassed both will feedwater heat post bypass</p>
  ...
  end SteamTurbine_OpenFeedHeat_DivertPowerControl_HTGR_AR1;

  model SteamTurbine_OpenFeedHeat_DivertPowerControl_HTGR_AR1_HighDiversion
    "Two stage BOP model"
    extends BaseClasses.Partial_SubSystem_C(
      redeclare replaceable
        ControlSystems.CS_SteamTurbine_L2_PressurePowerFeedtemp CS,
    ...
    TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance R_entry1(R=10,
      ...
      annotation (Placement(transformation(
          ...
          rotation=270,
          origin={-92,-128})));
    TRANSFORM.Fluid.Valves.ValveLinear SHS_charge_bypass(
      redeclare package Medium = Modelica.Media.Water.StandardWater,
      m_flow_start=400,
      dp_nominal=14500000,
      m_flow_nominal=50) annotation (Placement(transformation(
          ...
          rotation=180,
          origin={-26,-170})));
    TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance R_entry2(R=10,
        redeclare package Medium = Modelica.Media.Water.StandardWater)
      annotation (Placement(transformation(
          extent={{24,-15},{-24,15}},
          rotation=90,
  ...
  equation
    ...
    connect(R_entry1.port_b, SHS_charge_control.port_a) annotation (Line(points={
            {-92,-121},{-92,-102},{-70,-102}}, color={0,127,255}));
    connect(actuatorBus.opening_SHS_condensor, SHS_charge_bypass.opening)
      annotation (Line(
        points={{30,100},{30,54},{12,54},{12,-104},{-26,-104},{-26,-160.4}},
        color={111,216,99},
        pattern=LinePattern.Dash,
        thickness=0.5), Text(
        string="%first",
        ...
        extent={{-3,6},{-3,6}},
        horizontalAlignment=TextAlignment.Right));
    connect(SHS_charge_bypass.port_a, port_a1) annotation (Line(points={{-38,
            -170},{-76,-170},{-76,-160},{-92,-160}}, color={0,127,255}));
    connect(Condenser.port_a, R_entry2.port_b) annotation (Line(points={{153,
            -92},{153,-84},{178,-84},{178,-88},{199,-88},{199,-74.8}}, color={0,
            127,255}));
    connect(LPT.portLP, tee1.port_1) annotation (Line(points={{52,-50},{52,-56},
            {100,-56},{100,-32},{144,-32},{144,16},{214,16},{214,-6},{220,-6}},
          color={0,127,255}));
    connect(SHS_charge_bypass.port_b, tee1.port_2) annotation (Line(points={{
            -14,-170},{246,-170},{246,-6},{240,-6}}, color={0,127,255}));
    connect(tee1.port_3, R_entry2.port_a) annotation (Line(points={{230,-16},{
            230,-28},{199,-28},{199,-41.2}}, color={0,127,255}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false), graphics={
          Rectangle(
            extent={{-24,2},{24,-2}},
            lineColor={0,0,0},
...
<p>A two stage turbine rankine cycle with feedwater heating internal to the system - can be externally bypassed or LPT can be bypassed both will feedwater heat post bypass</p>
  ...
  end SteamTurbine_OpenFeedHeat_DivertPowerControl_HTGR_AR1_HighDiversion;
