...
model SteamTurbine_L2_HPOFWH_CE
  "Two Stage Turbine with open feed water heating using high pressure steam, with controlled steam extraction"
  ...
  TRANSFORM.Fluid.Volumes.SimpleVolume OFWH_1(redeclare package Medium =
    ...
    redeclare model Geometry =
        TRANSFORM.Fluid.ClosureRelations.Geometry.Models.LumpedVolume.GenericVolume
        (V=2)) annotation (Placement(transformation(extent={{8,-70},{28,-50}})));
  ...
  TRANSFORM.Fluid.Sensors.MassFlowRate sensor_m_flow(redeclare package Medium
    ...
    annotation (Placement(transformation(extent={{10,10},{-10,-10}},
        rotation=90,
        origin={8,-26})));
  TRANSFORM.Fluid.Volumes.MoistureSeparator separator
  ...
  TRANSFORM.Fluid.Valves.ValveLinear ECV(
    ...
    m_flow_nominal=data.mdot_lpt1) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={50,60})));
  TRANSFORM.Fluid.Sensors.Pressure IP_sensor(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
  ...
  TRANSFORM.Fluid.Valves.ValveLinear TBV(
    ...
    m_flow_nominal=data.mdot_total) annotation (Placement(transformation(
        ...
        rotation=270,
        origin={-20,10})));
  TRANSFORM.Fluid.Sensors.Pressure Imp_sensor(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{-56,60},{-36,80}})));
  TRANSFORM.Fluid.FittingsAndResistances.TeeJunctionVolume LPT1_bypass1(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    V=1,
    p_start=data.HPT_p_out,
    T_start=406.65)
    annotation (Placement(transformation(extent={{50,-50},{30,-70}})));
  TRANSFORM.Fluid.Sensors.Temperature DH_return(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
...
equation
  ...
  connect(HPT.shaft_b, LPT1.shaft_a) annotation (Line(points={{-26,54},{-26,44},
          {56,44},{56,54},{66,54}},
                              color={0,0,0}));
  connect(condenser.port_b, pump.port_a)
    annotation (Line(points={{80,-46},{78,-46},{78,-60}}, color={0,127,255}));
  connect(pump1.port_a, OFWH_1.port_a)
    annotation (Line(points={{0,-60},{12,-60}}, color={0,127,255}));
  connect(pump1.port_b, OFWH_2.port_b)
    annotation (Line(points={{-20,-60},{-38,-60}}, color={0,127,255}));
  ...
  connect(HPT.portLP, LPT1_bypass.port_1)
    annotation (Line(points={{-26,60},{-2,60}},  color={0,127,255}));
  ...
  connect(actuatorBus.opening_TCV, TCV.opening) annotation (Line(
      points={{30.1,100.1},{30.1,82},{-60,82},{-60,68}},
      color={111,216,99},
  ...
  connect(actuatorBus.LPT2_BV, HPT_bypass_valve.opening) annotation (Line(
      points={{30,100},{30,38},{-64,38},{-64,-28},{-42,-28}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(FWCP.port_b, Feed_T.port)
    annotation (Line(points={{-74,-60},{-80,-60}}, color={0,127,255}));
  ...
  connect(actuatorBus.Feed_Pump_Speed, FWCP.inputSignal) annotation (Line(
      points={{30,100},{30,38},{-64,38},{-64,-52.7}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(HPT_bypass_valve.port_b, OFWH_2.port_b) annotation (Line(points={{-34,-38},
          {-34,-60},{-38,-60}},                               color={0,127,
          255}));
  connect(SteamHeader.port_b, HPT_bypass_valve.port_a) annotation (Line(
        points={{-80,60},{-74,60},{-74,32},{-34,32},{-34,-18}},
        color={0,127,255}));
  connect(sensorBus.Extract_flow, sensor_m_flow.m_flow) annotation (Line(
      points={{-30,100},{-30,144},{120,144},{120,-26},{11.6,-26}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(sensor_m_flow.port_a, LPT1_bypass_valve.port_b)
    annotation (Line(points={{8,-16},{8,0}},   color={0,127,255}));
  connect(sensor_m_flow.port_b, port_b_bypass)
    annotation (Line(points={{8,-36},{8,-100},{40,-100}},
                                                color={0,127,255}));
  connect(LPT1.shaft_b, generator.shaft)
    annotation (Line(points={{86,54},{86,52},{100,52}}, color={0,0,0}));
  connect(LPT1.portLP, condenser.port_a) annotation (Line(points={{86,60},{90,
          60},{90,54},{87,54},{87,-31}},    color={0,127,255}));
  connect(LPT1_bypass.port_2, separator.port_a)
    annotation (Line(points={{18,60},{24,60}}, color={0,127,255}));
  connect(separator.port_Liquid, OFWH_1.port_b)
    annotation (Line(points={{26,56},{26,-60},{24,-60}}, color={0,127,255}));
  connect(actuatorBus.HPT_pArc, HPT.partialArc) annotation (Line(
      points={{30,100},{30,38},{-41,38},{-41,50}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
      ...
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(separator.port_b, ECV.port_a)
    annotation (Line(points={{36,60},{40,60}}, color={0,127,255}));
  connect(ECV.port_b, LPT1.portHP)
    annotation (Line(points={{60,60},{66,60}}, color={0,127,255}));
  connect(actuatorBus.ECV, ECV.opening) annotation (Line(
      points={{30,100},{30,82},{50,82},{50,68}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
      ...
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
  connect(HPT.portLP, IP_sensor.port)
    annotation (Line(points={{-26,60},{-16,60}}, color={0,127,255}));
  connect(sensorBus.I_pressure, IP_sensor.p) annotation (Line(
      points={{-30,100},{-30,70},{-22,70}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(TBV.port_a, SteamHeader.port_b) annotation (Line(points={{-20,20},{
          -20,32},{-74,32},{-74,60},{-80,60}}, color={0,127,255}));
  connect(actuatorBus.TBV, TBV.opening) annotation (Line(
      points={{30,100},{30,38},{-6,38},{-6,10},{-12,10}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
      ...
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
  connect(TBV.port_b, sensor_m_flow.port_a) annotation (Line(points={{-20,
          -3.55271e-15},{-20,-10},{8,-10},{8,-16}}, color={0,127,255}));
  connect(HPT.portHP, Imp_sensor.port)
    annotation (Line(points={{-46,60},{-46,60}}, color={0,127,255}));
  connect(sensorBus.Imp_pressure, Imp_sensor.p) annotation (Line(
      points={{-30,100},{-30,70},{-40,70}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
      ...
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(LPT1_bypass1.port_3, port_a_cond) annotation (Line(points={{40,-70},{
          40,-80},{-40,-80},{-40,-100}}, color={0,127,255}));
  connect(LPT1_bypass1.port_2, OFWH_1.port_b)
    annotation (Line(points={{30,-60},{24,-60}}, color={0,127,255}));
  connect(LPT1_bypass1.port_1, pump.port_b)
    annotation (Line(points={{50,-60},{58,-60}}, color={0,127,255}));
  connect(DH_return.port, port_a_cond) annotation (Line(points={{-80,-88},{-40,
          -88},{-40,-100}}, color={0,127,255}));
  connect(sensorBus.DH_re_T, DH_return.T) annotation (Line(
      points={{-30,100},{-30,144},{-120,144},{-120,-98},{-86,-98}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
