...
model RH_cycle_drumOFH_salt_TCV
...
replaceable package Medium = Modelica.Media.Water.StandardWater annotation (__Dymola_choicesAllMatching=true);
  ...
  TRANSFORM.Fluid.Valves.ValveLinear TCV(
    redeclare package Medium = Modelica.Media.Water.StandardWater,
    dp_nominal=30000,
    m_flow_nominal=57) annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=180,
        origin={36,72})));
  Modelica.Blocks.Sources.Constant const4(k=1)
    annotation (Placement(transformation(extent={{28,96},{40,108}})));
  Modelica.Blocks.Sources.Ramp ramp(
    height=-0.2,
    duration=300,
    offset=1,
    startTime=2000)
    annotation (Placement(transformation(extent={{24,120},{38,134}})));
equation
  ...
  connect(DHX2.Tube_out, sensor_pT8.port)
    annotation (Line(points={{-30,62},{-30,78},{4,78}},  color={0,127,255}));
  ...
  connect(actuatorBus.PartialAdmission, steamTurbine.partialArc) annotation (
      Line(
      points={{2,312},{-162,312},{-162,114},{66,114},{66,80},{65,80}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(sensorBus.Steam_Temperature, sensor_pT8.T) annotation (Line(
      points={{-58,312},{-166,312},{-166,112},{24,112},{24,85.8},{10,85.8}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
  ...
  connect(const1.y, valveLinear.opening) annotation (Line(points={{-219,-70},{-219,
          -72},{-76,-72}}, color={0,0,127}));
  ...
  connect(delay3.y, pump.in_m_flow) annotation (Line(points={{67.02,-64},{48,-64},
          {48,-36},{32,-36},{32,-26},{21.3,-26}},
                                     color={0,0,127}));
  connect(steamTurbine.portHP, TCV.port_b) annotation (Line(points={{60,70},{54,
          70},{54,72},{46,72}}, color={0,127,255}));
  connect(TCV.port_a, DHX2.Tube_out)
    annotation (Line(points={{26,72},{-30,72},{-30,62}}, color={0,127,255}));
  connect(ramp.y, TCV.opening) annotation (Line(points={{38.7,127},{50,127},{50,
