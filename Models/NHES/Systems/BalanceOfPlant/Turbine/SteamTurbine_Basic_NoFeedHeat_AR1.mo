...
model SteamTurbine_Basic_NoFeedHeat_AR1 "Two stage BOP model"
...
equation
  ...
  connect(LPT.shaft_b, powerSensor.flange_a)
    annotation (Line(points={{44,-50},{44,-76},{52,-76}}, color={0,0,0}));
  connect(LPT.portLP, Condenser.port_a) annotation (Line(points={{50,-50},{50,
  ...
  connect(TCV_LPT.port_a, LPT.portHP)
    annotation (Line(points={{96,72},{50,72},{50,-30}}, color={0,127,255}));
  ...
  connect(header.port_a[1], port_a) annotation (Line(points={{-142,40},{-152,40},
  ...
  connect(sensorBus.Power, sensorW.W) annotation (Line(
      points={{-30,100},{152,100},{152,16},{120,16},{120,-37}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5), Text(
      string="%first",
...
end SteamTurbine_Basic_NoFeedHeat_AR1;
