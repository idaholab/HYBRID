...
model SteamTurbine_L1_boundariesMEE2
  ...
  TRANSFORM.Fluid.Volumes.IdealCondenser
    ...
    set_m_flow=true,
    p=200000)
    annotation (Placement(transformation(extent={{87,-124},{107,-104}})));
  ...
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump(redeclare package Medium = Modelica.Media.Water.StandardWater, use_input=true)
    annotation (Placement(transformation(extent={{-68,-104},{-88,-84}})));
  Desalination.MEE.Multiple_Effect.MEE_PF8_FC
                          mEE_PF8_FC(redeclare Desalination.MEE.ControlSystems.CS_TemperatureControlSystem CS)
    annotation (Placement(transformation(extent={{108,-90},{148,-50}})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph Brine_Oulet(
    ...
    p=10000,
    X={0.92,0.08},
    nPorts=1) annotation (Placement(transformation(extent={{188,-70},{168,-90}})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph Brine_Source(
    ...
    h=1.5e5,
    X={0.92,0.08},
    nPorts=1) annotation (Placement(transformation(extent={{188,-70},{168,-50}})));
  TRANSFORM.Fluid.Sensors.MassFlowRate sensor_m_flow1(redeclare package Medium = Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{138,-130},{158,-110}})));
...
connect(boundary_m_flow_a3[i].ports[1], multiPort1.ports_b[i+2]);
  ...
  connect(volume_bypass.port_b, resistance3.port_a)
    annotation (Line(points={{-14,0},{4.44089e-16,0},{4.44089e-16,-3}},
                                                       color={0,127,255}));
  connect(resistance3.port_b, multiPort1.ports_b[1]) annotation (Line(points={{-3.88578e-16,-17},{-3.88578e-16,-56},{90,-56},{90,-42},{80,-42},{
          80,-44},{76,-44}},                          color={0,127,255}));
  connect(resistance.port_b, multiPort1.ports_b[2]) annotation (Line(points={{80,-37},{80,-44},{76,-44}},
                                            color={0,127,255}));
  ...
  connect(level_measure.y, PID1.u_m)
    annotation (Line(points={{-73,-52},{-56,-52}}, color={0,0,127}));
  connect(level_setpoint.y, PID1.u_s)
    annotation (Line(points={{-73,-40},{-68,-40}}, color={0,0,127}));
  connect(condenser.port_b, reservoir.port_a)
    annotation (Line(points={{97,-122},{96,-122},{96,-126},{64,-126},{64,-86},{0,-86},{0,-91.6}}, color={0,127,255}));
  connect(reservoir.port_b, pump.port_a) annotation (Line(points={{0,-108.4},{0,-114},{-60,-114},{-60,-94},{-68,-94}}, color={0,127,255}));
  connect(pump.port_b, port_b) annotation (Line(points={{-88,-94},{-144,-94},{-144,-40},{-160,-40}}, color={0,127,255}));
  connect(PID1.y, pump.in_m_flow) annotation (Line(points={{-45,-40},{-36,-40},{-36,-80},{-78,-80},{-78,-86.7}}, color={0,0,127}));
  connect(Brine_Oulet.ports[1],mEE_PF8_FC. Saltwater_Reject_Oulet)
    annotation (Line(points={{168,-80},{154,-80},{154,-78},{148,-78}},
                                                                     color={0,127,255}));
  connect(Brine_Source.ports[1],mEE_PF8_FC. Saltwater_Input) annotation (Line(points={{168,-60},{154,-60},{154,-62},{148,-62}},
                                                                                                                          color={0,127,255}));
  connect(mEE_PF8_FC.Condensate_Oulet,sensor_m_flow1. port_a) annotation (Line(points={{128,-89.6},{128,-120},{138,-120}},
                                                                                                                      color={0,127,255}));
  connect(sensor_m_flow1.port_b,Condensate_Out. ports[1]) annotation (Line(points={{158,-120},{168,-120}},
                                                                                                         color={0,127,255}));
  connect(multiPort1.port_a, mEE_PF8_FC.Steam_Input) annotation (Line(points={{76,-52},{76,-62},{108,-62}}, color={0,127,255}));
  connect(mEE_PF8_FC.Water_Outlet, condenser.port_a) annotation (Line(points={{108,-78},{90,-78},{90,-107}}, color={0,127,255}));
...
end SteamTurbine_L1_boundariesMEE2;
