within NHES.Systems.BalanceOfPlant.StagebyStageTurbine.Examples;
model Eight_Stage_Turbine
  ...
  Boundary_Conditions.Boundary_ph                   boundary(
    redeclare package Medium = Modelica.Media.Examples.TwoPhaseWater,
    use_p_in=false,
    p(displayUnit="Pa") = 8000,
    h=2383e3,
    nPorts=1) annotation (Placement(transformation(extent={{176,-2},{156,18}})));

  MS                                        MoistSep3(eta=0.2)
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph boundary2(
    ...
    annotation (Placement(transformation(extent={{-3,-4},{3,4}},
        rotation=90,
        origin={122,-1})));
  Turbine_Physical                                                 turbine_Editable(nSt=8)
  ...
  Boundary_Conditions.MassFlowSource_h                              boundary1(
    v_rotational=0,
    redeclare package Medium = Modelica.Media.Examples.TwoPhaseWater,
    use_m_flow_in=true,
    m_flow=68.404,
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_ph boundary3(
    ...
    annotation (Placement(transformation(extent={{-3,-4},{3,4}},
        rotation=90,
        origin={78,-1})));
  Turbine_Tap                                      turbine_Tap2
    annotation (Placement(transformation(extent={{38,0},{44,16}})));
  NHES.Systems.BalanceOfPlant.Control_and_Distribution.ValveLineartanh valveLineartanh2(
    redeclare package Medium = Modelica.Media.Examples.TwoPhaseWater,
    dp_nominal=10,
    m_flow_nominal=40)
  ...
  NHES.Systems.BalanceOfPlant.Control_and_Distribution.ValveLineartanh valveLineartanh1(
    redeclare package Medium = Modelica.Media.Examples.TwoPhaseWater,
    dp_nominal=10,
    m_flow_nominal=50)
  ...
  NHES.Systems.BalanceOfPlant.StagebyStageTurbine.Turbine_Tap turbine_Tap
  ...
  Modelica.Blocks.Sources.Constant const1(k=3.403)
    annotation (Placement(transformation(extent={{-3,-3},{3,3}},
        rotation=270,
        origin={-49,3})));
  NHES.Systems.BalanceOfPlant.Control_and_Distribution.ValveLineartanh valveLineartanh(
    redeclare package Medium = Modelica.Media.Examples.TwoPhaseWater,
    dp_nominal=10,
