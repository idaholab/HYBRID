...
model Rotor_Stage
  ...
  parameter Real ksq = 0.33;
  parameter Real klin = 0.33;
  parameter Real ktanh = 0.34;
  parameter Modelica.Units.SI.SpecificEnthalpy ref_Eperkg=1e5;
...
//  stream Modelica.SIunits.SpecificEnthalpy hin;
  ...
  parameter Modelica.Units.SI.Area A_flow[nV + 1]=0.1*ones(nV + 1)
    annotation (dialog(group="Geometry"));
  parameter Modelica.Units.SI.Length dz[nV]={0.3,0.1}
    annotation (dialog(group="Geometry"));
  parameter Modelica.Units.SI.Length dheight[nV]=zeros(nV)
    annotation (dialog(group="Geometry"));
  parameter Modelica.Units.SI.Length ri[nV + 1]=zeros(nV + 1)
    annotation (dialog(group="Geometry"));
  parameter Modelica.Units.SI.Length ro[nV + 1]={0.21,0.21,0.21}
    annotation (dialog(group="Geometry"));
  ...
  parameter Modelica.Units.SI.SpecificEnthalpy h_init=3000e3
    annotation (dialog(group="Initialization"));
  parameter Modelica.Units.SI.MassFlowRate m_init=66.3
    annotation (dialog(group="Initialization"));
  parameter Modelica.Units.SI.Pressure p_in_init=500000
    annotation (dialog(group="Initialization"));
  parameter Modelica.Units.SI.Pressure p_out_init=500000
    annotation (dialog(group="Initialization"));
  parameter Modelica.Units.SI.Pressure p_init=0;
  parameter Modelica.Units.SI.Velocity v_the_init=0
    annotation (dialog(group="Initialization"));
  parameter Modelica.Units.SI.Velocity v_r_init=0
    annotation (dialog(group="Initialization"));
...
//  Modelica.SIunits.Power Mech_Q2;
  ...
  BaseClasses.FluidFlow Inlet(redeclare replaceable package
              Medium = Medium) annotation (Placement(transformation(extent={{-112,
            -12},{-92,8}}), iconTransformation(extent={{-118,-22},{-78,18}})));
  BaseClasses.FluidFlow Outlet(redeclare replaceable package
