...
partial model Partial_SubSystem_B
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_a1(redeclare package Medium = Medium_1,
      m_flow(min=if allowFlowReversal then -Constants.inf else 0))
    "Fluid connector a (positive design flow direction is from port_a_1 to port_b_1)"
    annotation (Placement(transformation(extent={{-110,30},{-90,50}}),
        iconTransformation(extent={{-110,30},{-90,50}})));

   Modelica.Fluid.Interfaces.FluidPort_b port_b1(redeclare package Medium = Medium_1,
       m_flow(max=if allowFlowReversal then +Constants.inf else 0))
     "Fluid connector b (positive design flow direction is from port_a_1 to port_b_1)"
     annotation (Placement(transformation(extent={{-110,-50},{-90,-30}}),
         iconTransformation(extent={{-110,-50},{-90,-30}})));

   Modelica.Fluid.Interfaces.FluidPort_a port_a2(redeclare package Medium = Medium_2,
       m_flow(min=if allowFlowReversal then -Constants.inf else 0))
     "Fluid connector a (positive design flow direction is from port_a_2 to port_b_2)"
     annotation (Placement(transformation(extent={{90,-50},{110,-30}}),
         iconTransformation(extent={{90,-50},{110,-30}})));

  Modelica.Fluid.Interfaces.FluidPort_b port_b2(redeclare package Medium = Medium_2,
      m_flow(max=if allowFlowReversal then +Constants.inf else 0))
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_a3[nPorts_a3](redeclare package Medium =
        Medium_3, m_flow(each min=if allowFlowReversal then -Constants.inf else 0))
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_b3[nPorts_b3](redeclare package Medium =
        Medium_3, m_flow(each max=if allowFlowReversal then +Constants.inf else 0))
