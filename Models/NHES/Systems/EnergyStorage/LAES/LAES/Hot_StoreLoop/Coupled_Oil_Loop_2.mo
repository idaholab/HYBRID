...
model Coupled_Oil_Loop_2
  ...
  TRANSFORM.Fluid.BoundaryConditions.MassFlowSource_T boundary11(
    ...
    m_flow=-90,
    T=288.15,
    nPorts=1) annotation (Placement(transformation(extent={{18,192},{-2,212}})));
  Modelica.Blocks.Sources.RealExpression HotTankDemand(y=-90)
    annotation (Placement(transformation(extent={{58,192},{38,212}})));
  Modelica.Blocks.Sources.RealExpression realExpression1(y=-HotTankDemand.y)
    annotation (Placement(transformation(extent={{-178,-4},{-158,16}})));
  Modelica.Blocks.Sources.RealExpression ColdTankDemand(y=-90)
    annotation (Placement(transformation(extent={{46,66},{66,86}})));
  TRANSFORM.Fluid.BoundaryConditions.MassFlowSource_T boundary12(
    redeclare package Medium =
        TRANSFORM.Media.Fluids.Therminol_66.LinearTherminol66_A_250C,
    use_m_flow_in=true,
    use_T_in=false,
    m_flow=-90,
    T=288.15,
    nPorts=1) annotation (Placement(transformation(extent={{78,70},{98,90}})));
  Modelica.Blocks.Sources.RealExpression realExpression2(y=-ColdTankDemand.y)
    annotation (Placement(transformation(extent={{-284,176},{-264,196}})));
equation
  ...
  connect(boundary9.ports[1], Hot_Tank.port_a) annotation (Line(points={{-60,
          200},{-48,200},{-48,214},{-45,214}}, color={0,127,255}));
  connect(boundary11.ports[1], Hot_Tank.port_b) annotation (Line(points={{-2,
          202},{-26,202},{-26,214},{-31,214}}, color={0,127,255}));
  connect(realExpression1.y, boundary.m_flow_in)
    annotation (Line(points={{-157,6},{-138,6}}, color={0,0,127}));
  connect(HotTankDemand.y, boundary11.m_flow_in) annotation (Line(points={{37,
          202},{28,202},{28,210},{18,210}}, color={0,0,127}));
  connect(boundary1.ports[1], Cold_Tank.port_a) annotation (Line(points={{88,38},
          {96,38},{96,51.6},{102.7,51.6}}, color={0,127,255}));
  connect(Cold_Tank.port_b, boundary12.ports[1]) annotation (Line(points={{
          115.3,51.6},{122,51.6},{122,80},{98,80}}, color={0,127,255}));
  connect(ColdTankDemand.y, boundary12.m_flow_in) annotation (Line(points={{67,
          76},{70,76},{70,88},{78,88}}, color={0,0,127}));
  connect(boundary8.m_flow_in, realExpression2.y) annotation (Line(points={{
          -258,168},{-258,178},{-256,178},{-256,186},{-263,186}}, color={0,0,
          127}));
  annotation (
