...
algorithm
  ...
  delTw:=(-Chi/2.) + (0.5)*sqrt((Chi^2. + 4.*Chi*(TbarB - NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.Tsat(
    P_Boiler))));
...
TbarB:=NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.Tsat(
    P_Boiler) - ((NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.Tsat(
    P_Boiler) - T_HT)/BetaB)*(1 -
    exp(-BetaB));

    AA:=T_HT - NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.Tsat(
    P_Boiler);

CC:=NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.Tsat(P_Boiler);
    BB:=T_Boilerexit - NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.Tsat(
    P_Boiler);
...
x_exhturb:=(NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.sg(P_Boiler) -
    NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.sf(Pcond))/
    NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.sfg(Pcond);

h_exhturb:=NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.hf(Pcond) +
    x_exhturb*NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.hfg(Pcond);

Wturb:=m_LPT*(NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.hg(P_Boiler) -
       h_exhturb)*eff/947.817; //MW
