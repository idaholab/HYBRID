...
Twmax:=TsatIHX - 0.0001;
 TwIHX:=Modelica.Math.Nonlinear.solveOneNonlinearEquation(
     function NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.ho_IHX(
       P_IHX=P_IHX,
       ktube=ktube,
       ro_IHX=ro_IHX,
       ri_IHX=ri_IHX,
...
hfg1:=NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.hfg(P_IHX)*((1 +
    ...
    TwIHX))/NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.hfg(P_IHX)));

BB:=NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.Viscosity(TsatIHX,P_IHX)*dihx*(TsatIHX - TwIHX);
...
//m_bypassref=100;
//hsg=1244;
m_tesref:=m_bypassref*(hsg - NHES.Systems.EnergyStorage.SensibleHeatStorage.Components.hf(700))/(cp_tes*(T_setpoint - T_CT));
