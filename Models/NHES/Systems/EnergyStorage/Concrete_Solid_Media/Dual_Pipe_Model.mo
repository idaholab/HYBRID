...
public
  ...
  parameter Modelica.Units.SI.Time tau=0.025
                                            "Smoothing derivative time for heat transfer coefficient";
  ...
  parameter Modelica.Units.SI.SpecificEnthalpy HTF_h_start_hot=300e3 annotation(dialog(tab = "Initialization"));
  parameter Modelica.Units.SI.SpecificEnthalpy HTF_h_start_cold=300e3 annotation(dialog(tab = "Initialization"));
  parameter Modelica.Units.SI.Temperature Hot_Con_Start=500 annotation(dialog(tab = "Initialization"));
  parameter Modelica.Units.SI.Temperature Cold_Con_Start=407 annotation(dialog(tab = "Initialization"));
  ...
  for i in 1:nX loop
   for k in 1:2 loop
    ...
    //Check on all of this. Need to.
      ...
      Q_Exch[i,2] = UA[i,2]*(HTF_State[i,2].T-Con_State[nX+1-i,nY].T);
