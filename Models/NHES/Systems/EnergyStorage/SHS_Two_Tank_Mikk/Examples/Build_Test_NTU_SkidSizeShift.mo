...
model Build_Test_NTU_SkidSizeShift "Using old model to start new one"
  ...
  Two_Tank_SHS_System_NewUpdate
                          two_Tank_SHS_System_NewUpdate(
    redeclare
      NHES.Systems.EnergyStorage.SHS_Two_Tank_Mikk.Controls.CS_Experimental_02
      CS(m_flow_rate_steam_trigger=0.04),
  ...
  Modelica.Fluid.Sources.Boundary_pT boundary1(
    ...
    p=5000000,
    T=343.15,
    nPorts=1) annotation (Placement(transformation(extent={{-144,20},{-124,40}})));
  ...
  Modelica.Blocks.Sources.Constant const(k=boilerDrum.geometry.length*0.5)
...
//  two_Tank_SHS_System_NTU.y = triggeredTrapezoid.u;
 // two_Tank_SHS_System.Produced_steam_flow = valveLinear.port_a.m_flow;
  ...
  connect(CHX_Discharge_T.port_a, two_Tank_SHS_System_NewUpdate.port_ch_b)
    annotation (Line(points={{-78,31},{-58,31},{-58,30.54},{-49.04,30.54}},
                               color={0,127,255}));
  connect(CHX_Discharge_T.port_b, boundary1.ports[1])
    annotation (Line(points={{-98,31},{-112,31},{-112,30},{-124,30}},
                                                   color={0,127,255}));
  connect(boundary5.m_flow_in, realExpression1.y)
    annotation (Line(points={{-142,-10},{-157,-10}}, color={0,0,127}));
  connect(pump.port_a,boilerDrum. downcomerPort) annotation (Line(points={{110,10},
  ...
  connect(realExpression.y, hysteresis.u) annotation (Line(points={{149,-44},{186,
          -44},{186,-34}},       color={0,0,127}));
  ...
  connect(boilerDrum.riserPort, two_Tank_SHS_System_NewUpdate.port_dch_b)
    annotation (Line(points={{88.7,32.2},{80,32.2},{80,-28.62},{46,-28.62}},
        color={0,127,255}));
...
end Build_Test_NTU_SkidSizeShift;
