...
model Two_Tank_SHS_System_NewUpdate_Bypasses_Uncertainty
  ...
  Modelica.Blocks.Sources.RealExpression Charging_HTF_Temp(y=CHX.Shell.medium.T)
    annotation (Placement(transformation(extent={{-128,148},{-108,168}})));
  ...
  BalanceOfPlant.StagebyStageTurbineSecondary.Control_and_Distribution.Delay
    delay1(Ti=5.0)
    annotation (Placement(transformation(extent={{-100,154},{-92,162}})));
  Modelica.Blocks.Math.Add add
    annotation (Placement(transformation(extent={{-86,158},{-80,164}})));
  Modelica.Blocks.Noise.UniformNoise uniformNoise(
    samplePeriod=90,
    y_min=-2,
    y_max=2)
    annotation (Placement(transformation(extent={{-112,170},{-102,180}})));
equation
  ...
  connect(sensorBus.discharging_logical, greaterThreshold.y) annotation (Line(
      ...
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(Charging_HTF_Temp.y, delay1.u)
    annotation (Line(points={{-107,158},{-100.8,158}}, color={0,0,127}));
  connect(add.u2, delay1.y) annotation (Line(points={{-86.6,159.2},{-86.6,160},
          {-91.44,160},{-91.44,158}}, color={0,0,127}));
  connect(uniformNoise.y, add.u1) annotation (Line(points={{-101.5,175},{-86.6,
          175},{-86.6,162.8}}, color={0,0,127}));
  connect(sensorBus.Charging_HTF_Temp, add.y) annotation (Line(
      points={{-30,100},{-76,100},{-76,161},{-79.7,161}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5));
  annotation (experiment(
      ...
      __Dymola_Algorithm="Esdirk45a"), Icon(graphics={
        ...
        Rectangle(
          ...
          fillPattern=FillPattern.HorizontalCylinder,
          lineThickness=1)}));
end Two_Tank_SHS_System_NewUpdate_Bypasses_Uncertainty;
