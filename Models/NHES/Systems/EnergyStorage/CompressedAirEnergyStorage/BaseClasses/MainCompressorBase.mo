...
partial model MainCompressorBase "Gas compressor"
  ...
  SI.Power CompPower "compressor Power";

  Modelica.Fluid.Interfaces.FluidPort_a
                          inlet(redeclare package Medium = Medium, m_flow(min=if allowFlowReversal then -Modelica.Constants.inf
           else 0)) annotation (Placement(transformation(extent={{-100,60},{-60,100}}, rotation=0)));
  Modelica.Fluid.Interfaces.FluidPort_b
                          outlet(redeclare package Medium = Medium, m_flow(max=if allowFlowReversal then +Modelica.Constants.inf
           else 0)) annotation (Placement(transformation(extent={{60,60},{100,100}}, rotation=0)));
