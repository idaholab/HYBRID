...
10, 1.008699476, 1.084706987, 1.164672046, 1.260592087, 1.416422043, 1.556213826];
  ...
  Components.SpeedControlCompressor compressor3_1(
    ...
    Tdes_in=298.15,
    Ndesign=45) annotation (Placement(transformation(extent={{-144,-88},{-84,-28}}, rotation=0)));
  TRANSFORM.Mechanics.Rotational.Sources.VariableSpeed variableSpeed1(w_fixed=50)
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_pT Source(
    ...
    p=10000,
    T=298.15,
    nPorts=1) annotation (Placement(transformation(extent={{-180,-20},{-160,0}})));
  ...
  TRANSFORM.Fluid.Volumes.SimpleVolume_1Port cavern(
    redeclare package Medium = Media.Air,
    p_start=compressor3_1.pstart_out,
    redeclare model Geometry = TRANSFORM.Fluid.ClosureRelations.Geometry.Models.LumpedVolume.GenericVolume (V=15000))
  ...
  Modelica.Blocks.Sources.RealExpression realExpression(y=compressor3_1.CompPower)
...
equation
  connect(variableSpeed1.flange, compressor3_1.shaft_b) annotation (Line(points={{-68,-58},{-96,-58}}, color={0,0,0}));
  connect(compressor3_1.inlet, Source.ports[1])
    annotation (Line(points={{-138,-34},{-138,-10},{-160,-10}}, color={159,159,223}));
  connect(CompValve.port_b, cavern.port) annotation (Line(points={{-48,60},{-48,61},{-16,61}}, color={0,127,255}));
  connect(compressor3_1.outlet, vol1.port_a)
    annotation (Line(points={{-90,-34},{-90,14.8},{-88,14.8}}, color={0,127,255}));
  connect(vol1.port_b, CompValve.port_a) annotation (Line(points={{-88,29.2},{-88,60},{-68,60}}, color={0,127,255}));
