...
10, 1.008699476, 1.084706987, 1.164672046, 1.260592087, 1.416422043, 1.556213826];
  ...
  TRANSFORM.Fluid.Valves.ValveLinear TurbValve(
    redeclare package Medium = Media.Air,
    dp_nominal=1000,
    m_flow_nominal=94) annotation (Placement(transformation(extent={{58,50},{78,70}})));
  Modelica.Blocks.Sources.Trapezoid trapezoid(
    ...
    offset=0.001,
    startTime=100)
    annotation (Placement(transformation(extent={{116,100},{96,120}})));
  ...
  TRANSFORM.Fluid.Volumes.SimpleVolume Cavern(
    ...
    annotation (Placement(transformation(extent={{-26,34},{30,88}})));
equation
  ...
  connect(TurbValve.port_b, Pipe2.port_a)
    annotation (Line(points={{78,60},{86,60},{86,24}}, color={0,127,255}));
  ...
  connect(trapezoid.y, TurbValve.opening)
    annotation (Line(points={{95,110},{68,110},{68,68}}, color={0,0,127}));
  ...
  connect(CompValve.port_b, Cavern.port_a)
    annotation (Line(points={{-64,60},{-64,61},{-14.8,61}}, color={0,127,255}));
  connect(Cavern.port_b, TurbValve.port_a)
    annotation (Line(points={{18.8,61},{58,60}}, color={0,127,255}));
  annotation (
