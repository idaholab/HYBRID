...
10, 1.008699476, 1.084706987, 1.164672046, 1.260592087, 1.416422043, 1.556213826];
  ...
  TRANSFORM.Fluid.Valves.ValveLinear CompValve(
    ...
    dp_start=100,
    dp_nominal=1000,
    m_flow_nominal=94) annotation (Placement(transformation(extent={{-86,52},{-66,72}})));
  ...
  TRANSFORM.Fluid.Volumes.SimpleVolume vol1(
    ...
    redeclare model Geometry =
        TRANSFORM.Fluid.ClosureRelations.Geometry.Models.LumpedVolume.GenericVolume (V=10000))
    annotation (Placement(transformation(extent={{-148,46},{-116,76}})));
  TRANSFORM.Fluid.Volumes.SimpleVolume vol2(
    redeclare package Medium = Media.Air,
    p_start=compressor2.pstart_out,
    redeclare model Geometry =
        TRANSFORM.Fluid.ClosureRelations.Geometry.Models.LumpedVolume.GenericVolume (V=10000))
    annotation (Placement(transformation(extent={{84,12},{116,42}})));
equation
  ...
  connect(step.y, CompValve.opening)
    annotation (Line(points={{-87,98},{-76,98},{-76,70}}, color={0,0,127}));
  connect(CompValve.port_b, Cavern.port_a)
    annotation (Line(points={{-66,62},{-38,62},{-38,101},{-18.8,101}}, color={0,127,255}));
  connect(Cavern.port_b, TurbValve.port_a)
    annotation (Line(points={{14.8,101},{46,101},{46,62},{56,62}}, color={0,127,255}));
  connect(vol1.port_a, compressor2.outlet) annotation (Line(points={{-141.6,61},{-150,61},{-150,
          -22},{-108,-22},{-108,-34}}, color={0,127,255}));
  connect(vol1.port_b, CompValve.port_a)
    annotation (Line(points={{-122.4,61},{-122.4,62},{-86,62}}, color={0,127,255}));
  connect(TurbValve.port_b, vol2.port_a)
    annotation (Line(points={{76,62},{82,62},{82,27},{90.4,27}}, color={0,127,255}));
  connect(vol2.port_b, turbine2.inlet) annotation (Line(points={{109.6,27},{122,27},{122,-22},{
          114,-22},{114,-34}}, color={0,127,255}));
  annotation (
