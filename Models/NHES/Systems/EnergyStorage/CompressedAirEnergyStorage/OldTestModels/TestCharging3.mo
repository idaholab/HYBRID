within NHES.Systems.EnergyStorage.CompressedAirEnergyStorage.OldTestModels;
model TestCharging3
  parameter Real tablePhicC[11,7] = [0, 50, 60, 70, 80, 90, 100;
1, 0.00150, 0.00180, 0.00216, 0.00253, 0.00299, 0.00347;
2, 0.00162, 0.00193, 0.00228, 0.00266, 0.00311, 0.00354;
3, 0.00175, 0.00204, 0.00242, 0.00278, 0.00321, 0.00361;
4, 0.00187, 0.00218, 0.00252, 0.00290, 0.00329, 0.00367;
5, 0.00198, 0.00229, 0.00263, 0.00300, 0.00336, 0.00372;
6, 0.00210, 0.00240, 0.00273, 0.00308, 0.00343, 0.00376;
7, 0.00221, 0.00250, 0.00282, 0.00314, 0.00348, 0.00379;
8, 0.00232, 0.00259, 0.00290, 0.00318, 0.00353, 0.00382;
9, 0.00241, 0.00269, 0.00296, 0.00322, 0.00356, 0.00383;
10, 0.00249, 0.00276, 0.00299, 0.00324, 0.00359, 0.00384];

  parameter Real tablePR[11,7] = [0, 50, 60, 70, 80, 90, 100;
1, 1.203154154, 1.307102195, 1.435027275, 1.618785876, 1.870405144, 2.181863019;
2, 1.191305278, 1.299246521, 1.427159716, 1.606942942, 1.85856221, 2.138068526;
3, 1.187436863, 1.283404442, 1.423309129, 1.599081326, 1.838715045, 2.090286774;
4, 1.175582045, 1.27555471, 1.407449223, 1.58723245, 1.802906956, 2.034518617;
5, 1.159739966, 1.255725372, 1.387613943, 1.555429447, 1.755125204, 1.970769999;
6, 1.139910628, 1.235890092, 1.36378546, 1.519615417, 1.703356191, 1.903022236;
7, 1.11609403, 1.20408709, 1.323996054, 1.46783452, 1.643594833, 1.823306752;
8, 1.084296971, 1.172272203, 1.284200706, 1.408067219, 1.575853012, 1.739604007;
9, 1.048500767, 1.128507422, 1.228432549, 1.336332196, 1.50411799, 1.647908917;
10, 1.008699476, 1.084706987, 1.164672046, 1.260592087, 1.416422043, 1.556213826];

  parameter Real tablePhicT[5, 4]=[1, 90, 100, 110; 2.36, 4.68e-3, 4.68e-3,
      4.68e-3; 2.88, 4.68e-3, 4.68e-3, 4.68e-3; 3.56, 4.68e-3, 4.68e-3,
      4.68e-3; 4.46, 4.68e-3, 4.68e-3, 4.68e-3];
  parameter Real tableEtaT[5, 4]=[1, 90, 100, 110; 2.36, 89e-2, 89.5e-2,
      89.3e-2; 2.88, 90e-2, 90.6e-2, 90.5e-2; 3.56, 90.5e-2, 90.6e-2,
      90.5e-2; 4.46, 90.2e-2, 90.3e-2, 90e-2];
  Components.SpeedControlCompressor compressor3(
    redeclare package Medium = Media.Air,
    tablePhic=tablePhicC,
    pstart_in=100000,
    pstart_out=4200000,
    Tstart_in=298.15,
    tablePR=tablePR,
    Table=NHES.Systems.EnergyStorage.CompressedAirEnergyStorage.Data.TableTypes.matrix,
    Tstart_out=298.15,
    explicitIsentropicEnthalpy=true,
    Tdes_in=298.15,
    Ndesign=SpeedControl.w_fixed - 10) annotation (Placement(transformation(extent={{-146,-128},{-86,-68}}, rotation=0)));
  TRANSFORM.Mechanics.Rotational.Sources.VariableSpeed SpeedControl(w_fixed=120)
    annotation (Placement(transformation(extent={{-48,-108},{-68,-88}})));
  inner Modelica.Fluid.System system
    annotation (Placement(transformation(extent={{176,174},{196,194}})));
  TRANSFORM.Fluid.BoundaryConditions.Boundary_pT Source(
    redeclare package Medium = Media.Air,
    p=100000,
    T=298.15,
    nPorts=1) annotation (Placement(transformation(extent={{-180,-60},{-160,-40}})));
  TRANSFORM.Fluid.Valves.ValveLinear CompValve(
    redeclare package Medium = Media.Air,
    dp_start=100,
    dp_nominal=1000,
    m_flow_nominal=94) annotation (Placement(transformation(extent={{-68,50},{-48,70}})));
  TRANSFORM.Fluid.Volumes.SimpleVolume_1Port cavern(
    redeclare package Medium = Media.Air,
    p_start=compressor3.pstart_out,
    redeclare model Geometry = TRANSFORM.Fluid.ClosureRelations.Geometry.Models.LumpedVolume.GenericVolume (V=150000),
    use_HeatPort=true)
    annotation (Placement(transformation(
        extent={{30,-31},{-30,31}},
        rotation=180,
        origin={4,59})));
  TRANSFORM.Fluid.Volumes.SimpleVolume vol1(
    redeclare package Medium = Media.Air,
    p_start=compressor3.pstart_out,
    redeclare model Geometry = TRANSFORM.Fluid.ClosureRelations.Geometry.Models.LumpedVolume.GenericVolume (V=1))
    annotation (Placement(transformation(
        extent={{-12,-12},{12,12}},
        rotation=90,
        origin={-92,-12})));
  Modelica.Blocks.Sources.TimeTable timeTable(table=[0.0,2e8; 1,2e8; 2,2e8; 3,2e8], timeScale(displayUnit="h") = 3600)
    annotation (Placement(transformation(extent={{-190,122},{-170,142}})));
  Modelica.Blocks.Sources.RealExpression realExpression(y=compressor3.CompPower)
    annotation (Placement(transformation(extent={{-188,80},{-168,100}})));
  Modelica.Blocks.Sources.Constant const(k=1) annotation (Placement(transformation(extent={{-188,34},{-168,54}})));
  TRANSFORM.Controls.LimPID PID(
    controllerType=Modelica.Blocks.Types.SimpleController.PI,
    k=-1e-9,
    yMax=0,
    yMin=-0.99999)
                 annotation (Placement(transformation(extent={{-136,122},{-116,142}})));
  Modelica.Blocks.Math.Add add annotation (Placement(transformation(extent={{-88,92},{-68,112}})));
  TRANSFORM.HeatAndMassTransfer.Resistances.Heat.Convection convection(surfaceArea=25000, alpha=30)
    annotation (Placement(transformation(extent={{42,94},{94,142}})));
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature(T=298.15)
    annotation (Placement(transformation(extent={{134,108},{114,128}})));
equation
  connect(SpeedControl.flange, compressor3.shaft_b) annotation (Line(points={{-68,-98},{-98,-98}}, color={0,0,0}));
  connect(compressor3.inlet, Source.ports[1])
    annotation (Line(points={{-140,-74},{-140,-50},{-160,-50}}, color={159,159,223}));
  connect(CompValve.port_b, cavern.port) annotation (Line(points={{-48,60},{-48,59},{-14,59}}, color={0,127,255}));
  connect(compressor3.outlet, vol1.port_a) annotation (Line(points={{-92,-74},{-92,-19.2}}, color={0,127,255}));
  connect(vol1.port_b, CompValve.port_a) annotation (Line(points={{-92,-4.8},{-92,60},{-68,60}}, color={0,127,255}));
  connect(timeTable.y, PID.u_s) annotation (Line(points={{-169,132},{-138,132}}, color={0,0,127}));
  connect(realExpression.y, PID.u_m) annotation (Line(points={{-167,90},{-126,90},{-126,120}}, color={0,0,127}));
  connect(const.y, add.u2) annotation (Line(points={{-167,44},{-100,44},{-100,96},{-90,96}}, color={0,0,127}));
  connect(PID.y, add.u1) annotation (Line(points={{-115,132},{-100,132},{-100,108},{-90,108}}, color={0,0,127}));
  connect(add.y, CompValve.opening) annotation (Line(points={{-67,102},{-58,102},{-58,68}}, color={0,0,127}));
  connect(convection.port_a, cavern.heatPort) annotation (Line(points={{49.8,118},{4,118},{4,77.6}}, color={191,0,0}));
  connect(convection.port_b, fixedTemperature.port) annotation (Line(points={{86.2,118},{114,118}}, color={191,0,0}));
  annotation (
    Diagram(coordinateSystem(
        preserveAspectRatio=false,
        extent={{-200,-200},{200,200}},
        initialScale=0.1)),
    Icon(coordinateSystem(
        preserveAspectRatio=false,
        extent={{-200,-200},{200,200}},
        initialScale=0.1), graphics={Rectangle(
          extent={{-200,200},{200,-200}},
          lineColor={170,170,255},
          fillColor={255,255,255},
          fillPattern=FillPattern.Solid), Text(
          extent={{-140,140},{140,-140}},
          lineColor={170,170,255},
          textString="P")}),
    Documentation(revisions="<html>
<ul>
<li><i>10 Dec 2008</i>
    by <a>Luca Savoldelli</a>:<br>
       First release.</li>
</ul>
</html>",
      info="<html>
<p>This model contains the  gas turbine, generator and network models. The network model is based on swing equation.
</html>"));
end TestCharging3;
