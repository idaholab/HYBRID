...
model Guide_Tube_New_Air "Model describing the guide tube portion of the HITB"
  ...
  input Modelica.Units.SI.Length z_min_heatpipe annotation(Dialog(tab = "External"));
  input Modelica.Units.SI.Length z_max_heatpipe annotation(Dialog(tab = "External"));
  ...
  input Real Q_HP_Tape_Input = 0 annotation(Dialog(tab = "External"));
...
algorithm
  ...
  for i in 1:nV_ZGT_Total loop
    for j in 1:nV_Zh loop
      if z_vec_GT[i+1] <= z_vec_HP[j] or z_vec_GT[i] >= z_vec_HP[j+1] then
        A_HP_Overlap[i,j] := 0; //this is if right edge < left edge or left edge > right edge, there's no overlap, so = 0
  ...
  for i in 1:nV_ZHT loop
    if k == nR then
  Q_HP_Tape[k,i] :=1/(nV_ZHT)*Q_HP_Tape_Input;
    else
      Q_HP_Tape[k,i] := 0;
    end if;
  end for;
