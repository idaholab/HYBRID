...
model PCM_Chamber_vertsym_03_DynamicHPLocs "Contained PCM chamber generalizable interact with a heat pipe 
  ...
  parameter Real HPFracs[len_HPMatrix] = {0.5, 0.7, 0.3}; //Heat transfer (area fraction) fraction of heat pipe that is within the node associated with the nth term of HPMatrix
...
equation
  ...
  for i in 1:n_Thermocouples loop
    for k in 1:nZ loop
    T_TCs[i,k] = Temp_Profile[TCs[i,1],TCs[i,2],k];
    end for;
  end for;
  for i in 1:nR loop
...
equation
  ...
  for j in 1:nTheta loop
    ...
    for k in 1:nZ loop
      SA_shell[j,k] = dthetas_one[j]/(2*Modelica.Constants.pi)*(R_PCM+t_PCM_wall+t_insulation+t_heat_trace)*dzs_one[k];
...
/*  for i in 1:nR loop
  ...
  connect(End_Insulation_1.port_b, convection4.port_b)
    annotation (Line(points={{70,106},{70,119}}, color={191,0,0}));
  connect(End_Insulation_1.port_a, simpleWall1.port_b)
    annotation (Line(points={{70,86},{70,76}}, color={191,0,0}));
