...
model Cylinder_2D_r_theta_interior_pipes
  extends
  ...
  input SI.Length dzs[nR,nTheta](min=0) = fill(
    ...
    annotation (Dialog(group="Inputs"));
    input SI.Volume dVs_int[nR,nTheta] "Volume deficits (should be negative values) due to vessel penetrations" annotation(dialog(tab = "General", group = "Custom Adjustments"));
    input SI.Area dAs_1[nR,nTheta] "Area correction term multiplier in radial direction" annotation(dialog(tab = "General", group = "Custom Adjustments"));
    input SI.Area dAs_2[nR,nTheta] "Area correction term multiplier in angular direction" annotation(dialog(tab = "General", group = "Custom Adjustments"));
...
algorithm
  ...
  for i in 1:nR loop
    for j in 1:nTheta loop
      Vs[i, j] := 0.5*((rs[i, j] + 0.5*drs[i, j])^2 - (rs[i, j] - 0.5*drs[i, j])
        ^2)*dthetas[i, j]*dzs[i, j]+dVs_int[i,j];
    end for;
  end for;
  for j in 1:nTheta loop
    for i in 1:nR loop
      crossAreas_1[i, j] := (rs[i, j] - 0.5*drs[i, j])*dthetas[i, j]*dzs[i, j]*dAs_1[i,j];
    end for;
  ...
  for i in 1:nR loop
    for j in 1:nTheta loop
      crossAreas_2[i, j] := drs[i, j]*dzs[i, j]*dAs_2[i,j];
    end for;
    crossAreas_2[i, nTheta + 1] := drs[i, nTheta]*dzs[i, nTheta]*dAs_2[i,nTheta];
  end for;
  for i in 1:nR loop
    for j in 1:nTheta loop
...
end Cylinder_2D_r_theta_interior_pipes;
