...
model HITB_Experiment_01
  ...
  parameter Modelica.Units.SI.Length R_PCM = 0.5 "Outer radius of PCM container";
  parameter Modelica.Units.SI.Length t_PCM_wall = 0.002 "Outer PCM container wall thickness";

  parameter Modelica.Units.SI.Length l_HITB = 0.6+0.45+0.45;
  parameter Modelica.Units.SI.Length l_CHX = 0.45 "Length of section where heat pipe can be heated by CHX";
  parameter Modelica.Units.SI.Length l_DHX = l_CHX "Length of section where heat pipe can be cooled by DHX";
  parameter Modelica.Units.SI.Length l_gap_CHX = 0.45 "Length of section between the CHX and the PCM";
  parameter Modelica.Units.SI.Length l_gap_DHX = 0.45 "Length of section between the DHX and the PCM";
...
equation
  ...
  for i in 1:nV_Z loop
    connect(HITB_PCM_Gap[i].port_a, collector.port_a[i+2])
    annotation (Line(points={{18,-8},{2,-8}}, color={191,0,0}));
  end for;
  ...
  connect(HITB_PCM_Gap.port_b, PCM_Pipe_Wall.port_a)
    annotation (Line(points={{38,-8},{68,-8}}, color={191,0,0}));
  ...
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-120,-100},
        ...
        Line(
          ...
          thickness=DynamicSelect(0,8*(max(Position,0.5)-0.5))),
        Ellipse(
          extent=DynamicSelect({{-94,4},{-98,-2}},{{-94+64*Position,4},{-98+64*Position,-2}}),
          lineColor={28,108,200},
          fillColor={244,125,35},
          fillPattern=FillPattern.CrossDiag),
        Ellipse(
          extent=DynamicSelect({{30,4},{26,-2}},{{30+64*Position,4},{26+64*Position,-2}}),
          lineColor={28,108,200},
          fillColor={244,125,35},
          fillPattern=FillPattern.CrossDiag),
        Rectangle(
          extent={{-30,4},{22,-2}},
          lineColor={28,108,200},
          fillColor={238,169,170},
          fillPattern=FillPattern.Solid),
        Rectangle(
          extent=DynamicSelect({{-96,4},{28,-2}},{{-96+64*Position,4},{28+64*Position,-2}}),
          lineColor={28,108,200},
