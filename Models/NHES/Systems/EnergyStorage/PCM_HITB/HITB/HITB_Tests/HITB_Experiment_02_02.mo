...
model HITB_Experiment_02_02
  ...
  TRANSFORM.HeatAndMassTransfer.BoundaryConditions.Heat.Temperature Air_PCM_Axial[nV_Z](T=293.15)
    annotation (Placement(transformation(extent={{216,-18},{196,2}})));
...
equation
  ...
  */
  ...
  connect(convection2.port_a, Air_PCM_Axial.port)
    annotation (Line(points={{185,-8},{196,-8}}, color={191,0,0}));
  ...
  connect(signal_Position.u1, const.y) annotation (Line(points={{258,50},{252,
          50},{252,68},{237,68}},
                               color={0,0,127}));
