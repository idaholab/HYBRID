...
model GenericPipe_MultiTransferSurface_MassExchange
  ...
  TRANSFORM.Fluid.Pipes.BaseClasses.Summary summary(
  ...
  TRANSFORM.Fluid.Interfaces.FluidPort_Flow port_a(redeclare package Medium =
        Medium, m_flow(min=if allowFlowReversal then -Modelica.Constants.inf
           else 0)) annotation (Placement(transformation(extent={{-110,-10},{-90,
  ...
  input SI.MassFlowRate[nV] mexch annotation(Dialog(tab = "General",group = "MassExchange"));
  input SI.SpecificEnthalpy[nV] hfg annotation(Dialog(tab = "General", group = "MassExchange"));
  extends TRANSFORM.Fluid.Pipes.BaseClasses.PartialDistributedVolume(
    ...
    hs_start=if not use_Ts_start then linspace_1D(
        h_a_start,
        h_b_start,
        nV) else {Medium.specificEnthalpy_pTX(
        ps_start[i],
        Ts_start[i],
  ...
  TRANSFORM.HeatAndMassTransfer.Interfaces.HeatPort_Flow heatPorts[nV,geometry.nSurfaces]
    if use_HeatTransfer annotation (Placement(transformation(extent={{-10,50},{10,
            70}}), iconTransformation(extent={{-10,40},{10,60}})));
  TRANSFORM.HeatAndMassTransfer.Interfaces.MolePort_Flow massPorts[nV,geometry.nSurfaces](
     each nC=traceMassTransfer.nC) if use_TraceMassTransfer annotation (
      Placement(transformation(extent={{-50,50},{-30,70}}), iconTransformation(
          extent={{-50,40},{-30,60}})));
  // Visualization
...
protected
  TRANSFORM.HeatAndMassTransfer.Interfaces.HeatPort_State[nV,geometry.nSurfaces]
    heatPorts_int;
...
equation
  ...
  for i in 1:nV loop
    mbs[i] = m_flows[i] - m_flows[i + 1]+mexch[i];
    Ubs[i] = Wb_flows[i] + H_flows[i] - H_flows[i + 1] + sum(heatTransfer.Q_flows[i,:]) + internalHeatGen.Q_flows[i]/nParallel+mexch[i]*hfg[i];
...
end GenericPipe_MultiTransferSurface_MassExchange;
