within NHES.Systems.PrimaryHeatSystem.Westinghouse4LoopPWR.Data;
model Data_Basic
  ...
  import TRANSFORM.Units.Conversions.Functions.Distance_m.from_ft;
  import TRANSFORM.Units.Conversions.Functions.Temperature_K.from_degF;
  import TRANSFORM.Units.Conversions.Functions.Volume_m3.from_ft3;
  ...
  import TRANSFORM.Units.Conversions.Functions.MassFlowRate_kg_s.from_lbm_hr;
   ...
   parameter SI.Length dimension_hotleg = from_in(29) "Hot leg inner diameter (d1s1)";
   parameter SI.Length dimension_coldleg = from_in(27.5) "Hot leg inner diameter (d1s1)";
   parameter SI.Length length_hotleg = from_ft(50) "Hot leg length";
   parameter SI.Length length_coldleg = from_ft(50) "Cold leg length";

   parameter SI.Power Q_total_th=3411e6 "Total thermal output (d2s1)";
   parameter TRANSFORM.Units.NonDim Q_frac=0.974
    "Fraction of heat generatated in fuel (d2s1)";
   ...
   parameter SI.Length dimension_core = from_ft(11.06) "Equivalent core diameter (d2s1)";
   parameter SI.Length length_fuel = from_ft(12) "Core length between fuel ends (d2s1)";

   parameter SI.Mass m_uranium_1 = 81639 "Fuel weight of uranium (first core) (d2s1)";
   parameter TRANSFORM.Units.NonDim nAssembly=193
    "Number of assemblies - 17x17 (d2s1)";
   ...
   parameter TRANSFORM.Units.NonDim sizeAssembly=17
    "square size of assembly (e.g., 17 = 17x17)";
   parameter TRANSFORM.Units.NonDim nRodFuel_assembly=264
    "# of fuel rods per assembly (d3s1)";
   parameter TRANSFORM.Units.NonDim nRodNonFuel_assembly=sizeAssembly^2 - 264
    "# of non-fuel rods per assembly (d3s1)";
   ...
   parameter SI.Area crossArea_downcomer = Modelica.Constants.pi*0.25*(from_in(10*12+11)^2 - from_in(10*12+3)^2) "Estimated flow area of downcomer from nozzle faces (d4s1)";
   parameter SI.Length perimeter_downcomer = Modelica.Constants.pi*(from_in(10*12+11)+from_in(10*12+3)) "Estimated perimeter of downcomer";
   parameter SI.Height pressurizer_level=1.68 "nominal pressurizer level";
   parameter TRANSFORM.Units.NonDim pressurizer_Vfrac_liquid=0.34
    "nominal fraction of liquid in pressurizer";

   // Pressurizer
   parameter SI.Length dimension_pzr = from_in(7*12+8) "Pressurizer diameter (d5s1)";
   parameter SI.Length th_pzr = from_in(12) "Guess of pressurizer wall thickness";
   parameter SI.Length length_pzr = from_in(52*12+9) "Pressurizer height (d5s1)";
   parameter SI.Volume V_pzr = 0.25*Modelica.Constants.pi*dimension_pzr^2*length_pzr "Empty volume of pressurizer (d5s1)";
   parameter SI.Volume V_water_pzr_nominal = from_ft3(1080) "Pressurizer nominal water volume (d5s1)";
   parameter SI.Volume V_steam_pzr_nominal = V_pzr-V_water_pzr_nominal "Pressurizer nominal steam volume (d5s1)";
   parameter TRANSFORM.Units.NonDim Vfrac_liquid_pzr=V_water_pzr_nominal/V_pzr
    "Nominal volume fraction of pressurizer";
   ...
   parameter SI.Length length_SGshell = from_in(67*12+8) "Overall height of steam generator (d6s2)";
   ...
   parameter SI.Length diameter_outer_lowerShell = from_in(11*12+3) "Lower shell outer diameter (d6s1)";
   parameter SI.Length diameter_outer_upperShell = from_in(14*12+7.75) "Upper shell outer diameter (d6s1)";
   ...
   parameter SI.Length th_shell = from_in(8) "Estimate of shell thickness (s2 picture)";
