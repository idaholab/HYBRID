...
model SMR_High_fidelity_no_pump_test_full_TES_Salt
  ...
  //Model allows for non-water balance of plant side calculations. Note that steam generator shouldn't be changed geometrically, that's assumed to be unchangeable"
 // import TRANSFORM;
...
Real Tave=(Tcore_inlet.T+Tcore_exit.T)/2.0;
  ...
  TRANSFORM.HeatExchangers.GenericDistributed_HX STHX(
    ...
    redeclare package Medium_tube = Medium_Secondary)                       annotation (Placement(transformation(
        extent={{-12,-11},{12,11}},
        rotation=90,
        origin={27,0})));
  ...
  Modelica.Fluid.Sensors.MassFlowRate massFlowRate1(
      ...
      package Medium = Medium_Secondary)
...
end SMR_High_fidelity_no_pump_test_full_TES_Salt;
