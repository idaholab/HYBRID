...
model HTGR_Open_Turbine_Control
 ...
 max(SC.W_totalSetpoint_BOP/SC.W_nominal_BOP*fracNominal_BOP
  ...
  NHES.Systems.Examples.BaseClasses.Data_Capacity dataCapacity(IP_capacity(
        displayUnit="MW") = 53303300, BOP_capacity(displayUnit="MW") = 60000000)
  ...
  BalanceOfPlant.Turbine.HTGR_RankineCycles.HTGR_Rankine_Cycle_Transient BOP(
      redeclare
      BalanceOfPlant.Turbine.ControlSystems.CS_Rankine_Xe100_Based_Secondary CS)
          annotation (Placement(transformation(extent={{46,-22},{86,22}})));
equation
