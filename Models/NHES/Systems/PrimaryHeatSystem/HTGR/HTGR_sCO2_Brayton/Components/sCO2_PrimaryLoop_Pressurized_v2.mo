...
model sCO2_PrimaryLoop_Pressurized_v2
...
equation
  ...
  connect(ShellInSensor.port_b, PrimaryHX.Shell_in) annotation (Line(points={{54,18},{30.8,18},{30.8,12}},  color={0,127,255}));
  connect(PrimaryHX.Shell_out, ShellOutSensor.port_a) annotation (Line(points={{30.8,-12},{30.8,-34},{52,-34}}, color={0,127,255}));
  connect(Blower.inlet, TubeOutSensor.port_b) annotation (Line(points={{-18,28},{-2,28}}, color={0,127,255}));
  connect(TubeOutSensor.port_a, PrimaryHX.Tube_out) annotation (Line(points={{18,28},{20,28},{20,12},{22.4,12}},
                                                                                                         color={0,127,255}));
  connect(PrimaryHX.Tube_in, CoreOutTemp.port_b) annotation (Line(points={{22.4,-12},{22.4,-32},{10,-32}},  color={0,127,255}));
  ...
  connect(Thermal_Power1.y,Primary_PRV. opening) annotation (Line(points={{13.4,62},{0.4,62}},
                                      color={0,0,127}));
  connect(Primary_PRV.port_b, Pressurizer.ports[1]) annotation (Line(points={{-6,70},{-6,76},{-5,76}},
                                                                                                     color={0,127,255}));
  connect(Primary_PRV.port_a, TubeOutSensor.port_b)
    annotation (Line(points={{-6,54},{-6,28},{-2,28}},                                   color={0,127,255}));
...
end sCO2_PrimaryLoop_Pressurized_v2;
