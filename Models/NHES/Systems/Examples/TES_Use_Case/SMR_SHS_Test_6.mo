 ...
 max(SC.W_totalSetpoint_BOP/SC.W_nominal_BOP*fracNominal_BOP
  ...
  TRANSFORM.Fluid.Valves.ValveLinear SHS_Throttle(
    ...
    m_flow_nominal=30) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={50,-30})));
  BalanceOfPlant.Turbine.Intermediate_Rankine_Cycle_TESUC_SmallCycle_Pressure
    ...
    port_b_nominal(
      ...
      m_flow=-20),
    redeclare
      NHES.Systems.BalanceOfPlant.Turbine.ControlSystems.CS_IntermediateControl_PID_6_Pressure
      CS,
    port_a_start(
      p=1100000,
      h=2e6,
