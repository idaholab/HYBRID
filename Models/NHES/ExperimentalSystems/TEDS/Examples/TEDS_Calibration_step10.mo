...
model TEDS_Calibration_step10 "Test from TEDS calibration experiment step 8"
  ...
  Control_Systems.Control_System_Therminol_4_element_summer_day_real_70F
    control_System_Therminol_4_element_summer_day_real_70F(
    redeclare package Medium =
        TRANSFORM.Media.Fluids.Therminol_66.LinearTherminol66_A_250C,
    T_hot_design(displayUnit="K") = Modelica.Units.Conversions.from_degF(150),
    T_cold_design(displayUnit="K") = Modelica.Units.Conversions.from_degF(70))
    annotation (Placement(transformation(extent={{22,116},{44,138}})));
  ...
  Modelica.Blocks.Sources.Constant const1(k=
        control_System_Therminol_4_element_summer_day_real_70F.T_cold_design)
    annotation (Placement(transformation(
        extent={{4,-4},{-4,4}},
        rotation=0,
        origin={12,-184})));
  ...
  Modelica.Blocks.Sources.Constant const(k=
        control_System_Therminol_4_element_summer_day_real_70F.T_hot_design)
    annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=0,
        origin={-146,46})));
...
equation
  ...
  connect(pipe2.port_b,GBV201. port_a)
    annotation (Line(points={{72,76},{84,76},{84,62}}, color={0,127,255}));
  ...
  connect(control_System_Therminol_4_element_summer_day_real_70F.actuatorSubBus,
    actuatorSubBus) annotation (Line(
      points={{32.7067,116.092},{32.7067,106},{-31,106}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(control_System_Therminol_4_element_summer_day_real_70F.sensorSubBus,
    sensorSubBus) annotation (Line(
      points={{38.1333,116.092},{38.1333,106},{1,106}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
