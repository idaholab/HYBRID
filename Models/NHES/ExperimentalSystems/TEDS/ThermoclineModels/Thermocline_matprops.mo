...
//Tank Parameters
parameter Modelica.Units.SI.Length Radius_Tank=7.3
    "Radius of the thermocline tank [m]";
parameter Real Porosity=0.25 "Porosity in the tank";
Modelica.Units.SI.Area XS_fluid=Porosity*(Radius_Tank^2.0)*Modelica.Constants.pi
    "Cross Sectional Area seen by the fluid at each axial location";
Modelica.Units.SI.Length Height_Tank=14.6 "Tank Height [m]";

//Discretization
parameter Integer nodes=200 "Number of axial nodes";

Modelica.Units.SI.Length dz=Height_Tank/nodes "delta height in each node";

//Attempt to input medium package
Modelica.Units.SI.Density d_T[nodes]=Medium.density(mediums.state);
Modelica.Units.SI.Pressure ps[nodes]=fill(1e5, nodes);
...
Modelica.Units.SI.SpecificHeatCapacity Cf[nodes]=fill(userspecificheat, nodes);
                                                                  //This can be used if the user knows an approximately constant value over their desired operations range, else comment this line and uncomment the one above it.

Medium.BaseProperties mediums[nodes];

//End attempt to input medium package.

Modelica.Units.SI.Temperature Tf[nodes]
    "Fluid Temperature up and down the thermocline";
Modelica.Units.SI.Temperature Tr[nodes]
    "Filler Temperature up and down the thermocline";

//SI.Density fluid_density=753.75 "Fluid Density Assumed Constant for now kg/m^3"; //First go is to match the paper
parameter Modelica.Units.SI.Density filler_density=2630
    "Filler Density (Granite)";
...
parameter Modelica.Units.SI.SpecificHeatCapacity Cr=775.0 "J/kg*K of granite";

//SI.ThermalConductivity kf = 0.086 "W/m*K of fluid";
parameter Modelica.Units.SI.ThermalConductivity kr=2.8 "W/m*K of filler";

Modelica.Units.SI.ReynoldsNumber Re[nodes] "Unitless";
Modelica.Units.SI.MassFlowRate mf;
                     //=-128.74 "kg/s";
Modelica.Units.SI.Velocity vel[nodes] "meter/s";
Modelica.Units.SI.CoefficientOfHeatTransfer h_c[nodes] "W/m^2*K";
Modelica.Units.SI.PrandtlNumber Pr[nodes] "Unitless";
Modelica.Units.SI.Length r_char "hydraulic radius [m]";
parameter Modelica.Units.SI.Length dr=0.04
    "nominal diameter of filler material [m]";

Modelica.Units.SI.ThermalConductivity kf[nodes]=Medium.thermalConductivity(
      mediums.state) "W/m*K of fluid";
Modelica.Units.SI.DynamicViscosity mu_f[nodes]=Medium.dynamicViscosity(mediums.state);
Modelica.Units.SI.Density fluid_density[nodes]=Medium.density(mediums.state);

Modelica.Units.SI.Length Sr
    "Heat Transfer Surface Area of rocks per unit length of tank [m]";
parameter Real fs=3.0 "Surface Shape Factor ,may vary between 2 and 3 
depending rocks packing scheme (default is 3.0)";
parameter Modelica.Units.SI.Temperature T_inlet_hot=395 + 273.15;
parameter Modelica.Units.SI.Temperature T_inlet_cold=310 + 273.15;

Modelica.Units.SI.Temperature Tf_edge[nodes + 1]
    "Boundary Nodes for fluid temperatures";

Modelica.Units.SI.Length z_star[nodes];
