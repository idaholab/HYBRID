...
model Model_1kkk
  ...
  Modelica.Blocks.Interfaces.RealInput u(start=1)
    annotation (Placement(transformation(extent={{-240,40},{-200,80}})));
protected
  Modelica.Blocks.Math.Gain gain(k=data.Q_vc)
    annotation (Placement(transformation(extent={{-142,50},{-122,70}})));
  Modelica.Blocks.Nonlinear.Limiter limiter(uMax=1.0, uMin=0)
  ...
  TRANSFORM.Fluid.Volumes.SimpleVolume vc(
    ...
    redeclare model Geometry =
        ...
        (V=0.1),
    Q_gen=gain.y)      annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=90,
        origin={-92,52})));
...
equation
  ...
  connect(u, limiter.u)
    annotation (Line(points={{-220,60},{-180,60}}, color={0,0,127}));
  connect(limiter.y, gain.u)
    annotation (Line(points={{-157,60},{-144,60}}, color={0,0,127}));
